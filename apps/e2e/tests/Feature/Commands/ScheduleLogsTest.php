...
it('reads schedule run logs from the operator node through the gateway api', function (): void {
    ...
    try {
        ...
        $result = $topology->ssh(
            'operator',
            sprintf(
                'cd %s && orbit schedule:logs %s --instance=%s --json',
        ...
        expect($result->successful())
            ->toBeTrue()
            ->and($payload['success']['data']['run']['status'])
            ->toBe('completed')
            ->and($payload['success']['data']['output']['stdout'])
            ->toContain('e2e-logs-output')
            ->and($payload['success']['meta']['lines'])
            ->toBe(100)
            ->and($payload['success']['meta']['truncated'])
            ->toBeFalse();
    } finally {
        $topology->cleanup();
    }
