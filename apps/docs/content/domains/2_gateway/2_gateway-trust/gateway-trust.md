...
not onboard a client, verify node identity, or mutate gateway fleet
configuration.
...
client onboarding. `gateway:add` uses the same trust behavior as part of
...
- [`gateway:add`](../1_gateway-add/gateway-add.md) - configure a client
