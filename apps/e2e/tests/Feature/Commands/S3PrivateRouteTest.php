...
it('routes the private s3 service to a seaweedfs backend through the router and returns correct credentials', function (): void {
    ...
    try {
        ...
        expect($renderedRoute)
            ->toContain('s3.orbit {')
            ->toContain('reverse_proxy http://app-dev-1.s3.orbit:8333 {')
