...
it('boots as an external harness and resolves Orbit entry points', function (): void {
    ...
    expect($paths->repoRoot())
        ->toBe(repo_path())
        ->and($paths->cliLauncher())
        ->toBe(repo_path('apps/cli/orbit'))
        ->and($paths->gatewayArtisan())
        ->toBe(repo_path('apps/gateway/artisan'))
        ->and($paths->e2eAppRoot())
        ->toBe(repo_path('apps/e2e'))
        ->and($paths->cliLauncher())
        ->toBeFile()
        ->and($paths->gatewayArtisan())
        ->toBeFile();
...
it('provides a runtime test app key without committing key material', function (): void {
    ...
    expect(config('app.key'))
        ->toStartWith('base64:')
        ->and($envTesting)
        ->not->toContain('APP_KEY=base64:')->and($phpunit)
        ->not->toContain('APP_KEY')->and($phpunit)->toContain('bootstrap="tests/bootstrap.php"');
});
...
it('documents the apps/e2e internal support boundary', function (): void {
    ...
    expect($normalizedReadme)
        ...
        ->and(repo_path('packages/e2e-support'))
        ->not->toBeDirectory();
});
