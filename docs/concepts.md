...
- **Orbit launcher** — host `orbit` executable that launches the role-appropriate Orbit artifact and passes `ORBIT_HOST_CWD` when local path context is needed. See [Node Concepts](domains/1_node/node-concepts.md).
...
- **Host cwd context** — launcher-supplied `ORBIT_HOST_CWD` value used to preserve local app/workspace context for the dispatched role artifact (`apps/gateway/artisan` or `apps/cli/orbit`). See [Workspace Concepts](domains/6_workspace/workspace-concepts.md).
