...
/**
 * Make sure the local Docker daemon has the images the Orbit runtime
 * contract depends on: `orbit-runtime:current` (built from the Orbit
 ...
 * orbit-runtime / orbit-caddy container is created on a real node. The
 ...
 * matches the Docker-first runtime contract.
 *
 ...
 */
#[Signature('orbit:internal:build-runtime-images
    {--force : Rebuild orbit-runtime and re-pull the Caddy image even when both already exist locally}')]
...
class BuildRuntimeImagesCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        if (! $this->ensureOrbitRuntimeImage($force)) {
            return self::FAILURE;
        }
    ...
    private function ensureOrbitRuntimeImage(bool $force): bool
    {
