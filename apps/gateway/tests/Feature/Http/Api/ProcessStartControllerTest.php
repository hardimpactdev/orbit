...
describe('ProcessStartController', function (): void {
    ...
    it('authorizes app-hostname start against the selected node when process names collide', function (): void {
        $caller = createProcessStartCallerNode();
        $grantedNode = createTestAppHostNode(['name' => 'granted-node']);
        $otherNode = createTestAppHostNode(['name' => 'other-node']);
        grantProcessStartAccess($caller, $grantedNode);
        ...
        // Intentionally create the other-node process first so unscoped name lookup
        // would prefer the wrong node without app-hostname grant resolution.
        ...
        $response
            ...
            ->assertJsonPath('success.data.runtimes.0.project', 'docs')
            ->assertJsonPath('success.data.runtimes.0.event.type', 'started');
    });

    it('authorizes workspace-hostname start against the selected node when process names collide', function (): void {
        $caller = createProcessStartCallerNode();
        $grantedNode = createTestAppHostNode(['name' => 'workspace-node']);
        $otherNode = createTestAppHostNode(['name' => 'other-node']);
        grantProcessStartAccess($caller, $grantedNode);
        ...
        $response
            ->assertOk()
            ->assertJsonPath('success.data.runtimes.0.node', 'workspace-node')
    ...
    it('starts a node owned process for node context', function (): void {
        ...
        $response
            ...
            ->assertJsonPath('success.data.runtimes.0.project', null)
