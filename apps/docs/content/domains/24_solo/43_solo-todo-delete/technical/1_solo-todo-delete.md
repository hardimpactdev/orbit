...
- The caller has `solo:todo:delete` on the serving gateway node.

## Signature

```bash
orbit solo:todo:delete <todo> [--project=<project>] [--title=<title>] [--body=<body>] [--force] [--json]
```
...
The command calls `DELETE /api/solo/todo/delete` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
