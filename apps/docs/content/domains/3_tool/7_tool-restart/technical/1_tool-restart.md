# Technical Contract: `orbit tool:restart <tool> [--app=<app>] [--node=<node>] [--node-transport=<transport>] [--json|--stream-json]`
...
```bash
orbit tool:restart <tool> [--app=<app>] [--node=<node>] [--node-transport=<transport>] [--json|--stream-json]
```
...
- Dispatches the selected tool definition's explicit restart script.
...
- `tool:restart` does not imply support for tool log streaming or reload commands.
