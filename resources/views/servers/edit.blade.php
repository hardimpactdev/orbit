...
<div class="p-6 max-w-2xl">
    ...
    <h2 class="text-2xl font-bold text-gray-800 dark:text-white mb-6">Edit Server</h2>

    <form action="{{ route('servers.update', $server) }}" method="POST" class="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-4">
            ...
            <div class="flex items-center mb-4">
                <input type="checkbox" name="is_local" id="is_local" value="1" {{ old('is_local', $server->is_local) ? 'checked' : '' }}
            ...
            <div id="remote-fields">
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="host" id="host" value="{{ old('host', $server->host) }}"
                               class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                        @error('host')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                ...
                <div class="mt-4">
                    ...
                    <input type="text" name="user" id="user" value="{{ old('user', $server->user) }}"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:text-white">
                    @error('user')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="flex items-center">
                <input type="checkbox" name="is_default" id="is_default" value="1" {{ old('is_default', $server->is_default) ? 'checked' : '' }}
                       class="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500">
