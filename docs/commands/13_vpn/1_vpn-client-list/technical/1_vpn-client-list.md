...
Gateway callers execute the backend read locally. Control callers open the
gateway-local execution path over SSH through Orbit/WireGuard, then run the
backend read on the gateway. App-node and unknown callers are denied before
backend authentication or side effects.
