# Technical Contract: `orbit tool:stop <tool> [--app=<app>] [--node=<node>] [--json|--stream-json]`
...
```bash
orbit tool:stop <tool> [--app=<app>] [--node=<node>] [--json|--stream-json]
```
...
- Requires the selected tool definition to declare `stop`.
...
- A direct remote runtime dispatches through Agent push. A process-backed
  runtime dispatches the native process stop action.
...
- Support for `stop` does not imply support for any other runtime verb.
