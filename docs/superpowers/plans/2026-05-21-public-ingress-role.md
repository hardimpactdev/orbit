...
- [ ] **Step 1: Cover public ingress role placement**

Use the smallest prepared topology that contains operator, gateway, production
app, and ingress roles. Assert that public ingress role state exists on the
selected node and that app-production settings point to the selected ingress
node.

- [ ] **Step 2: Cover production app routing through public ingress**
...
on `app-prod-1`, and assert the rendered public route uses public ingress
placement and the expected backend pool.
