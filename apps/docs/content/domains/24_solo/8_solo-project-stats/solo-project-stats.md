# `orbit solo:project:stats`

Show aggregate stats for one Solo project.

## Usage

```bash
orbit solo:project:stats <project> [--json]
```

## Contract

`solo:project:stats` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `GET /api/solo/project/stats`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
