...
```bash
orbit solo:project:rename <project> [--node=<node>] [--name=<name>] [--json]
```

## Contract

`solo:project:rename` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `PATCH /api/solo/project/rename`; the gateway must also have the Solo extension enabled. Use `--node=<node>` to target that node's configured node-local Solo API; when omitted, Orbit uses local `node:default`, then the caller node.
