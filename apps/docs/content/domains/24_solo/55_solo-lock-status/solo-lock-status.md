# `orbit solo:lock:status`

Show Solo lock status.

## Usage

```bash
orbit solo:lock:status [--json]
```

## Contract

`solo:lock:status` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `GET /api/solo/lock/status`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
