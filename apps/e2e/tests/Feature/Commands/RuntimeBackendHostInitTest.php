...
it('verifies the VM runtime backend is managed by host init', function (): void {
    ...
    try {
        $runtimeBackend = $topology->ssh('gateway', 'command -v systemctl >/dev/null 2>&1 && systemctl --version', timeoutSeconds: 60);

        expect($runtimeBackend->successful())->toBeTrue()
            ->and($runtimeBackend->output())->toContain('systemd');
    } finally {
        $topology->cleanup();
    }
