...
| `run` | `argument` | `Required.` | `Never.` | `None.` | Positive integer deployment run id owned by the selected app instance. |
...
- Resolves the run id against deployment history for the selected app instance.
...
| Run not found | No visible deployment run matches the run id for the selected app instance. | `error.code=deploy.run_not_found` |
