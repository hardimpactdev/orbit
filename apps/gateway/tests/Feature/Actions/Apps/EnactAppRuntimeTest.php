...
it('skips the FrankenPHP runtime container for static apps and serves the proxy route via file_server only', function (): void {
    ...
    foreach ($shell->scripts as $script) {
        expect($script)
            ...
            ->not->toContain('docker network inspect')->and($script)
            ->not->toContain('docker container inspect');
    }
...
it(
    'returns app.php_version_unavailable when the image is pruned out from under a matching container (not runtime_container_*)',
    function (): void {
        $app = makeAppOnDevNode(AppRuntimeKind::Php);
