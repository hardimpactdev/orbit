...
- **RemoteLocalExecutor** — execution lane where the gateway SSHs to a node and invokes the installed Orbit CLI binary's internal executor command for packaged node-local helper logic that needs host file access and PHP/PDO (using the binary's embedded PHP). See [Runtime Execution Lanes](execution-lanes.md).
...
- **Host cwd context** — launcher-supplied `ORBIT_HOST_CWD` value used to preserve local app/workspace context for the dispatched Orbit CLI binary. See [Workspace Concepts](domains/6_workspace/workspace-concepts.md).
