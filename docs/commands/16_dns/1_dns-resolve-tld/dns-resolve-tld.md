...
Run `dns:resolve-tld` to write or remove a local resolver entry for a development TLD. For the resolve path, `dns:resolve-tld`:
...
Use these commands alongside `dns:resolve-tld` for related DNS and provisioning tasks.
