# `orbit solo:scratchpad:write`

Replace Solo scratchpad content with optional revision guard.

## Usage

```bash
orbit solo:scratchpad:write <scratchpad> [--content=<content>] [--heading=<heading>] [--search=<search>] [--replace=<replace>] [--name=<name>] [--expected-revision=<expected-revision>] [--force] [--json]
```

## Contract

`solo:scratchpad:write` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `PUT /api/solo/scratchpad/write`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
