...
final class WorkspaceExecCommand extends WorkspaceGatewayCommand
{
    ...
    public function handle(): int
    {
        ...
        $workspace = $this->stringArgument('workspace');
        $hostCwd = $this->hostCwd();

        if ($workspace === null && $hostCwd === null) {
            return $this->failValidation('workspace', 'Workspace is required.');
        }

        try {
            $response = $workspace === null
                ? $this->gatewayPost('/api/workspaces/exec/by-path', [
                    'command' => $command,
                    'host_cwd' => $hostCwd,
                ])
                : $this->gatewayPost($this->pathWithQuery(
                    '/api/workspaces/'.rawurlencode($workspace).'/exec',
