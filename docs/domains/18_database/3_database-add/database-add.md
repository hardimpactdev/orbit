...
`sqlite` requires both `--path` and `--node` for the node that owns the file.
`mysql` and `pgsql` use network fields and may omit `--node` when the
connection is not node-bound.
