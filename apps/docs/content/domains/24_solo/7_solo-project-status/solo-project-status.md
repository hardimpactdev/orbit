# `orbit solo:project:status`

Show runtime status for one Solo project.

## Usage

```bash
orbit solo:project:status <project> [--json]
```

## Contract

`solo:project:status` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `GET /api/solo/project/status`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
