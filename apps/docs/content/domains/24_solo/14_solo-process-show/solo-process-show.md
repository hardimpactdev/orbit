# `orbit solo:process:show`

Show one Solo process.

## Usage

```bash
orbit solo:process:show <process> [--json]
```

## Contract

`solo:process:show` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `GET /api/solo/process/show`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
