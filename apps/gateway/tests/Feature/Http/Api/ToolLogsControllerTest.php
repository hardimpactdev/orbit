...
describe('ToolLogsController target resolution', function (): void {
    ...
    it('reads logs from the selected process for a multi-instance Docker service tool', function (): void {
        $caller = createToolLogsControllerCaller();
        $node = createTestAppHostNode([
            'name' => 'visible-process-logs',
            'status' => 'active',
        ]);
        ...
        $shell = new ToolLogsControllerRecordingShell(stdout: "selected redis\n");
        app()->instance(RemoteShell::class, $shell);

        $ambiguous = call(
            'GET',
            '/api/tools/redis/logs',
            ['node' => 'visible-process-logs'],
            [],
            [],
            ['REMOTE_ADDR' => TOOL_LOGS_CONTROLLER_CALLER_WG_IP],
        );
        ...
        $selected = call(
            'GET',
            '/api/tools/redis/logs',
            ['node' => 'visible-process-logs', 'instance' => 'redis:8', 'lines' => 3],
            [],
            [],
            ['REMOTE_ADDR' => TOOL_LOGS_CONTROLLER_CALLER_WG_IP],
        );
        ...
        $selected->assertOk()
            ->assertJsonPath('success.data.logs.instance', 'redis:8')
            ->assertJsonPath('success.data.logs.process', 'redis8')
            ->assertJsonPath('success.data.logs.lines.0.message', 'selected redis');

        expect($shell->scripts)->toBe(["docker logs --tail 3 'redis8' 2>&1"]);
    });
