...
Enable WebSocket support for one concrete app instance.

`app:websocket enable` creates or updates the WebSocket binding for one selected
app-instance site and syncs its public host routes and Reverb configuration. The
...
```bash
orbit app:websocket enable <app.instance> [--host=<hostname>] [--json]
```

## Examples

```bash
orbit app:websocket enable docs.production
orbit app:websocket enable docs.production --host=ws.docs.example.com
orbit app:websocket enable docs.production --host=ws.docs.example.com --host=events.docs.example.com
orbit app:websocket enable docs.production --json
```
...
5. Syncs the Reverb runtime app configuration so the running Reverb node
   accepts connections for this app-instance site immediately.
...
- An active router node and at least one active WebSocket backend node exist in
  the fleet.
- The selected app instance exists in the gateway registry and has a serving
  node and domain.
