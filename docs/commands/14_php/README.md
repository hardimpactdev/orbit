...
selection intent that is verified by existing state-family doctors.
...
- App PHP version is gateway-tracked app intent.
- Workspace PHP version is gateway-tracked workspace intent. A workspace
...
- App-node callers may use `php:*` only as gateway clients. Local cwd context
  can resolve the target app or workspace, but it never grants write authority
  by itself.
