...
it('shows activity details and validates ids from an operator caller', function (): void {
    ...
    try {
        ...
        expect($jsonResult->successful())
            ->toBeTrue($jsonResult->output().$jsonResult->errorOutput())
            ->and($activity)
            ->toBeArray()
            ->and($activity['id'])
            ->toBe($id)
            ...
            ->and($activity['effect'])
            ->toBe('write')
            ->and($jsonPayload['success']['data']['related'])
            ->toBeArray()
            ->and($jsonPayload['success']['meta']['related_count'])
            ->toBe(0);
        ...
        expect($missingResult->successful())
            ->toBeFalse()
            ->and($missingPayload['error']['code'])
            ->toBe('validation_failed')
            ->and($missingPayload['error']['meta']['field'])
            ->toBe('id')
            ->and($missingPayload['error']['meta']['reason'])
            ->toBe('missing');
        ...
        expect($invalidResult->successful())
            ->toBeFalse()
            ->and($invalidPayload['error']['code'])
            ->toBe('validation_failed')
            ->and($invalidPayload['error']['meta']['reason'])
            ->toBe('invalid');
    } finally {
        $topology->cleanup();
    }
