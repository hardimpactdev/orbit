 ...
 */
function grantAppInstanceApiAccess(
    Node $caller,
    Node $serving,
    array $permissions = ['instance:read', 'instance:write'],
): void {
...
describe('AppInstanceController', function (): void {
    it('adds lists shows and removes app instances', function (): void {
        ...
        Project::factory()->for($node, 'node')->create([
            'name' => 'billing',
            'path' => '/home/orbit/apps/billing',
            'document_root' => 'public',
        ]);

        $created = appInstanceApiJson('POST', '/api/projects/billing/instances', [
        ...
        $created
            ->assertOk()
            ->assertJsonPath('success.data.instance.project', 'billing')
        ...
        $list = appInstanceApiJson('GET', '/api/projects/billing/instances');
        ...
        $show = appInstanceApiJson('GET', '/api/projects/billing/instances/production-cloud');

        $show->assertOk()
            ->assertJsonPath('success.data.instance.driver_config.environment_name', 'production');

        $withoutForce = appInstanceApiJson('DELETE', '/api/projects/billing/instances/production-cloud');
        ...
        $removed = appInstanceApiJson('DELETE', '/api/projects/billing/instances/production-cloud', [
    ...
    it('reuses the default Laravel Cloud environment when the environment is omitted', function (): void {
        ...
        $node = Node::factory()->appDev()->create(['name' => 'app-dev-1']);
        grantAppInstanceApiAccess($caller, $node);
        Project::factory()->for($node, 'node')->create(['name' => 'billing']);
    ...
    it('stores orbit driver config with node placement and rejects invalid driver config', function (): void {
        ...
        $app = Project::factory()->for($devNode, 'node')->create(['name' => 'billing']);
        ...
        $invalid = appInstanceApiJson('POST', '/api/projects/billing/instances', [
            'name' => 'broken-cloud',
            'driver' => 'laravel-cloud',
            'cloud_application_id' => 'app_123',
        ]);
