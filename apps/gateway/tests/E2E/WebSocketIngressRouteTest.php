...
it('delivers public websocket events through ingress and router to websocket-role backends', function (): void {
    ...
    try {
        ...
        expect($delivery)->toMatchArray([
            'published' => true,
            'event' => 'orbit.e2e',
            'channel' => $eventChannel,
            'message' => $eventMessage,
        ])
            ->and($delivery['handshake_status'])->toContain('101')
            ->and($delivery['publish_status'])->toContain('200');
    } finally {
