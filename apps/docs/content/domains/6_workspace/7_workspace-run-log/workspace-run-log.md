...
The `workspace:run:log` command retrieves the captured execution output for one
...
use [`orbit process:log`](../../7_process/8_process-log/process-log.md).

`workspace:run:log` pairs with
[`orbit workspace:history`](../6_workspace-history/workspace-history.md):
`workspace:history` lists runs, `workspace:run:log` shows the captured detail of
one run.
...
[See the technical contract for detailed behavior and failure semantics.](technical/1_workspace-run-log.md)
