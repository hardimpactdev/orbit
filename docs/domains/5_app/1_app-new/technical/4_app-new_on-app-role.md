# Technical Contract: `app:new` On A Node Carrying An App Role

[Back to technical contract](1_app-new.md)

This page describes the gateway-side authorization decision when `orbit app:new`
is invoked from a peer the gateway identifies as a node carrying an app role.

`app:new` is **rejected** by the gateway when the authenticated WireGuard peer
is a node carrying an app role. The CLI does not detect this locally; it forwards the request,
...
- **Reason:** `app:new` manages app configuration and node application, which
  are gateway-owned responsibilities. App-role identities are not authorized to
...
- **Human message:** "Command 'app:new' cannot be initiated from a node carrying an app role."
...
| Path | Coverage |
| --- | --- |
| `tests/Feature/Commands/Apps/AppNewOnAppNodeRejectionTest.php` | App-role caller rejection for `app:new` (see breakdown below). |
...
node carrying an app role. It asserts `error.code=caller_role_not_allowed` in JSON output and
