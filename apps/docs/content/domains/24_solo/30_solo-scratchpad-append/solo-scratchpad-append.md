# `orbit solo:scratchpad:append`

Append content to a Solo scratchpad with optional revision guard.

## Usage

```bash
orbit solo:scratchpad:append <scratchpad> [--content=<content>] [--heading=<heading>] [--search=<search>] [--replace=<replace>] [--name=<name>] [--expected-revision=<expected-revision>] [--force] [--json]
```

## Contract

`solo:scratchpad:append` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `POST /api/solo/scratchpad/append`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
