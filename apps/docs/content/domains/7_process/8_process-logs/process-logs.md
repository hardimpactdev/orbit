...
`process:logs` reads process logs from the resolved node or app instance serving node process manager through
the gateway for a resolved node, app-instance, or workspace context. On agent-capable
...
```bash
orbit process:logs vite --app=docs.production
orbit process:logs vite --app=docs.development --workspace=feature-docs --follow
orbit process:logs opencode-server --node=app-dev-1 --lines=200
orbit process:logs queue --app=docs.production --lines=200 --json
```
...
- **Context Resolution**: Resolves the process and node/app-instance/workspace runtime context. Prefer `<app.instance>`; a bare app slug is accepted only when that logical app has exactly one instance.
- **Placement**: App-instance and workspace logs are read from the instance's serving node.
