...
it('returns deterministic core adapter descriptors without reserved tokens', function (): void {
    ...
    expect($registry->adapters())
        ...
        ->and($registry->isRegisteredAdapter('opencode'))
        ->toBeTrue()
        ->and($registry->isRegisteredAdapter('none'))
        ->toBeFalse()
        ->and($registry->isRegisteredAdapter('inherit'))
        ->toBeFalse();
});
...
it('returns command scoped choices with reserved tokens first', function (): void {
    ...
    expect($registry->choicesForScope('node'))
        ->toBe([
