...
it('follows a durable update all operation from an operator through gateway event replay', function (): void {
    ...
    try {
        ...
        $requestLog = e2eRunInRoleRuntime(
            $topology,
            'gateway',
...
it('uses topology candidate cli artifacts for the local update all fan-out', function (): void {
    ...
    try {
        ...
        expect($payload['event'])
            ->toBe('complete')
            ->and($payload['data']['exit_code'])
            ->toBe(0)
            ->and($payload['data']['data']['status'])
            ->toBe('succeeded')
            ->and($payload['data']['data']['target_version'])
            ->toBe($version)
            ->and($payload['data']['data']['manifest_source'])
            ->toBe('topology-candidate')
            ->and(trim($installedOutput))
            ->toBe("orbit {$version}")
            ->and($requests)
            ->toContain([
                'method' => 'POST',
