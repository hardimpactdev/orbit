...
- The caller has `solo:process:spawn` on the serving gateway node.
...
The command calls `POST /api/solo/process/spawn` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
