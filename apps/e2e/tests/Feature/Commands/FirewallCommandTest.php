...
it('writes lists and removes firewall intent on a prepared app node', function (): void {
    ...
    try {
        ...
        $allow = $topology->ssh(
            'gateway',
            "cd {$checkout} && orbit firewall:allow "
            .escapeshellarg($rule)
            .' --node=app-dev-1 --port=49281 --from=10.6.0.0/24 --reason=e2e --json',
            timeoutSeconds: 120,
        );
        $allowPayload = firewallCommandPayload($allow->output());

        expect($allow->successful())
            ->toBeTrue()
            ->and($allowPayload['success']['data']['rule'])
            ->toMatchArray([
                'name' => $rule,
                'node' => 'app-dev-1',
                'action' => 'allow',
            ...
            ->and($allowPayload['success']['meta']['backend_enacted'])
            ->toBeFalse()
            ->and($allowPayload['success']['meta']['warnings'][0]['code'])
            ->toBe('firewall_rule.enactment_deferred');

        $deny = $topology->ssh(
            'gateway',
            "cd {$checkout} && orbit firewall:deny "
            .escapeshellarg($denyRule)
            .' --node=app-dev-1 --port=49282 --from=10.6.0.0/24 --reason=e2e --json',
            timeoutSeconds: 120,
        );
        $denyPayload = firewallCommandPayload($deny->output());

        expect($deny->successful())
            ->toBeTrue()
            ->and($denyPayload['success']['data']['rule'])
            ->toMatchArray([
                'name' => $denyRule,
                'node' => 'app-dev-1',
                'action' => 'deny',
            ...
            ->and($denyPayload['success']['meta']['backend_enacted'])
            ->toBeFalse()
            ->and($denyPayload['success']['meta']['warnings'][0]['code'])
            ->toBe('firewall_rule.enactment_deferred');

        $list = $topology->ssh(
            'gateway',
            "cd {$checkout} && orbit firewall:list --node=app-dev-1 --json",
            timeoutSeconds: 120,
        );
        $rules = firewallCommandPayload($list->output())['success']['data']['rules'];

        expect(array_column($rules, 'name'))->toContain($rule)->and(array_column($rules, 'name'))->toContain($denyRule);

        $missingConsent = $topology->ssh(
            'gateway',
            "cd {$checkout} && orbit firewall:remove ".escapeshellarg($rule).' --node=app-dev-1 --json || true',
            timeoutSeconds: 120,
        );
        ...
        $protectedRemove = $topology->ssh(
            'gateway',
            "cd {$checkout} && orbit firewall:remove orbit-public-ssh-deny-v4 --node=app-dev-1 --force --json || true",
            timeoutSeconds: 120,
        );
        ...
        $remove = $topology->ssh(
            'gateway',
            "cd {$checkout} && orbit firewall:remove ".escapeshellarg($rule).' --node=app-dev-1 --force --json',
            timeoutSeconds: 120,
        );
        ...
        expect($remove->successful())
            ->toBeTrue()
            ->and($removePayload['success']['data']['rule'])
            ->toMatchArray([
                'name' => $rule,
                'node' => 'app-dev-1',
                'status' => 'removed_with_drift',
            ])
            ->and($removePayload['success']['meta']['backend_removed'])
            ->toBeFalse()
            ->and($removePayload['success']['meta']['warnings'][0]['code'])
            ->toBe('firewall_rule.cleanup_deferred');
        ...
        $topology->ssh(
            'gateway',
            "cd {$checkout} && orbit firewall:remove "
            .escapeshellarg($rule)
        ...
        $topology->ssh(
            ...
            "cd {$checkout} && php apps/gateway/artisan tinker --execute="
            .escapeshellarg(
                '\App\Models\FirewallRule::query()->where("name", "orbit-public-ssh-deny-v4")->delete(); echo "cleaned";',
            )
            .' >/dev/null 2>&1 || true',
            timeoutSeconds: 120,
        );
...
function firewallCommandPrepareAppNode(E2ETopologyHarness $topology, string $checkout): void
{
    $topology->ssh(
        ...
        "cd {$checkout} && php apps/gateway/artisan tinker --execute="
            .escapeshellarg(
                '$node = \App\Models\Node::query()->where("name", "app-dev-1")->firstOrFail(); $node->update(["platform" => "ubuntu", "status" => "active"]); echo "prepared";',
            ),
        timeoutSeconds: 120,
    );
}
