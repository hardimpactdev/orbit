...
it('shows a database connection from the operator node through the gateway api', function (): void {
    $config = E2EConfig::fromEnvironment();
    $topology = e2eTopology(E2ETopologyKind::OperatorGatewayAppdev, withGatewayApi: true);

    $slug = 'e2e-db-show-'.strtolower(bin2hex(random_bytes(3)));

    try {
        $topology->withCurrentCheckout(roles: ['operator', 'gateway']);
        $gatewayApiIp = $topology->lease()->gatewayApiIp();

        e2eRestartGatewayApi($topology, 'database-show');
        ...
        $result = $topology->ssh(
            'operator',
            sprintf(
                'cd %s && orbit database:show %s --json',
        ...
        expect($result->successful())
            ...
            ->toBe($slug)
            ->and($payload['success']['data']['connection']['driver'])
            ->toBe('sqlite')
            ->and($payload['success']['data']['connection']['node'])
            ->toBe('app-dev-1');
    } finally {
        $slugValue = var_export($slug, true);
