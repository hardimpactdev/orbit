...
The gateway is the source of truth for process configuration. When node-side work is required, the gateway renders and applies derived runtime units on the resolved node or the selected instance's serving node.
...
- Runtime units are the process-family product noun: concrete runnable units
  derived from node/instance/workspace scope and process configuration. They are not
  gateway state rows.
...
- Runtime unit names use an Orbit-owned identity that is safe for backends and has five parts:
...
connects the caller to the resolved node or instance serving node. The CLI does not detect or
...
- Runtime-lifecycle commands (`process:start`, `process:stop`,
  ...
  calls from the instance serving node are authorized by its
...
- Configuration mutation commands (`process:add`, `process:update`,
  ...
  self-grant: an app-dev node can create, update, and remove instance-owned
