...
describe('app setup commands', function (): void {
    it('streams app:setup through the app setup endpoint', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ...
            ->and($output)->not->toContain('Running setup');
    });
    ...
    it('posts app-setup-step:add payloads to the gateway', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['step']['command'])->toBe('composer install');
    });
    ...
    it('lists app setup steps in json mode', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['steps'][0]['command'])->toBe('composer install');
    });
    ...
    it('validates app setup command inputs before gateway IO', function (string $command, array $params, string $field): void {
        Http::fake();
        ...
        'add bad timeout' => ['app-setup-step:add', ['app' => 'docs', '--command' => 'composer install', '--timeout' => '0'], 'timeout'],
