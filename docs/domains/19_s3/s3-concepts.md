...
- **S3 role:** Private workload role that runs one RustFS backend in a Docker
  runtime container rendered by Orbit, binds only to WireGuard, and receives
  traffic through router-owned S3 service routes.
- **RustFS backend:** The S3-compatible RustFS runtime behind the `s3` role.
  Router backend pools target concrete private RustFS backend URLs such as
  `http://storage-1.s3.orbit:9000`.
- **S3 service endpoint:** Stable private HTTPS endpoint `https://s3.orbit`.
  ...
  concrete S3 nodes.
- **S3 backend pool:** Router-owned ordered list of RustFS backend URLs behind
...
- **S3 route publication:** Gateway-owned route convergence that records one
  public S3 host on the selected `rustfs` tool row, creates or updates an S3
...
- **S3 service credentials:** RustFS access key and secret stored for the
  service on the `rustfs` tool row and returned by `s3:credentials` and
  `tool:credentials rustfs`.
- **S3 role data path:** Absolute host path stored on the `s3` role assignment,
  defaulting to `/srv/orbit/s3/data`, mounted into RustFS as `/data`.
...
- **S3-domain boundaries:** S3 commands coordinate public host publication,
  ...
  rendering. Node role settings remain node-owned, RustFS runtime lifecycle
...
- **S3-domain exclusions:** S3 commands do not create buckets, manage bucket
  policy, create per-app credentials, rotate credentials, expose the RustFS
  console, configure wildcard DNS or TLS, build distributed RustFS, or manage
