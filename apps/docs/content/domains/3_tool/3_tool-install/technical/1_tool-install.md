...
| Field | Value |
| --- | --- |
| Type | `api:POST /tools/{tool}/install` |
| Effect | `write` |
...
| `apps/gateway/tests/Unit/Services/Tools/ToolInstallerPostWriteFailureIntentTest.php` | Post-write install failures preserve the expected-intent `NodeTool` row for new and pre-existing rows after both transport-style and script nonzero-exit failures, and later reconciliation observes identical `expected_version`/`expected_state`. |
