...
| `php:use` | `php:write` | explicit CLI target node, selected workspace serving node, or every affected Orbit instance serving node for an app policy write | Project policy writes preauthorize every affected Orbit instance and verify each node's image availability before mutation | `authorization_failed` | Standard missing-permission meta plus denied `instance` and serving node when instance-scoped |
