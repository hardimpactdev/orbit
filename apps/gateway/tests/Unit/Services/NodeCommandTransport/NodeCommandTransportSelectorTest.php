...
it('selects agent-push for fleet-update commands on roleless unmanaged nodes', function (string $command): void {
    ...
    'install-cli' => ['internal:fleet-update:install-cli'],
    'verify' => ['internal:fleet-update:verify'],
]);

it('does not widen agent-push for an unknown fleet-update prefix on roleless unmanaged nodes', function (): void {
    $node = activeAgentUnavailableNode();
    $selector = new NodeCommandTransportSelector;
    $envelope = NodeCommandEnvelope::agentPushBinary(
        operationId: 'op-1',
        binary: 'orbit',
        argv: ['internal:fleet-update:future'],
    );
