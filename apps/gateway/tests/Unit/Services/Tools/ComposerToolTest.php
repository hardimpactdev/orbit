...
describe('ComposerTool', function (): void {
    it('has the correct slug and category', function (): void {
        ...
        expect($tool->slug())->toBe('composer')->and($tool->category())->toBe('runtime');
    });
    ...
    it('installScript uses the static default php binary, not a versioned one', function (): void {
        ...
        expect($script)
            ->toContain('php ')
            ->and($script)
            ->not->toContain('php8.5')->and($script)
    ...
    it('installScript installs the phar to /usr/local/bin with filename composer', function (): void {
        ...
        expect($script)->toContain('--install-dir=/usr/local/bin')->and($script)->toContain('--filename=composer');
    });
    ...
    it('probeMetadata identifies the Orbit-managed composer binary', function (): void {
        ...
        expect($metadata['binary'])
            ->toBe('/usr/local/bin/composer')
            ->and($metadata['version_command'])
            ->toBe('/usr/local/bin/composer --version')
            ->and($metadata['update_command'])
            ->toBe('sudo /usr/local/bin/composer self-update 2>/dev/null');
    });
