...
- **Orbit Scheduler:** Resident `orbit-scheduler` Artisan-command daemon supervised by the process manager (Supervisor) on every gateway and app node.
  - Owns schedule evaluation, due-run dispatch, and overlap policy for schedules whose target resolves to the local node.
  - Records run history and emits a heartbeat to the gateway.
