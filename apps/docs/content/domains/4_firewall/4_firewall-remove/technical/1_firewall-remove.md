# Technical Contract: `orbit firewall:remove <name> [--node=<node>] [--node-transport=<transport>] [--force] [--json]`
...
```bash
orbit firewall:remove <name> [--node=<node>] [--node-transport=<transport>] [--force] [--json]
```
