# Technical Contract: `orbit solo:process:start`

[Back to public `solo:process:start` documentation.](../solo-process-start.md)
...
```bash
orbit solo:process:start <process> [--force] [--json]
```
...
The command calls `POST /api/solo/process/start` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
