...
| `version` | `--tool-version` | Optional. | `Never.` | Tool-defined latest supported version when applicable. | Specific version or installer channel supported by the selected tool definition. For `claude-code`, `latest` and `stable` are floating installer channels. |
| `config.install_users` | `--user` (repeatable) | Optional for `claude-code` only. | For tools other than `claude-code`. | `None.` | Additional existing Linux usernames for `claude-code` installs. Each value must match a conservative Linux username allow-list; Orbit does not create the account. |
...
For `claude-code`, the CLI and gateway reject `config.install_users` for all
other tools. The node's stored default user is always included. Empty values
fail before row writes. `latest` and `stable` version targets are stored as the
requested installer channel, while live probes report the concrete Claude Code
binary version returned by `claude --version`.
...
- For `claude-code`, treats the tool as a normal installable `runtime` tool with
  `requiredNodeRole() === null`. Eligibility is explicit target authorization,
  active non-gateway node selection, and supported operating system metadata.
  ...
  shell actions, and installs through Anthropic's native installer as each
  existing target user with `sudo -u <user> -H bash -lc`. Tool doctor later
  probes the persisted `default_user` from the `claude-code` row, not a shared
  launcher. When `expected_version` is `latest` or `stable`, doctor treats it as
  a floating installer channel instead of a literal version prefix.
...
| Unsupported install users field | API input includes `config.install_users` for a tool other than `claude-code`. | `error.code=validation_failed`; `error.meta.field=config.install_users`; `error.meta.reason=unsupported_field` |
...
| Unsupported user option | `--user` is supplied for a tool other than `claude-code`. | `error.code=validation_failed`; `error.meta.field=user`; `error.meta.reason=unsupported_field` |
...
| Path | Coverage |
| --- | --- |
| `apps/cli/tests/Feature/Commands/Tool/ToolWriteCommandTest.php` | CLI install write flow: prompts, payloads, default node resolution, `--tool-version`, `claude-code` `--user` forwarding, unsupported `--user` for other tools, empty `--user` gateway rejection, unsupported `--status`, and gateway install error envelope pass-through. |
...
| `apps/gateway/tests/Feature/Http/Api/ToolInstallControllerTest.php` | Gateway/API install: row writes, `claude-code` install-user config persistence and validation, rejected runtime/instance fields, process configuration and skip/idempotence, authorization failure, unsupported status, unsupported action, and update-only version intent rejection. |
