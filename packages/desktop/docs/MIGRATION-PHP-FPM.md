...
### Phase 1: CLI Infrastructure (orbit-cli)
...
; Generated by orbit-cli - do not edit manually

[orbit-LAUNCHPAD_PHP_VERSION]
user = LAUNCHPAD_USER
...
<dict>
    <key>Label</key>
    <string>com.orbit.horizon</string>
...
class PhpManager
{
    ...
    public function getSocketPath(string $version): string
    {
        ...
        return $this->homePath() . "/.config/orbit/php/php{$normalized}.sock";
    }
...
This normalization is used for socket naming (`php84.sock`) and pool names (`[orbit-84]`).
...
```php
// Before
"reverse_proxy orbit-php-{$version}:8080"
...
mysite.ccc {
    root * /home/orbit/projects/mysite/public
...
**Linux (systemd):** `/etc/systemd/system/orbit-horizon.service`
**macOS (launchd):** `~/Library/LaunchAgents/com.orbit.horizon.plist`
...
```json
{
    ...
    "php": {
        ...
        "socket_path": "~/.config/orbit/php"
    },
    "caddy": {
        "config_path": "~/.config/orbit/caddy/Caddyfile",
        "data_path": "~/.config/orbit/caddy/data"
    }
}
```
...
### Phase 2: Desktop App Updates (orbit-desktop)
...
**File:** `~/.config/orbit/php/php{version}-fpm.conf`
...
listen = /home/orbit/.config/orbit/php/php84.sock
listen.owner = orbit
listen.group = orbit
listen.mode = 0660
...
; Logging
php_admin_value[error_log] = /home/orbit/.config/orbit/logs/php84-fpm.log
php_admin_flag[log_errors] = on
...
**File:** `/etc/systemd/system/orbit-horizon.service`
...
User=orbit
Group=orbit
WorkingDirectory=/home/orbit/.config/orbit/web
ExecStart=/usr/bin/php artisan horizon
Restart=always
RestartSec=5
Environment="PATH=/home/orbit/.local/bin:/home/orbit/.bun/bin:/usr/local/bin:/usr/bin:/bin"
...
**File:** `~/Library/LaunchAgents/com.orbit.horizon.plist`
...
<dict>
    ...
    <key>WorkingDirectory</key>
    <string>/Users/orbit/.config/orbit/web</string>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
    <true/>
...
{
    return $this->docker->containerExists('orbit-php-84');
}
```
...
- [x] `orbit init` completes successfully on fresh Ubuntu 24.04
- [x] `orbit init` completes successfully on fresh macOS (with Homebrew)
...
### Web App .env File (`~/.config/orbit/web/.env`)
...
The `orbit restart` command currently stops Caddy and Horizon but doesn't restart them (known bug). After running restart, manually start them:

```bash
# Linux
sudo systemctl start caddy orbit-horizon

# macOS
brew services start caddy
launchctl load ~/Library/LaunchAgents/com.orbit.horizon.plist
```
