...
- [~] `doctor` — verify-mode dispatcher + family probes. Currently
  `DoctorReportRunner::SUPPORTED_FAMILIES` dispatches `node`, `proxy`,
  `firewall_rule`, `tool`, `schedule`. Generic `--fix` / `--adopt`
  orchestration reaches family dispatch and records unsupported actions as
  skipped.
  - [ ] Verify-mode dispatch for `app`, `workspace`, and `process` —
    probes exist but are not yet wired into `DoctorReportRunner` /
    `DoctorScopeValidator`.
