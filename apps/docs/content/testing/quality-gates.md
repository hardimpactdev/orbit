...
`apps/docs/`, outside tests, outside repository tooling under `bin/`, and
outside repository-only SDK package surfaces under `packages/sdk/` and
`packages/sdk-typescript/`, the final packet also needs
`Retained topology proof: passed`. Repository tooling under `bin/` and SDK
package automation still require diff-routed `composer quality-check` but have
no retained topology target. Shared core under `packages/core/src/`, CLI, and
node runtime stay on retained topology. PHP under `apps/docs/` is docs tooling
and does not need retained topology proof unless the slice also changes
topology behavior. Derive the venue early with
