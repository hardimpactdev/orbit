...
- database connection targets owned by the gateway: the app instance or workspace
  mapping and the supported environment prefix used in that target's `.env`;
- supported env-prefix expansions in app-instance and workspace `.env` files;
...
selected app-instance/workspace `.env` files and checks these layers:

1. **Target mapping presence:** every selected database connection record has
   the app-instance or workspace target mapping needed for the requested scope.
2. **Target env visibility:** the selected app instance or workspace has a readable
...
4. **Expected env shape:** observed env values for a mapped prefix match the
   ...
   For app-instance and workspace targets on the same node as a managed Docker MySQL
   ...
   comparison use the renamed Docker service alias for same-node app-instance/workspace
...
| `database_connection.target_missing` | A gateway database connection record that should be materialized for the selected app instance or workspace has no matching target mapping. |
| `database_connection.target_extra` | During an explicit adoption scope, a selected app instance or workspace has an adopted-or-mappable database target that does not yet exist in gateway target mappings. |
...
| --- | --- |
| `database_connection.env_missing` | Write the stored gateway connection values for the mapped prefix into the selected app-instance or workspace `.env`. |
| `database_connection.env_mismatch` | Rewrite the mapped prefix in the selected app-instance or workspace `.env` so the supported keys match the stored gateway connection values. |
| `database_connection.target_missing` | Recreate the missing target mapping when the owning app instance or workspace and selected prefix are already unambiguous from gateway state. |
...
| --- | --- |
| `database_connection.env_extra` | Parse the observed supported env-prefix group, create or update a gateway database connection record, and create or update the matching target mapping for the selected app instance or workspace. |
...
- Create or update `database_connection_targets` for the selected app instance
  or workspace and observed prefix.
- Support first-rollout adoption for existing registered app instances and workspaces
...
| `apps/gateway/tests/Unit/Services/DatabaseConnections/DatabaseConnectionAdopterTest.php` | Supported-prefix adoption, existing app-instance/workspace rollout adoption, connection/target upsert behavior, and encrypted credential storage for passwords. |
