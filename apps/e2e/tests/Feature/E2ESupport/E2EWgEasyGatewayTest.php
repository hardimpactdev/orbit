...
it('reuses the baked wg-easy container when it is already healthy and recreates it otherwise', function (): void {
    ...
    expect($reuseGate)->toBeInt()
        ->and($reuseAttempt)->toBeInt()
        ->and($recreateFallback)->toBeInt()
        ->and($reuseGate)->toBeLessThan($recreateFallback)
        ->and($reuseAttempt)->toBeLessThan($recreateFallback)
        ->and($script)
