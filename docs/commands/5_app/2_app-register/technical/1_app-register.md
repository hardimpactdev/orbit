...
| `--node` | `text` | No local context or default. | Local default / Existing owner | Valid app node name. |
| `--root` | `text` | Optional | `public` | Path relative to app path. |
| `--php-version` | `text` | Optional | Existing app value; otherwise `8.5` | Supported app PHP-FPM version. This is app runtime intent, not the owning node's CLI PHP default. |
...
| --- | --- | --- |
| `control` | `valid` | Full registration and enactment through the gateway. |
| `gateway` | `valid` | Local path resolution skips the proxy hop for enactment. |
| `app` | `invalid` | App-node callers are denied before prompts or side effects with `error.code=caller_role_not_allowed`. |
...
4. **Resolve PHP version**:
   ...
   - Do not read or inherit the owning node's CLI PHP default; node CLI PHP and
     app PHP-FPM intent are separate blueprint concepts.
...
`app:register` converges gateway app intent and node artifacts:
...
- **Idempotence (Re-enactment Refresh)**: `app:register` always re-applies
  ...
  verifies command-owned enactment; if nothing changes, the command still
  ...
  - `converged` — idempotent re-enactment of an already-managed app where no
