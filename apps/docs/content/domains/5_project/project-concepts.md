...
This document defines project and instance vocabulary and invariants. It supports the
project and instance command contracts and the [instance doctor](instance-doctor.md); it does not override
...
- **Project:** Logical application record owned by the gateway, with a stable
  identity slug, shared runtime policy, and optional repository. A project may
  have multiple instances and owns no placement defaults.
- **Instance:** Concrete runtime or deployment target for one project. An
  instance belongs to exactly one project, has a name unique within that project, selects
...
- **Initial instance:** Concrete instance created together with a
  project by `project:new`. Orbit names it `development` when `--domain` is absent and
  `production` when `--domain` is present. Instance-owned commands must resolve
  this or another concrete instance. A project selector fails when more
  than one instance could own the operation; it never falls back to project-owned
  placement state.
- **Project identity slug:** Lowercase identity slug used as the project's globally
  unique gateway registry key. Maximum 40 characters.
- **Project name argument:** Positional `[name]` argument used by commands that
  create, adopt, or re-converge project configuration. It is not a hostname selector.
...
These terms describe runtime and deployment environments. The project record
...
- **Project PHP version:** Gateway-tracked configuration for the PHP version used by
...
- **Project runtime kind:** The runtime shape selected for a project. Instances of `php`
  projects run in a dedicated FrankenPHP container; `static` projects serve files directly
...
- **App runtime container:** Dedicated Docker container for one PHP app runtime.
  ...
  Static projects do not have a runtime container. The concrete instance runtime, managed
...
- **Instance runtime mount:** Extra Docker bind mount stored on an instance and
  ...
  the current slice, configurable runtime mounts are accepted only for PHP projects
  ...
  instances of the same project may use different host source paths for the same
  ...
  containers that use that instance. Project-level runtime-mount rows are not a
...
- **FrankenPHP app runtime:** PHP app/workspace web runtime. Classic mode is
  ...
  represented as a process with Docker runtime. The project family owns shared
...
- **Instance analytics binding:** Gateway-owned binding for one concrete instance
  ...
  generate credentials, or inject scripts into the project. Project owners add the
...
- **Reverb app credentials:** Reverb application id, key, and secret material
  ...
  shared across projects; rotating or disabling one binding must not invalidate
...
- **Project dependency audit posture:** Gateway-owned compact summary of a read-only
  ...
  slice.
- **Dependency audit manager:** Supported package-manager audit lane for one project
  ...
  source path; the project exposes only the aggregate summary.
- **Dependency audit status:** Per-manager or aggregate posture state. `clean`
  ...
  exists yet.
- **Dependency audit severity bands:** Headline counts exposed on project list/show
...
The terms below describe project and instance lifecycle.

- **Instance registration:** Idempotent convergence of project configuration and node artifacts
  performed by `instance:register`. Used to install Orbit management on a new path,
  re-apply management to an existing project, or retry production domain activation.
...
- **Instance setup pipeline:** Ordered instance-owned commands recorded with
  ...
  migrations, seeders, and project-owned user creation.
- **Instance setup run:** Gateway record of one `instance:setup` execution owned by one
  ...
  affecting another instance of the same project.

## Boundaries

These boundaries define what the project and instance families own and what belongs to other families.
...
- **Project and instance boundaries:** Project commands own the project registry; instance commands own the
  ...
  Project and instance commands do not install or own host Caddy, Reverb, or the host PHP
