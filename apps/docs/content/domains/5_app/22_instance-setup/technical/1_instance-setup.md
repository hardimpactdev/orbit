# Technical Contract: `orbit instance:setup`

[Back to public `instance:setup` documentation.](../instance-setup.md)
...
- The target instance exists in gateway configuration and resolves an Orbit
  serving node.
...
```bash
orbit instance:setup [instance] [--json|--stream-json]
```
...
| --- | --- | --- | --- | --- | --- |
| `instance` | `[instance]` | Always. | Never. | None. | Dotted instance selector. A bare name or hostname auto-resolves only a sole instance; otherwise fail with `validation_failed` and `meta.reason=instance_required`. |
...
5. Creates an instance-owned setup run when execution is needed.
...
7. Dispatches each routed setup command through typed `internal:instance-setup-step` over agent-push on agent-capable nodes. Setup environment values travel only in the token-bound stdin payload, not in transport metadata or activity summaries.
...
- [Human renderer](6.1_instance-setup_output-render_human.md)
- [JSON renderer](6.2_instance-setup_output-render_json.md)
