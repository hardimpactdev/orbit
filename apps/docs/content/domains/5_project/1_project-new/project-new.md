...
```bash
orbit project:new [name]
orbit project:new [name] --node=app-1
orbit project:new [name] --repo=hardimpactdev/orbit --php-version=8.5
orbit project:new [name] --template-repo=hardimpactdev/laravel-template --new-repo=hardimpactdev/my-app
orbit project:new [name] --repo=hardimpactdev/orbit --domain=example.com --json
orbit project:new [name] --repo=hardimpactdev/orbit --domain=example.com --stream-json
```

## Description

`project:new` creates a new application on a target node from an explicit source
plan. After creating the project source, it atomically writes the project and
...
- **Source Resolution:** Before it starts creating anything, interactive
  `project:new` resolves the target node, project slug, and whether to create a new
...
- **Credentials:** Repository creation and cloning run non-interactively on the
  ...
  embedded credentials, query strings, or fragments are rejected. `project:new`
...
- **Registry Write:** Atomically writes authoritative logical-project identity and
  ...
  domain, environment, and `adopted=false` are instance-owned. Project names are
  identity slugs and must be globally unique in the gateway project registry.
...
- **Production Activation:** When `--domain` is provided, production placement
  ...
  through [`instance:register <project.instance> --domain=<host>`](../2_instance-register/instance-register.md),
  which is safe to call repeatedly.
- **Retry Safety:** If source creation fails, no project configuration is written; fix the
  node-side source problem and rerun `project:new`. After gateway configuration is written,
  registration failures preserve both records. Subsequent runs of
  `instance:register <project.instance>` or `doctor --family=instance --instance=<project.instance>
  --restore` will attempt to complete
  artifact convergence. If a previous `project:new` run already cloned the requested
  repository but failed before writing project configuration, rerunning `project:new`
...
- [`orbit instance:register <project.instance>`](../2_instance-register/instance-register.md)
- [`orbit project:list`](../3_project-list/project-list.md)
- [`orbit project:remove [project]`](../6_project-remove/project-remove.md)
- [Technical Contract](technical/1_project-new.md)
