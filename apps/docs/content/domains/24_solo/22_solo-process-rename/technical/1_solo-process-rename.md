# Technical Contract: `orbit solo:process:rename`

[Back to public `solo:process:rename` documentation.](../solo-process-rename.md)
...
The command calls `PATCH /api/solo/process/rename` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
