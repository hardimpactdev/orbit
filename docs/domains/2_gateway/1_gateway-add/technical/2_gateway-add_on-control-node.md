# Technical Contract: `gateway:add` On A Operator Node
...
- The operator node has an active gateway-issued WireGuard identity.
- The operator node has joined the active Orbit WireGuard network.
- The target gateway is reachable over the Orbit network.
- The local platform is supported for operator nodes (macOS or Ubuntu).

Evaluate each path eligibility rule as soon as the fields needed for that rule
are known. For example, a operator caller with no active WireGuard identity
...
mirror of the gateway or current operator node. It stores local gateway endpoint
...
- Fails when local config cannot be written despite successful gateway
  verification.
- Fails when the local platform is not supported for operator nodes.
...
| `tests/E2E/GatewayAddTest.php` | Real-node operator node join flow via `gateway:add`, including omitted-argument gateway IP derivation and idempotent convergence. |
