...
- If start enactment fails after the lifecycle intent changes, Orbit keeps the
...
`tool-start` changes gateway lifecycle intent and performs command-owned enactment only. [`tool-doctor.md`](../../tool-doctor.md) owns the authoritative tool-family probe, issue codes, fix map, and adopt map.
