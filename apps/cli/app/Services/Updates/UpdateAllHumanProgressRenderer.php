...
final class UpdateAllHumanProgressRenderer
{
    ...
    private const string STAGE_SKIPPED_AGENT = 'skipped_agent';
    ...
    private function applyKeyedStep(
        ...
        if (str_starts_with($key, 'workload.')) {
            ...
            if ($status === 'done') {
                ...
                if ($message !== null && str_contains($message, 'skipped: Orbit Agent is not running')) {
                    $this->setRow($output, $node, self::STATE_SKIPPED, self::STAGE_SKIPPED_AGENT);
    ...
    private function stageName(string $stage): string
    {
        return match ($stage) {
            ...
            self::STAGE_SKIPPED_AGENT => 'Skipped: Orbit Agent is not running',
