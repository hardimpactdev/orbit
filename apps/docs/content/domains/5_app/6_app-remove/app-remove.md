...
`app:remove` accepts an app slug only. It never accepts a hostname,
dotted instance selector, or parent-only removal mode. To remove one concrete
placement while keeping the app, use
...
Use this command when you want to remove an app and every instance it
...
After consent, Orbit removes the app, every instance, and all
...
- [`instance:remove`](../28_instance-remove/instance-remove.md) - remove one placement
  while retaining the app and its other instances.
- [`app:list`](../3_app-list/app-list.md) - list apps.
