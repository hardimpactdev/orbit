# `orbit solo:scratchpad:find`

Search Solo scratchpads.

## Usage

```bash
orbit solo:scratchpad:find <query> [--json]
```

## Contract

`solo:scratchpad:find` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `GET /api/solo/scratchpad/find`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
