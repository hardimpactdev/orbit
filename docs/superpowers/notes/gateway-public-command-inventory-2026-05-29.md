...
- Invokable command rows covered: 13 total, including 1 gateway-owned runtime row (orbit-scheduler), 11 internal `App\Console\Commands\Internal\*` rows kept explicit for the allowed `orbit:internal:*` category, and `orbit:internal:node-register`. E2E runner commands (e2e:*) were extracted to apps/e2e in #9H and are no longer registered in gateway Artisan.
