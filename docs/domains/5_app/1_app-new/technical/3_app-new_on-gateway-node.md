...
- **Allowed:** The gateway identifies the caller through its WireGuard peer
  ...
  authorizes the request just like a operator caller.
...
| Path | Coverage |
| --- | --- |
| `tests/Feature/Commands/Apps/AppNewOnGatewayNodeContractTest.php` | Gateway-peer authorization of `app:new`: gateway accepts the request, writes app configuration, applies app-node artifacts via `RemoteShell`, and produces the same observable result as the operator-caller path. |
