...
- **Orbit launcher** — host `orbit` executable that always enters `apps/cli/orbit` and passes `ORBIT_HOST_CWD` when local path context is needed. Gateway maintenance uses `bin/orbit-gateway-artisan` or direct `php apps/gateway/artisan` from a controlled gateway shell. See [Node Concepts](domains/1_node/node-concepts.md).
- **Orbit runtime container** — one `orbit-runtime` container per node; it is the gateway runtime execution target and, on the gateway, the API and scheduler runtime. See [Node Concepts](domains/1_node/node-concepts.md).
...
- **RemoteLocalExecutor** — execution lane where the gateway SSHs to a node and invokes the host-installed `apps/cli` internal executor command for packaged node-local helper logic that needs host file access and PHP/PDO. See [Runtime Execution Lanes](execution-lanes.md).
...
- **Host cwd context** — launcher-supplied `ORBIT_HOST_CWD` value used to preserve local app/workspace context for the dispatched CLI artifact at `apps/cli/orbit`. See [Workspace Concepts](domains/6_workspace/workspace-concepts.md).
