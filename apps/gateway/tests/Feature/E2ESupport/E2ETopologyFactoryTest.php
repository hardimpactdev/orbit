...
it('has correct enum string values', function (): void {
    expect(E2ETopologyKind::Operator->value)
        ...
        ->and(E2ETopologyKind::tryFromInput('operator_gateway_app-prod_ingress')?->value)
        ->toBe('operator_gateway_app-prod_ingress')
        ->and(E2ETopologyKind::tryFromInput('operator_gateway_app-dev_websocket')?->value)
        ->toBe('operator_gateway_app-dev_websocket')
        ->and(E2ETopologyKind::tryFromInput('operator_gateway_app-dev_app-prod_websocket')?->value)
        ->toBe('operator_gateway_app-dev_app-prod_websocket')
        ->and(E2ETopologyKind::tryFromInput('operator_gateway_app-dev_app-prod_agent_websocket')?->value)
        ->toBe('operator_gateway_app-dev_app-prod_agent_websocket')
        ->and(E2ETopologyKind::Operator)
        ...
        ->and(E2ETopologyKind::tryFromInput('operator-gateway-appdev-websocket'))
        ->toBe(E2ETopologyKind::OperatorGatewayAppdevWebsocket)
        ->and(E2ETopologyKind::tryFromInput('operator-gateway-dev-prod-websocket'))
        ->toBe(E2ETopologyKind::OperatorGatewayAppdevAppprodWebsocket)
        ->and(E2ETopologyKind::tryFromInput('operator-gateway-appdev-appprod-websocket'))
        ->toBe(E2ETopologyKind::OperatorGatewayAppdevAppprodWebsocket)
        ->and(E2ETopologyKind::tryFromInput('operator-gateway-appdev-appprod-agent-websocket'))
...
it('resolves requested topology kinds exactly', function (): void {
    withE2ETopologyEnvironment([], function (): void {
        ...
        expect($factory->resolveKind(E2ETopologyKind::OperatorGateway))
            ->toBe(E2ETopologyKind::OperatorGateway)
            ->and($factory->resolveKind(E2ETopologyKind::Operator))
            ->toBe(E2ETopologyKind::Operator)
            ->and($factory->resolveKind(E2ETopologyKind::OperatorGatewayAppdev))
            ->toBe(E2ETopologyKind::OperatorGatewayAppdev)
            ->and($factory->resolveKind(E2ETopologyKind::OperatorGatewayAppdevAppprod))
            ->toBe(E2ETopologyKind::OperatorGatewayAppdevAppprod)
            ->and($factory->resolveKind(E2ETopologyKind::OperatorGatewayAppdevAppprodIngress))
            ->toBe(E2ETopologyKind::OperatorGatewayAppdevAppprodIngress)
            ->and($factory->resolveKind(E2ETopologyKind::OperatorGatewayAgent))
            ->toBe(E2ETopologyKind::OperatorGatewayAgent)
            ->and($factory->resolveKind(E2ETopologyKind::OperatorGatewayAppdevWebsocket))
            ->toBe(E2ETopologyKind::OperatorGatewayAppdevWebsocket)
            ->and($factory->resolveKind(E2ETopologyKind::OperatorGatewayAppdevAppprodWebsocket))
            ->toBe(E2ETopologyKind::OperatorGatewayAppdevAppprodWebsocket)
            ->and($factory->resolveKind(E2ETopologyKind::OperatorGatewayAppdevAppprodAgentWebsocket))
            ->toBe(E2ETopologyKind::OperatorGatewayAppdevAppprodAgentWebsocket)
            ->and($factory->resolveKind($ingressKind))
            ->toBe($ingressKind);
    });
});
    ...
    withE2ETopologyEnvironment([
        ...
        expect(fn () => $factory->require(E2ETopologyKind::Operator))
            ->toThrow(
                ...
                'incus: prepared topology operator is not available on any Incus host: orbit-e2e-nonexistent.invalid is missing prepared templates or snapshots',
            );
    });
});
...
it('stores requested SSH users immutably', function (): void {
    ...
    expect($operatorOnly)
        ...
        ->toBe($factory)
        ->and(
            new ReflectionClass($operatorOnly)
                ->getProperty('sshUsers')
                ->getValue($operatorOnly),
        )
...
it('stores source mounted checkout acquisition options immutably', function (): void {
    ...
    expect($sourceDevFactory)
        ->not
        ->toBe($factory)
        ->and(
            new ReflectionClass($sourceDevFactory)
                ->getProperty('sourceMountedCheckout')
                ->getValue($sourceDevFactory),
        )
        ->toBeTrue()
        ->and(
            new ReflectionClass($factory)
        ...
        ->toBeFalse()
        ->and($options->sshUsers)
        ->toBe(['operator' => 'operator'])
        ->and($options->startGatewayApi)
        ->toBeTrue()
        ->and($options->sourceMountedCheckout)
        ->toBeTrue();
});
