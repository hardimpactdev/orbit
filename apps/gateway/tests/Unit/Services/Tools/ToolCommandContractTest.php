...
describe('tool command shared contract', function (): void {
    ...
    it('keeps observed state out of the registry model because tool:list does not probe live state', function (): void {
        ...
        expect($tool->getFillable())
            ...
            ->and($tool->getFillable())
            ->not->toContain('observed_state', 'instance_key', 'version_family', 'runtime', 'runtime_config');
    });
    ...
    it('filters registry lists to visible app hosts by node selector and app selector', function (): void {
        ...
        expect(
            $registry
                ->list()
                ->map(fn (NodeTool $tool): string => "{$tool->node?->name}:{$tool->name}")
                ->all(),
        )
            ->toBe([
    ...
    it('returns contract failures for invalid or conflicting registry filters', function (): void {
        ...
        expect($invalidNode)
            ...
            ->and($invalidNode->code)
            ->toBe('validation_failed')
            ->and($invalidNode->meta)
            ->toMatchArray(['field' => 'node', 'value' => 'missing-node'])
            ->and($unassignedNodeFilter)
            ->toBeNull()
            ->and($invalidApp)
            ->toBeInstanceOf(ToolRegistryFailure::class)
            ->and($invalidApp->code)
    ...
    it('resolves shared tool targets by app slug domain combined selector and matching node rules', function (): void {
        ...
        expect($slugResult)
            ...
            ->and($slugResult->node?->name)
            ->toBe($firstNode->name)
            ->and($domainResult)
            ->toBeInstanceOf(NodeTool::class)
            ->and($domainResult->node?->name)
            ->toBe($firstNode->name)
            ->and($combinedResult)
            ->toBeInstanceOf(NodeTool::class)
            ->and($combinedResult->node?->name)
            ->toBe($firstNode->name)
            ->and($matchingResult)
            ->toBeInstanceOf(NodeTool::class)
            ->and($matchingResult->node?->name)
            ->toBe($firstNode->name);
    });
    ...
    it('does not resolve service process versions as tool instances', function (): void {
        ...
        expect($registry->show('php', node: $node->name))
            ->toBeInstanceOf(NodeTool::class)
            ->and($registry->show('php')->code)
            ->toBe('validation_failed')
            ->and($withInstance)
            ->toBeInstanceOf(ToolRegistryFailure::class)
            ->and($withInstance->code)
            ->toBe('tool.not_found');
    });
    ...
    it('exposes the shared tool failure shape and allowed remote action metadata', function (): void {
        ...
        expect(array_keys(get_object_vars($failure)))
            ->toBe([
                'code',
                'message',
                'meta',
            ...
            ->and($failure->message)
            ->toBe("Tool 'caddy' start failed on node 'app-contract-a'.")
            ->and($failure->meta)
            ->toBe([
                'tool' => 'caddy',
                'node' => 'app-contract-a',
