...
- The caller has `solo:process:input` on the serving gateway node.
...
The command calls `POST /api/solo/process/input` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
