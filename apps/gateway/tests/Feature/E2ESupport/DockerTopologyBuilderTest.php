...
it('defines the Docker topology host PHP 8.5 CLI baseline without ad hoc helper CLIs', function (): void {
    ...
    expect(preg_match('/(?:^|\s)python3(?:\s|\\\\|$)/m', $dockerfile))
        ->toBe(0)
        ->and(preg_match('/(?:^|\s)sqlite3(?:\s|\\\\|$)/m', $dockerfile))
        ->toBe(0);
});
...
it('starts Docker build topology client nodes with the host Docker socket and no gateway sibling', function (): void {
    ...
    Process::fake(function ($process) use (&$commands) {
        ...
        if (
            ...
            || str_contains($process->command, 'id_ed25519.pub')
        ) {
    ...
    expect($setup)
        ...
        ->not->toContain('$(stat')->toContain("--volume '/var/run/docker.sock:/var/run/docker.sock'")->toContain(
            "--mount 'type=volume,src=orbit-e2e-prepared-build-operator-operator-etc-caddy,dst=/etc/caddy'",
        )->toContain(
            "--mount 'type=volume,src=orbit-e2e-prepared-build-operator-operator-etc-orbit,dst=/etc/orbit'",
        )->toContain(
            "--mount 'type=volume,src=orbit-e2e-prepared-build-operator-operator-opt-orbit,dst=/opt/orbit'",
        )->toContain(
            "--mount 'type=volume,src=orbit-e2e-prepared-build-operator-operator-var-lib-orbit,dst=/var/lib/orbit'",
        )->toContain(
            "--mount 'type=volume,src=orbit-e2e-prepared-build-operator-operator-run-php,dst=/run/php'",
        )->toContain("--env 'ORBIT_E2E_DOCKER_NETWORK=orbit-e2e-prepared-build-operator'")->toContain(
            "--env 'ORBIT_NODE_CONTAINER=orbit-e2e-prepared-build-operator-operator'",
        )->toContain("--env 'ORBIT_CONFIG_ROOT=/home/orbit/.config/orbit'")->toContain('ip addr add')->toContain(
            '10.6.0.3/24',
        )->toContain("--mount 'type=volume,src=orbit-e2e-composer-cache-")->toContain(
            'dst=/tmp/orbit-composer-cache',
        )->toContain("docker run -d --name 'orbit-e2e-prepared-build-operator-operator-composer'")->toContain(
            "'composer:2' tail -f /dev/null",
        )->toContain("--env 'ORBIT_SOURCE_PATH=/home/orbit/orbit'")->toContain(
            "--env 'COMPOSER_CACHE_DIR=/tmp/orbit-composer-cache'",
        )->toContain("--env 'COMPOSER_HOME=/tmp/orbit-composer-home'")->toContain(
            "--env 'COMPOSER_ALLOW_SUPERUSER=1'",
        )->toContain("cd '\\''/home/orbit/orbit/apps/gateway'\\''")->toContain('/home/orbit/orbit/apps/cli')->toContain(
            'mkdir -p',
        )->toContain('/tmp/orbit-composer-home/config.json')->toContain('cache-dir')->toContain(
            'github-protocols',
        )->toContain("git config --global --add safe.directory '\\''*'\\''")->toContain(
            '/tmp/orbit-composer-cache',
        ...
        ->not->toContain(
            "docker run -d --restart unless-stopped --name 'orbit-e2e-prepared-build-operator-operator-orbit-gateway'",
        );
...
it('uses the remote docker build host socket gid when DOCKER_HOST is ssh://beast', function (): void {
    ...
    try {
        Process::fake(function ($process) use (&$commands) {
            ...
            if (
                ($gidFake = dockerSocketGroupIdProcessFake($command, localGroupId: 999, remoteGroupId: 1000)) !== null
            ) {
                return $gidFake;
            }
        ...
        $dockerRun = collect($commands)->first(
            fn (string $command): bool => (
                str_starts_with($command, 'docker run -d ') && str_contains($command, '--cap-add NET_ADMIN')
            ),
        );
        ...
        expect($dockerRun)
            ->toBeString()
            ->and($dockerRun)
            ->toContain('--group-add 1000')
            ->and($dockerRun)
            ->not->toContain('--group-add 999')->and($dockerRun)
            ->not->toContain('$(stat');
...
it('can bind a build-host Composer cache into Docker topology containers', function (): void {
    ...
    withE2EEnvironment(
        ['ORBIT_E2E_DOCKER_COMPOSER_CACHE', 'ORBIT_E2E_DOCKER_COMPOSER_CACHE_READ_ONLY'],
        [
...
it('uses the same lockfile keyed Composer cache volume across topology builds', function (): void {
    ...
    expect($mounts)->toHaveCount(1)->and($mounts[0])->toStartWith('orbit-e2e-composer-cache-');
});
...
it('retries docker build network allocation outside the orbit WireGuard subnet when a pool overlaps', function (): void {
    ...
    expect($networkCreates)
        ...
        ->toContain('/24')
        ->and($networkCreates[0])
        ->not->toContain('10.6.')->and($networkCreates[1])
        ->not->toBe($networkCreates[0]);
...
it('syncs the current checkout into each Docker topology node before installing role dependencies', function (): void {
    ...
    expect($setup)
        ...
        ->toContain('-T ')
        ->toContain(
            "docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-operator' tar --warning=no-unknown-keyword -xzf - -C '/home/orbit/orbit' < '",
        )
        ->toContain(
            "docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-gateway' tar --warning=no-unknown-keyword -xzf - -C '/home/orbit/orbit' < '",
        )
    ...
    $operatorSync = array_search(
        collect($commands)->first(
            fn (string $command): bool => (
                str_contains(
                    $command,
                    "docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-operator'",
                )
                && str_contains($command, "tar --warning=no-unknown-keyword -xzf - -C '/home/orbit/orbit'")
                && str_contains($command, 'orbit-current-')
            ),
        ),
        $commands,
        strict: true,
    );
    $operatorInstall = array_search(
        collect($commands)->first(
            fn (string $command): bool => (
                str_contains($command, "docker exec --env 'ORBIT_SOURCE_PATH=/home/orbit/orbit'")
                && str_contains(
                    $command,
                    "'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-operator-composer'",
                )
    ...
    $gatewaySync = array_search(
        collect($commands)->first(
            fn (string $command): bool => (
                str_contains(
                    $command,
                    "docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-gateway'",
                )
                && str_contains($command, "tar --warning=no-unknown-keyword -xzf - -C '/home/orbit/orbit'")
                && str_contains($command, 'orbit-current-')
            ),
    ...
    $gatewayReuse = array_search(
        collect($commands)->first(
            fn (string $command): bool => (
                str_contains(
                    $command,
                    "docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-operator-composer'",
                )
                && str_contains($command, "tar -C '/home/orbit/orbit' -cf - apps/gateway/vendor apps/cli/vendor")
                && str_contains(
                    $command,
                    "docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-gateway-orbit-gateway'",
                )
            ),
        ),
        $commands,
    ...
    expect($operatorSync)
        ...
        ->toBeInt()
        ->and($operatorSync)
        ->toBeLessThan($operatorInstall)
        ->and($gatewaySync)
        ->toBeInt()
        ->and($gatewayReuse)
        ->toBeInt()
        ->and($gatewaySync)
        ->toBeLessThan($gatewayReuse);
});
...
it('installs monorepo dependencies once and reuses them across Docker topology roles', function (): void {
    ...
    expect($composerInstallCommands)
        ->toHaveCount(1)
        ->and($cliInstallCommands)
        ->toHaveCount(1)
        ->and($cliInstallCommands[0])
        ->toContain('/home/orbit/orbit/apps/cli')
        ->and($gatewayInstallCommands)
        ->toHaveCount(1)
        ->and($gatewayInstallCommands[0])
        ->toContain('/home/orbit/orbit/apps/gateway')
        ->and($reuseCommands)
        ->toHaveCount(4)
        ->and($reuseCommands[0])
        ->toContain("docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-operator-composer'")
        ->and($reuseCommands[0])
        ->toContain(
            "docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-gateway-orbit-gateway'",
        )
        ...
        ->toContain("docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-operator-composer'")
        ->and($reuseCommands[1])
        ->toContain("docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-dev-composer'")
        ->and($reuseCommands[2])
        ->toContain("docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-operator-composer'")
        ->and($reuseCommands[2])
        ->toContain("docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-prod-composer'")
        ->and($reuseCommands[3])
        ->toContain("docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-operator-composer'")
        ->and($reuseCommands[3])
        ->toContain("docker exec -i 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent-agent-composer'");
...
it('builds Docker checkout sync archives without gitignored local secrets', function (): void {
    ...
    try {
        ...
        expect($exitCode)
            ...
            ->and($entries)
            ->not->toContain('apps/gateway/storage/app/orbit/t384-topology-secret.key')->and($entries)
            ->not->toContain('./apps/gateway/storage/app/orbit/t384-topology-secret.key');
    } finally {
        Process::preventStrayProcesses();
...
it('fails clearly when the orbit gateway sibling image is missing during docker topology preparation', function (): void {
    ...
    expect(
        fn () => new DockerTopologyBuilder(E2EConfig::fromEnvironment())
...
it('builds Docker topology state through the host orbit launcher', function (): void {
    ...
    $setup = implode("\n", $commands);
    $operatorComposer = strpos(
        $setup,
        "docker exec --env 'ORBIT_SOURCE_PATH=/home/orbit/orbit' --env 'COMPOSER_CACHE_DIR=/tmp/orbit-composer-cache' --env 'COMPOSER_HOME=/tmp/orbit-composer-home' --env 'COMPOSER_PROCESS_TIMEOUT=1200' --env 'COMPOSER_ALLOW_SUPERUSER=1' --workdir '/home/orbit/orbit' 'orbit-e2e-prepared-build-operator_gateway-operator-composer'",
    );
    $operatorMigrate = strpos(
        $setup,
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-operator' sh -lc 'cd /home/orbit/orbit && ORBIT_CONFIG_ROOT='\\''/home/orbit/.config/orbit'\\'' DB_CONNECTION=sqlite DB_DATABASE='\\''/home/orbit/.config/orbit/gateway.sqlite'\\'' SESSION_DRIVER=file php apps/gateway/artisan migrate --force --no-interaction --ansi'",
    );
    $gatewayMigrate = strpos(
        $setup,
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc 'cd /home/orbit/orbit && ORBIT_CONFIG_ROOT='\\''/home/orbit/.config/orbit'\\'' DB_CONNECTION=sqlite DB_DATABASE='\\''/home/orbit/.config/orbit/gateway.sqlite'\\'' SESSION_DRIVER=file php apps/gateway/artisan migrate --force --no-interaction --ansi'",
    );
    $gatewayRefresh = strpos(
        $setup,
        "docker exec 'orbit-e2e-prepared-build-operator_gateway-gateway' tar -C '/home/orbit/orbit' -cf - . | docker exec -i 'orbit-e2e-prepared-build-operator_gateway-gateway-orbit-gateway' tar -C '/home/orbit/orbit' -xf -",
    );
    $gatewayBootstrap = strpos(
        $setup,
        'cd /home/orbit/orbit && php apps/gateway/artisan orbit:internal:bootstrap-gateway-local gateway 10.6.0.2',
    );

    expect($operatorComposer)
        ->toBeInt()
        ->and($operatorMigrate)
        ->toBeInt()
        ->and($gatewayMigrate)
        ...
        ->toBeInt()
        ->and($operatorComposer)
        ->toBeLessThan($operatorMigrate)
        ->and($operatorMigrate)
        ->toBeLessThan($gatewayMigrate)
        ->and($gatewayMigrate)
        ->toBeLessThan($gatewayRefresh)
        ->and($gatewayRefresh)
    ...
    expect($setup)
        ...
        ->toContain(
            "ORBIT_CONFIG_ROOT='\\''/home/orbit/.config/orbit'\\'' DB_CONNECTION=sqlite DB_DATABASE='\\''/home/orbit/.config/orbit/gateway.sqlite'\\'' SESSION_DRIVER=file php apps/gateway/artisan tinker --execute",
        )
        ...
        ->toContain('10.6.0.2')
        ->not->toContain(
            "docker run -d --restart unless-stopped --name 'orbit-e2e-prepared-build-operator_gateway-operator-orbit-gateway'",
        )
...
it('starts the build gateway scheduler before schedule doctor verification', function (): void {
    ...
    $scheduler = strpos(
        $setup,
        "docker exec --detach --workdir '/home/orbit/orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway-orbit-gateway' orbit orbit-scheduler",
    );
...
it('tolerates Docker build gateway schedule doctor self-call authorization failure', function (): void {
    ...
    expect($manifest)->toBe([
        [
            'role' => 'operator',
            'container' => 'orbit-e2e-prepared-build-operator_gateway-operator',
            'image' => 'orbit-e2e:operator_base',
        ],
        [
            'role' => 'gateway',
            'container' => 'orbit-e2e-prepared-build-operator_gateway-gateway',
...
it('keeps the build gateway container marked without starting services before migration', function (): void {
    ...
    $gatewayRuntimeStart = collect($commands)
        ->first(
            fn (string $command): bool => str_contains(
                $command,
                "docker run -d --restart unless-stopped --name 'orbit-e2e-prepared-build-operator_gateway-gateway-orbit-gateway'",
            ),
        );
...
it('normalizes persisted gateway orbit state ownership before committing prepared images', function (): void {
    ...
    $persist = collect($commands)->first(
        fn (string $command): bool => str_contains(
            $command,
            "docker exec -i 'orbit-e2e-prepared-build-operator_gateway-gateway' tar -C '/home/orbit/orbit' -xf -",
        ),
    );
    $ownershipCommands = collect($commands)->filter(
        fn (string $command): bool => (
            str_contains($command, "docker exec 'orbit-e2e-prepared-build-operator_gateway-gateway'")
    ...
    $migration = collect($commands)->first(
        fn (string $command): bool => (
            str_contains(
                $command,
                "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc",
            )
            && str_contains($command, 'php apps/gateway/artisan migrate --force --no-interaction --ansi')
        ),
    );
    $commit = collect($commands)->first(
        fn (string $command): bool => (
            str_contains($command, 'docker commit')
    ...
    expect($persist)
        ->toBeString()
        ->and($ownershipCommands->count())
        ->toBeGreaterThanOrEqual(2)
        ->and($ownershipBeforeMigration)
        ->toBeString()
        ->toContain('install -d -m 775 -o orbit -g orbit')
        ->toContain('chmod -R u+rwX,g+rwX')
        ->and($ownershipBeforeCommit)
        ->toBeString()
        ->toContain('install -d -m 775 -o orbit -g orbit')
        ->toContain('chmod -R u+rwX,g+rwX')
        ->and($migration)
        ...
        ->and($commit)
        ->toBeString()
        ->and(array_search($ownershipBeforeMigration, $commands, strict: true))
        ->toBeLessThan(array_search($migration, $commands, strict: true))
        ->and(array_search($persist, $commands, strict: true))
        ->toBeLessThan(array_search($ownershipBeforeCommit, $commands, strict: true))
        ->and(array_search($ownershipBeforeCommit, $commands, strict: true))
        ->toBeLessThan(array_search($commit, $commands, strict: true));
});
...
it('commits Docker build topology images from node image-layer state instead of mounted home volumes', function (): void {
    ...
    $operatorCommit = strpos(
        $setup,
        "docker commit --change 'CMD [\"/usr/local/bin/orbit-e2e-container\"]' --change 'LABEL org.orbit.e2e.topology-mode=dns-alias' --change 'LABEL org.orbit.e2e.kind=operator_gateway' --change 'LABEL org.orbit.e2e.role=operator'",
    );
    $gatewaySync = strpos(
        $setup,
        "docker exec 'orbit-e2e-prepared-build-operator_gateway-gateway-orbit-gateway' tar -C '/home/orbit/orbit' -cf - . | docker exec -i 'orbit-e2e-prepared-build-operator_gateway-gateway' tar -C '/home/orbit/orbit' -xf -",
    );
    $gatewayCommit = strpos(
        $setup,
        "docker commit --change 'CMD [\"/usr/local/bin/orbit-e2e-container\"]' --change 'LABEL org.orbit.e2e.topology-mode=dns-alias' --change 'LABEL org.orbit.e2e.kind=operator_gateway' --change 'LABEL org.orbit.e2e.role=gateway'",
    );

    expect($setup)
        ->not->toContain(
            "--mount 'type=volume,src=orbit-e2e-prepared-build-operator_gateway-operator-home-operator,dst=/home/operator'",
        )
        ->not->toContain(
            "--mount 'type=volume,src=orbit-e2e-prepared-build-operator_gateway-gateway-home-orbit,dst=/home/orbit'",
        );
    ...
    expect($operatorSync)
        ...
        ->toBeInt()
        ->and($operatorSync)
        ->toBeLessThan($operatorCommit)
        ->and($gatewaySync)
        ->toBeInt()
        ->and($gatewayCommit)
        ->toBeInt()
        ->and($gatewaySync)
        ->toBeLessThan($gatewayCommit);
});
...
it('defines downstream small topology role matrices for current roles', function (): void {
    expect(DockerTopologyBuilder::rolesFor(E2ETopologyKind::OperatorGatewayAppdev))
        ->toBe(['operator', 'gateway', 'dev'])
        ->and(DockerTopologyBuilder::rolesFor(E2ETopologyKind::OperatorGatewayAgent))
        ->toBe(['operator', 'gateway', 'agent'])
        ->and(DockerTopologyBuilder::rolesFor(E2ETopologyKind::OperatorGatewayAppdevAppprod))
        ->toBe(['operator', 'gateway', 'dev', 'prod'])
        ->and(DockerTopologyBuilder::rolesFor(E2ETopologyKind::OperatorGatewayAppdevAppprodAgent))
        ->toBe(['operator', 'gateway', 'dev', 'prod', 'agent'])
        ->and(DockerTopologyBuilder::rolesFor(E2ETopologyKind::OperatorGatewayAppdevAppprodIngress))
        ...
        ->and(DockerTopologyBuilder::rolesFor(E2ETopologyKind::OperatorGatewayAppdevWebsocket))
        ->toBe(['operator', 'gateway', 'dev'])
        ->and(DockerTopologyBuilder::rolesFor(E2ETopologyKind::OperatorGatewayAppdevAppprodWebsocket))
        ->toBe(['operator', 'gateway', 'dev', 'prod'])
...
it('provisions Docker downstream role source images in parallel after the gateway baseline', function (): void {
    ...
    $gatewayBootstrap = collect($commands)
        ->first(
            fn (string $command): bool => str_contains(
                $command,
                'orbit:internal:bootstrap-gateway-local gateway 10.6.0.2',
            ),
        );
    $scriptWrite = collect($commands)
        ->first(
            fn (string $command): bool => (
                str_contains($command, 'cat >') && str_contains($command, 'orbit-e2e-docker-downstream.sh')
            ),
        );
    ...
    expect($gatewayBootstrap)
        ...
        ->and($scriptRun)
        ->toBeString()
        ->and(array_search($gatewayBootstrap, $commands, strict: true))
        ->toBeLessThan(array_search($scriptWrite, $commands, strict: true))
        ->and(array_search($scriptWrite, $commands, strict: true))
        ->toBeLessThan(array_search($scriptRun, $commands, strict: true));
...
it('builds operator_gateway prepared images through transient docker resources', function (): void {
    Process::fake([
        ...
        "docker run -d --cap-add NET_ADMIN --cap-add NET_BIND_SERVICE --group-add * --name 'orbit-e2e-prepared-build-operator_gateway-operator' *" => Process::result(
            output: "operator-id\n",
        ),
        "docker run -d --name 'orbit-e2e-prepared-build-operator_gateway-operator-composer' *" => Process::result(
            output: "composer-id\n",
        ),
        "docker run -d --cap-add NET_ADMIN --cap-add NET_BIND_SERVICE --group-add * --name 'orbit-e2e-prepared-build-operator_gateway-gateway' *" => Process::result(
            output: "gateway-id\n",
        ),
        "docker run -d --restart unless-stopped --name 'orbit-e2e-prepared-build-operator_gateway-gateway-orbit-gateway' *" => Process::result(
            output: "gateway-id\n",
        ),
        ...
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-operator' sh -lc *migrate*" =>
            Process::result(),
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc *migrate*" =>
            Process::result(),
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc *bootstrap-gateway-local*" =>
            Process::result(),
        "docker exec --detach --workdir '/home/orbit/orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway-orbit-gateway' orbit orbit-scheduler" =>
            Process::result(),
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc *orbit doctor*--family=schedule*" =>
            Process::result(),
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc *ssh-keyscan*" =>
            Process::result(),
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc *id_ed25519.pub*" => Process::result(
            output: "ssh-ed25519 AAAATEST orbit-e2e-gateway\n",
        ),
        ...
        "docker exec 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc 'if [ -f /home/orbit/.ssh/id_ed25519 ]; then install -d -m 700 /root/.ssh && cp /home/orbit/.ssh/id_ed25519 /root/.ssh/id_ed25519 && chmod 600 /root/.ssh/id_ed25519 && if [ -f /home/orbit/.ssh/id_ed25519.pub ]; then cp /home/orbit/.ssh/id_ed25519.pub /root/.ssh/id_ed25519.pub; fi; fi'" =>
            Process::result(),
        "docker exec 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc *sudo docker exec*id_ed25519*" =>
            Process::result(),
        "docker exec 'orbit-e2e-prepared-build-operator_gateway-gateway' sh -lc *php -d display_errors=0 -d max_execution_time=0 -S*" =>
            Process::result(),
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-operator' sh -lc *curl*" =>
            Process::result(),
        "docker exec --user 'orbit' 'orbit-e2e-prepared-build-operator_gateway-operator' sh -lc *tinker*" =>
            Process::result(),
        'docker exec *ORBIT_GATEWAY_URL*' => Process::result(),
        "docker commit --change * 'orbit-e2e-prepared-build-operator_gateway-operator' 'orbit-e2e:operator_base'" =>
            Process::result(),
        "docker commit --change * 'orbit-e2e-prepared-build-operator_gateway-gateway' 'orbit-e2e:gateway_base'" =>
            Process::result(),
        "docker rm -f 'orbit-e2e-prepared-build-operator_gateway-operator-composer' 'orbit-e2e-prepared-build-operator_gateway-operator-orbit-caddy' 'orbit-e2e-prepared-build-operator_gateway-operator' >/dev/null 2>&1 || true" =>
            Process::result(),
        "docker rm -f 'orbit-e2e-prepared-build-operator_gateway-gateway-orbit-gateway' 'orbit-e2e-prepared-build-operator_gateway-gateway-orbit-caddy' 'orbit-e2e-prepared-build-operator_gateway-gateway' >/dev/null 2>&1 || true" =>
            Process::result(),
...
it('seeds gateway to app node ssh access for remote shell feature tests', function (): void {
    Process::fake([
        ...
        "docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev-gateway' sh -lc 'if [ -f /home/orbit/.ssh/id_ed25519 ]; then install -d -m 700 /root/.ssh && cp /home/orbit/.ssh/id_ed25519 /root/.ssh/id_ed25519 && chmod 600 /root/.ssh/id_ed25519 && if [ -f /home/orbit/.ssh/id_ed25519.pub ]; then cp /home/orbit/.ssh/id_ed25519.pub /root/.ssh/id_ed25519.pub; fi; fi'" =>
            Process::result(),
        "docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev-gateway' sh -lc *authorized_keys*" =>
            Process::result(),
        "docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev-dev' sh -lc *authorized_keys*" =>
            Process::result(),
    ...
    Process::assertRan(
        fn ($process): bool => (
            ...
            && str_contains(
                $process->command,
                "docker exec 'orbit-e2e-prepared-build-operator_gateway_app-dev-gateway' sh -lc",
            )
            && str_contains($process->command, 'authorized_keys')
            && str_contains($process->command, 'ssh-ed25519 AAAATEST orbit-e2e-gateway')
        ),
...
it('uses the configured instance prefix for transient resources but stable image tags', function (): void {
    Process::fake([
        ...
        "docker run -d --cap-add NET_ADMIN --cap-add NET_BIND_SERVICE --group-add * --name 'ci-foo-prepared-build-operator_gateway-operator' *" => Process::result(
            output: "operator-id\n",
        ),
        "docker run -d --name 'ci-foo-prepared-build-operator_gateway-operator-composer' *" => Process::result(
            output: "composer-id\n",
        ),
        "docker run -d --cap-add NET_ADMIN --cap-add NET_BIND_SERVICE --group-add * --name 'ci-foo-prepared-build-operator_gateway-gateway' *" => Process::result(
            output: "gateway-id\n",
        ),
        "docker run -d --restart unless-stopped --name 'ci-foo-prepared-build-operator_gateway-gateway-orbit-gateway' *" => Process::result(
            output: "runtime-id\n",
        ),
        'COPYFILE_DISABLE=1 tar *' => Process::result(),
        'docker exec -i *tar --warning=no-unknown-keyword*orbit-current-*' => Process::result(),
        "docker exec --user 'orbit' 'ci-foo-prepared-build-operator_gateway-gateway' sh -lc *id_ed25519.pub*" => Process::result(
            output: "ssh-ed25519 AAAATEST orbit-e2e-gateway\n",
        ),
        'docker exec --user *' => Process::result(),
        'docker exec *' => Process::result(),
        "docker commit --change * 'ci-foo-prepared-build-operator_gateway-operator' 'orbit-e2e:operator_base'" =>
            Process::result(),
        "docker commit --change * 'ci-foo-prepared-build-operator_gateway-gateway' 'orbit-e2e:gateway_base'" =>
            Process::result(),
        "docker rm -f 'ci-foo-prepared-build-operator_gateway-operator-composer' 'ci-foo-prepared-build-operator_gateway-operator-orbit-caddy' 'ci-foo-prepared-build-operator_gateway-operator' >/dev/null 2>&1 || true" =>
            Process::result(),
        "docker rm -f 'ci-foo-prepared-build-operator_gateway-gateway-orbit-gateway' 'ci-foo-prepared-build-operator_gateway-gateway-orbit-caddy' 'ci-foo-prepared-build-operator_gateway-gateway' >/dev/null 2>&1 || true" =>
            Process::result(),
        'docker volume rm -f *' => Process::result(),
    ...
    withE2EEnvironment(
        ...
        function (): void {
            ...
            expect($manifest)->toBe([
                [
                    'role' => 'operator',
                    'container' => 'ci-foo-prepared-build-operator_gateway-operator',
                    'image' => 'orbit-e2e:operator_base',
                ],
                [
                    'role' => 'gateway',
                    'container' => 'ci-foo-prepared-build-operator_gateway-gateway',
                    'image' => 'orbit-e2e:gateway_base',
                ],
            ]);
        },
...
it('bakes dns alias topology registry data into stable role images', function (): void {
    Process::fake([
        ...
        "docker image inspect 'orbit-gateway:prepared-current' >/dev/null" => Process::result(),
        "docker image inspect 'composer:2' >/dev/null" => Process::result(),
        "docker network create --subnet * 'orbit-e2e-prepared-build-operator_gateway_app-dev_app-prod_agent'" =>
            Process::result(),
        'docker run -d *' => Process::result(output: "container-id\n"),
        'docker exec *ip addr add*' => Process::result(),
        'docker exec *rm -rf*install -d*' => Process::result(),
    ...
    expect($manifest[0]['image'])
        ->toBe('orbit-e2e:operator_base')
        ->and($manifest[0]['reused'])
        ->toBeTrue()
        ->and($manifest[2]['image'])
        ->toBe('orbit-e2e:app-dev_base')
        ->and($manifest[2])
        ->not->toHaveKey('reused')->and($manifest[3]['image'])->toBe('orbit-e2e:app-prod_base')->and($manifest[3])
        ->not->toHaveKey('reused');
...
it('bakes app production ingress docker topology registry data without dev or agent roles', function (): void {
    Process::fake([
        ...
        "docker image inspect 'orbit-gateway:prepared-current' >/dev/null" => Process::result(),
        "docker image inspect 'composer:2' >/dev/null" => Process::result(),
        "docker network create --subnet * 'orbit-e2e-prepared-build-operator_gateway_app-prod_ingress'" =>
            Process::result(),
        'docker run -d *' => Process::result(output: "container-id\n"),
        'docker exec *ip addr add*' => Process::result(),
        'docker exec *rm -rf*install -d*' => Process::result(),
    ...
    $networkPlan = DockerTopologyNetworkPlan::fromEnvironment(
        'orbit-e2e-prepared-build-operator_gateway_app-prod_ingress',
    );
    $prodIp = $networkPlan->ipForRole('prod');

    expect(array_column($manifest, 'role'))
        ->toBe(['operator', 'gateway', 'prod'])
        ->and($manifest[0]['image'])
        ->toBe('orbit-e2e:operator_base')
        ->and($manifest[0]['reused'])
        ->toBeTrue()
        ->and($manifest[2]['image'])
        ->toBe('orbit-e2e:app-prod_base')
        ->and($manifest[2]['reused'])
        ->toBeTrue();
    ...
    Process::assertNotRan(
        fn ($process): bool => (
            ...
            && str_contains(
                $process->command,
                "--name 'orbit-e2e-prepared-build-operator_gateway_app-prod_ingress-ingress'",
            )
        ),
    );
...
it('bakes prepared app production image with a colocated ingress role', function (): void {
    ...
    withE2ETopologyEnvironment([], function (): void {
        ...
        expect(array_column($manifest, 'role'))
            ...
            ->and($manifest[0]['reused'])
            ->toBeTrue()
            ->and($manifest[1]['image'])
            ->toBe('orbit-e2e:gateway_base')
            ->and($manifest[1]['reused'])
            ->toBeTrue()
            ->and($manifest[2]['image'])
            ->toBe('orbit-e2e:app-dev_base')
            ->and($manifest[2])
            ->not->toHaveKey('reused')->and($manifest[3]['image'])->toBe('orbit-e2e:app-prod_base')->and($manifest[3])
            ->not->toHaveKey('reused')->and($manifest[4]['image'])->toBe('orbit-e2e:agent_base')->and($manifest[4])
            ->not->toHaveKey('reused');
    });
