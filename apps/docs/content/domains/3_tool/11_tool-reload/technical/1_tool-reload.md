# Technical Contract: `orbit tool:reload <tool> [--app=<app>] [--node=<node>] [--json|--stream-json]`
...
```bash
orbit tool:reload <tool> [--app=<app>] [--node=<node>] [--json|--stream-json]
```
...
`tool-reload` must not create apps, workspaces, processes, schedules, proxy
...
| Agent push required | The direct remote runtime cannot be reached through Agent push. | `error.code=node_transport_required` |
