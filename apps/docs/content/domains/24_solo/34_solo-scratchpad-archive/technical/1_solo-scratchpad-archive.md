...
- The caller has `solo:scratchpad:write` on the serving gateway node.

## Signature

```bash
orbit solo:scratchpad:archive <scratchpad> [--content=<content>] [--heading=<heading>] [--search=<search>] [--replace=<replace>] [--name=<name>] [--expected-revision=<expected-revision>] [--force] [--json]
```
...
The command calls `POST /api/solo/scratchpad/archive` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
