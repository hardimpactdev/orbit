...
function createInstanceEnvApiCaller(): Node
{
 ...
 */
function grantInstanceEnvApiAccess(
    Node $caller,
    Node $serving,
...
it('authorizes app instance env against the selected instance node', function (): void {
    ...
    grantInstanceEnvApiAccess($caller, $instanceNode, ['instance:read']);
...
it('sets lists and renders non-secret app instance env values with database attachments', function (): void {
    ...
    expect($render->getContent())
        ...
        ->and($connection->fresh()->targets()->where('instance_id', $instance->id)->exists())
        ->toBeTrue();
});
...
it('applies set env values to the remote app runtime when apply is requested', function (): void {
    ...
    $shell = new InstanceEnvControllerRecordingRemoteShell;
...
it('rejects secret env writes until secret storage is designed', function (): void {
    ...
    $response = appInstanceEnvApiJson(
        method: 'POST',
        uri: '/api/apps/billing/instances/development/env',
        data: [
            'key' => 'API_TOKEN',
            'value' => 'secret',
            'secret' => true,
...
final class InstanceEnvControllerRecordingRemoteShell implements RemoteShell
{
    /**
