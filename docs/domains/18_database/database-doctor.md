...
belong to `workspace`. Database service installation and credentials generated
for managed tools belong to `tool`.
...
5. **Observed env extras:** during an explicit adoption scope, supported
...
6. **Observed target extras:** during an explicit adoption scope, supported
...
7. **Verifiability:** malformed, partial, or unsupported env-prefix groups are
...
`database_connection.target_extra`, or `database_connection.unverifiable`.
