...
class WorkspaceNewGatewayStreamClient
{
    ...
    public function run(string $name, string $app, string $base, ?string $phpVersion, callable $onEvent): int|GatewayApiException
    {
        ...
        try {
            $response = ($this->client ?? new Client)->request('POST', "{$gatewayUrl}/api/workspaces/stream", [
        ...
        if ($body !== '') {
            ...
            if (is_array($error)) {
                return new GatewayApiException(
                    message: is_string($error['message'] ?? null) ? $error['message'] : 'Gateway connection is required to create a workspace.',
