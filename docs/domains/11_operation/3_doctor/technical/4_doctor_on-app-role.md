# Technical Contract: `doctor` On A Node Carrying An App Role

[Back to `doctor` technical contract.](1_doctor.md)

This page describes behavior when `orbit doctor` is invoked from a node with an app role — that is, when the gateway identifies the calling WireGuard peer as an app peer.
...
| `interactive`, `restore`, `adopt` | Denied by the gateway before side effects unless the selected family doctor contract documents a narrow app-role write-mode exception. |
...
- App-role working-directory reality is not enough to authorize an app or workspace scope. The gateway must still authorize the resulting scope.
...
- including app-role self facts when the selected target is the calling peer's node.
...
- run generic fleet-wide probes directly from the node;
...
- repair node, proxy, firewall, tool, schedule, or unrelated app/workspace state merely because the CLI is available on the node.
