# Technical Contract: `orbit solo:scratchpad:clear`

[Back to public `solo:scratchpad:clear` documentation.](../solo-scratchpad-clear.md)
...
```bash
orbit solo:scratchpad:clear <scratchpad> [--node=<node>] [--content=<content>] [--heading=<heading>] [--search=<search>] [--replace=<replace>] [--name=<name>] [--expected-revision=<expected-revision>] [--force] [--json]
```
...
- [Interactive input mode](5.1_solo-scratchpad-clear_input-mode_interactive.md)
- [Non-interactive input mode](5.2_solo-scratchpad-clear_input-mode_non-interactive.md)
...
The command calls `POST /api/solo/scratchpad/clear` through the configured gateway client. The CLI resolves an omitted target from local `node:default`, then falls back to the authenticated caller node. Gateway execution requires gateway Solo extension state and the caller permission listed above on the target node.

### Destructive Consent

After resolving the node and scratchpad, apply the shared [Solo destructive-consent contract](../../README.md#destructive-consent). No gateway request is sent before consent succeeds.
