# Technical Contract: `orbit solo:process:input`

[Back to public `solo:process:input` documentation.](../solo-process-input.md)
...
- The caller has `solo:process:input` on the target node, or on the gateway node when `--node` is omitted.

## Signature

```bash
orbit solo:process:input <process> [--node=<node>] [--node-transport=<transport>] [--input=<input>] [--json]
```
...
The command calls `POST /api/solo/process/input` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above on the target node, or on the gateway node when `--node` is omitted.
