...
- The caller has `solo:process:lifecycle` on the serving gateway node.
...
The command calls `POST /api/solo/process/restart` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
