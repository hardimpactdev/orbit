...
```bash
orbit solo:process:clear-output <process> [--force] [--json]
```
...
The command calls `POST /api/solo/process/clear-output` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
