# `orbit solo:process:output`

Read output from one Solo process.

## Usage

```bash
orbit solo:process:output <process> [--lines=<lines>] [--json]
```

## Contract

`solo:process:output` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `GET /api/solo/process/output`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
