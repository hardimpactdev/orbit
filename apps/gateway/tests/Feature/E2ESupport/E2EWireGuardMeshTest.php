...
it('renders the gateway host config as a peer of wg-easy', function (): void {
    ...
    expect($config)
        ...
        ->and($config)
        ->toContain('PrivateKey = gateway-host-private')
        ->and($config)
        ->toContain('PublicKey = wg-easy-public')
        ->and($config)
        ->toContain('PresharedKey = ')
        ->and($config)
        ->toContain('AllowedIPs = 10.6.0.0/24')
        ->and($config)
        ->toContain('Endpoint = 10.231.0.11:51820')
        ->and($config)
        ->toContain('PersistentKeepalive = 25')
        ->and($config)
        ->not->toContain('ListenPort = 51820')->and($config)
        ->not->toContain('PublicKey = operator-public')->and($config)
        ->not->toContain('PublicKey = dev-public');
});
...
it('renders non-gateway peer configs against the wg-easy server key', function (): void {
    ...
    expect($config)
        ->toContain('Address = 10.6.0.4/24')
        ->and($config)
        ->toContain('PrivateKey = dev-private')
        ->and($config)
        ->toContain('PublicKey = wg-easy-public')
        ->and($config)
        ->toContain('PresharedKey = ')
...
it('returns persistent wg-easy peer records for topology roles', function (): void {
    ...
    expect($peers)
        ...
        ->and($peers[0]['pre_shared_key'])
        ->toBe(base64_encode(hash('sha256', 'orbit-e2e-gateway-host-public', binary: true)))
        ->and($peers[1])
        ->toMatchArray([
            'name' => 'operator',
        ...
        ->and($peers[1]['pre_shared_key'])
        ->not->toBeEmpty()->and($peers[2])->toMatchArray([
            'name' => 'dev',
            'private_key' => 'dev-private',
            'public_key' => 'dev-public',
            'address' => '10.6.0.4',
        ])->and($peers[2]['pre_shared_key'])
        ->not->toBeEmpty();
});
...
it('can include an agent peer with a stable WireGuard address', function (): void {
    ...
    expect($mesh->addressFor('agent'))
        ->toBe('10.6.0.6')
        ->and($mesh->peerConfig('agent'))
        ->toContain('Address = 10.6.0.6/24')
        ->and($peers)
        ->toHaveCount(3)
        ->and($peers[2])
        ->toMatchArray([
...
it('can include an ingress peer with a stable WireGuard address', function (): void {
    ...
    expect($mesh->addressFor('ingress'))
        ->toBe('10.6.0.7')
        ->and($mesh->peerConfig('ingress'))
        ->toContain('Address = 10.6.0.7/24')
        ->and($peers)
        ->toHaveCount(3)
        ->and($peers[2])
        ->toMatchArray([
...
it('can include a websocket peer with a stable WireGuard address', function (): void {
    ...
    expect($mesh->addressFor('websocket'))
        ->toBe('10.6.0.8')
        ->and($mesh->peerConfig('websocket'))
        ->toContain('Address = 10.6.0.8/24')
        ->and($peers)
        ->toHaveCount(3)
        ->and($peers[2])
        ->toMatchArray([
...
it('installs and restarts wg-orbit for a role', function (): void {
    ...
    expect($commands[0])
        ...
        ->and($commands[0])
        ->toContain('command -v wg')
        ->and($commands[0])
        ->toContain('command -v wg-quick')
        ->and($commands[0])
        ->not
        ->toContain('apt-get')
        ->and($commands[0])
        ->toContain('PrivateKey = dev-private')
        ->and($commands[0])
        ->toContain('wg-quick down wg-orbit')
        ->and($commands[0])
        ->toContain('ip link delete dev wg-orbit')
        ->and($commands[0])
        ->toContain('wg-quick up wg-orbit')
        ->and($commands[0])
        ->toContain('systemctl enable wg-quick@wg-orbit');
...
it('verifies a role interface and peer reachability', function (): void {
    ...
    expect($commands[0])
        ->toContain('deadline=$((SECONDS+60))')
        ->and($commands[0])
        ->toContain('while true; do')
        ->and($commands[0])
        ->toContain('ip link show wg-orbit')
        ->and($commands[0])
        ->toContain('wg show wg-orbit')
        ->and($commands[0])
        ->toContain("ping -c 1 -W 2 '10.6.0.3'")
        ->and($commands[0])
        ->toContain("ping -c 1 -W 2 '10.6.0.4'");
});
