...
- The caller has `solo:project:create` on the serving gateway node.
...
The command calls `POST /api/solo/project/create` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
