...
describe('security installers', function (): void {
    it('installs the sysctl baseline through remote shell', function (): void {
        ...
        expect($report->successful)
            ...
            ->and($shell->runs[0]['script'])
            ->toContain('net.ipv4.tcp_syncookies=1')
            ->and($shell->runs[0]['script'])
            ->toContain('kernel.randomize_va_space=2')
            ->and($shell->runs[0]['script'])
    ...
    it('locks down the managed home directory for the node runtime user', function (): void {
        ...
        expect($report->successful)
            ...
            ->and($shell->runs[0]['script'])
            ->toContain('/home/orbit/.config/orbit/logs')
            ->and($shell->runs[0]['script'])
    ...
    it('renders hardened sshd configuration bound to wireguard and loopback', function (): void {
        ...
        expect($report->successful)
            ...
            ->and($shell->runs[0]['script'])
            ->toContain('ListenAddress 10.6.0.44')
            ->and($shell->runs[0]['script'])
    ...
    it('declares protected public SSH deny rules and applies them to the public interface', function (): void {
        ...
        expect($report->successful)
            ...
            ->toBe(['v4', 'v4', 'v6'])
            ->and($shell->runs[0]['script'])
            ->toContain('install -y -qq ufw')
            ->and($shell->runs[0]['script'])
            ->toContain('UFW is inactive; public SSH deny rules were staged but UFW was not enabled.')
            ->and($shell->runs[0]['script'])
            ->toContain('ip -o link show type wireguard')
            ->and($shell->runs[0]['script'])
            ->toContain('Could not resolve WireGuard interface.')
            ->and($shell->runs[0]['script'])
            ->toContain('ufw allow in on "$WG_IFACE" proto tcp from 10.6.0.0/24')
            ->and($shell->runs[0]['script'])
            ->toContain("comment 'Orbit node security baseline permits SSH only through WireGuard.'")
            ->and($shell->runs[0]['script'])
            ->toContain('ufw deny in on "$PUBLIC_IFACE" proto tcp from 0.0.0.0/0')
            ->and($shell->runs[0]['script'])
            ->toContain("comment 'Orbit node security baseline denies public SSH after bootstrap.'")
            ->and($shell->runs[0]['script'])
            ->toContain('ufw deny in on "$PUBLIC_IFACE" proto tcp from ::/0')
            ->and($shell->runs[0]['script'])
            ->not->toContain('ufw --force enable');
    });
});
