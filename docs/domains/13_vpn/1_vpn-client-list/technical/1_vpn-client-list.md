...
| Path | Coverage |
| --- | --- |
| `tests/Feature/Commands/Vpn/VpnClientListCommandTest.php` | Command contract: caller-role denial, operator-caller gateway-local SSH execution, gateway execution, backend authentication, TOTP handling, empty list success, node peer classification, and read-only behavior. |
