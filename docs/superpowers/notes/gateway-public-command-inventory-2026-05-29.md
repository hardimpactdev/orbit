...
| `orbit:update-lease-heartbeat` | `App\Console\Commands\UpdateLeaseHeartbeatCommand` | `gateway-owned` | none in required app call-site sweep | `keep` | none; hidden update lease heartbeat sidecar command | apps/gateway/tests/Feature/Commands/UpdateLeaseHeartbeatCommandTest.php |
