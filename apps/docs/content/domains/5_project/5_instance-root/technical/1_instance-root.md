...
1.  **Write Configuration:** Update `root` in the selected instance's
    driver configuration. The project is unchanged. If the requested root equals the current configuration, the
...
`project.creation_failed` and other drift codes specific to this command are
