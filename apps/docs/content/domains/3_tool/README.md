...
| [`openclaw`](catalog/openclaw.md) | OpenClaw | Host-managed runtime as `agent` with related `openclaw-gateway` process on port `8081` | Installable and removable by Orbit | `agent` | install, remove, update, reconfigure, credentials, service endpoint, fix, adopt; lifecycle and logs through `process:*` |
