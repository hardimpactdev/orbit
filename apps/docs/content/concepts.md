...
- **Metrics role** — optional private workload role that records Prometheus and Grafana process intent for a metrics node and node-exporter process intent for metrics and active workload nodes. See [Node Concepts](domains/1_node/node-concepts.md) and [Metrics Concepts](domains/20_metrics/metrics-concepts.md).
...
- **Metrics backend** — process-owned Prometheus, Grafana, and node-exporter runtime intent coordinated by the `metrics` role baseline. See [Metrics Concepts](domains/20_metrics/metrics-concepts.md).
