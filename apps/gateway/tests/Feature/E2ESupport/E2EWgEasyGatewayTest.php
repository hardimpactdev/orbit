...
it('persists the fixed wg-easy server keypair during startup', function (): void {
    $this->wgEasyGatewayTemp = sys_get_temp_dir().'/orbit-e2e-wg-easy-gateway-'.bin2hex(random_bytes(4));
    $databasePath = "{$this->wgEasyGatewayTemp}/wg-easy.db";
    createE2EWgEasyGatewayFixtureDatabase($databasePath);
    ...
    (new E2EWgEasyGateway($databasePath))->start($instance, '10.231.0.11');

    runE2EWgEasyGatewayPhpBlock($commands[0], 'ORBIT_WG_EASY_START_PHP', [
        'ORBIT_WG_EASY_ADVERTISED_HOST' => '10.231.0.11',
        'ORBIT_WG_EASY_DB_PATH' => $databasePath,
        'ORBIT_WG_EASY_PRIVATE_KEY' => E2EWireGuardMesh::FIXED_KEYS['wg-easy']['private_key'],
        'ORBIT_WG_EASY_PUBLIC_KEY' => E2EWireGuardMesh::FIXED_KEYS['wg-easy']['public_key'],
    ]);

    $interface = e2eWgEasyGatewayRow($databasePath, 'interfaces_table', "name = 'wg0'");

    expect($commands[0])->toContain('ORBIT_WG_EASY_PRIVATE_KEY=')
        ->and($commands[0])->toContain('ORBIT_WG_EASY_PUBLIC_KEY=')
        ->and($interface['private_key'])->toBe(E2EWireGuardMesh::FIXED_KEYS['wg-easy']['private_key'])
        ->and($interface['public_key'])->toBe(E2EWireGuardMesh::FIXED_KEYS['wg-easy']['public_key']);
});
