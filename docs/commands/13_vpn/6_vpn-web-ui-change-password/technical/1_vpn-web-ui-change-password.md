...
Gateway callers rotate the backend credential locally after destructive
consent. Control callers resolve input and destructive consent locally, open the
gateway-local execution path over SSH through Orbit/WireGuard, then rotate the
credential on the gateway. App-node and unknown callers are denied before
prompts, backend authentication, or side effects.
