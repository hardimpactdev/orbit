# `orbit solo:scratchpad:create`

Create a Solo scratchpad.
...
`solo:scratchpad:create` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `POST /api/solo/scratchpad/create`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
