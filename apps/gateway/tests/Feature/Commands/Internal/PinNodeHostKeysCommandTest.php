...
describe('orbit:internal:pin-node-host-keys', function (): void {
    ...
    it('pins hosted app and agent nodes while skipping topology peer identities', function (): void {
        ...
        expect(array_column($this->hostKeyPinner->calls, 'host'))
            ->toBe([
                '10.6.0.6',
                '10.6.0.4',
            ])
            ->and($agent->refresh()->host_key_fingerprint)
            ->toBe('SHA256:10-6-0-6')
            ->and($app->refresh()->host_key_fingerprint)
            ->toBe('SHA256:10-6-0-4')
            ->and($gateway->refresh()->host_key_fingerprint)
            ->toBeNull()
            ->and($operator->refresh()->host_key_fingerprint)
            ->toBeNull();
    });
