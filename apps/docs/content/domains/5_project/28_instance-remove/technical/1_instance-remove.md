...
Deletion removes one `AppInstance` and cascades only its instance-owned
...
| --- | --- | --- | --- |
| `DELETE` | `/api/projects/{project}/instances/{instance}` | `instance:write` | Remove one instance. |
...
Unknown targets return `project.not_found` or `instance.not_found`.
...
| Field | Value |
| --- | --- |
| Type | `api:DELETE /projects/{project}/instances/{instance}` |
| Effect | `destructive` |
...
| `apps/gateway/tests/Feature/AppInstanceControllerTest.php` | Authorization, consent, deletion scope, and response shape. |
