...
it('starts a managed system service tool on an app node from the gateway', function (): void {
    ...
    try {
        toolStartSeedGatewayIntent($topology);

        $result = $topology->ssh(
            'gateway',
            sprintf(
                'cd %s && php apps/gateway/artisan tool:start supervisor --node=app-dev-1 --json',
...
function toolStartSeedGatewayIntent(E2ETopologyHarness $topology): void
{
    $php = <<<'PHP'
