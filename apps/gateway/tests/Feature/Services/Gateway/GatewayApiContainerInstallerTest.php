...
describe('GatewayApiContainerInstaller', function (): void {
    ...
    it('issues a leaf certificate and routes the gateway API through orbit-caddy to orbit-gateway', function (): void {
        ...
        expect($writtenGlobalCaddyfile)
            ...
            ->toContain(':443 {')
            ->and($writtenGatewayApiCaddyfile)
            ->not->toContain('https://10.6.0.2:443')->and($writtenGatewayApiCaddyfile)
            ->not->toContain('bind 10.6.0.2')->and($writtenGatewayApiCaddyfile)->toContain(
                'tls /etc/orbit/certs/10.6.0.2.crt /etc/orbit/certs/10.6.0.2.key',
            )->and($writtenGatewayApiCaddyfile)->toContain('reverse_proxy http://orbit-gateway:8080')->and(
                $writtenGatewayApiCaddyfile,
            )->toContain('flush_interval -1')->and($writtenGatewayApiCaddyfile)->toContain(
                'header_up X-Forwarded-Proto http',
            )->and($writtenGatewayApiCaddyfile)->toContain('header_up X-Forwarded-Proto https')->and(
                $writtenGatewayApiCaddyfile,
            )->toContain('request_header -X-Orbit-WireGuard-Ip')->and(substr_count(
                $writtenGatewayApiCaddyfile,
                'header_up X-Orbit-WireGuard-Ip {remote_host}',
            ))->toBe(2)->and($writtenGatewayApiCaddyfile)
            ->not->toContain('php_fastcgi')->and($writtenGatewayApiCaddyfile)
            ->not->toContain('php-fpm')->and($writtenGatewayApiCaddyfile)
            ->not->toContain('orbit-api.sock');
    ...
    it('ensures the orbit-gateway container before writing the gateway API Caddy config', function (): void {
        ...
        expect($gatewayCreateIndex)
            ->not->toBeNull('orbit-gateway container must be created')->and($caddyConfigWriteIndex)
            ->not->toBeNull('gateway API Caddy config must be written')->and($caddyReloadIndex)
            ->not->toBeNull('orbit-caddy must be reloaded')->and($gatewayCreateIndex)->toBeLessThan(
                $caddyConfigWriteIndex,
                'orbit-gateway must be created before the Caddy config is written',
            )->and($caddyConfigWriteIndex)->toBeLessThan(
                $caddyReloadIndex,
                'Caddy config must be written before orbit-caddy is reloaded',
            );
    });
    ...
    it('reloads the orbit-caddy container and never installs or restarts host PHP-FPM or host Caddy', function (): void {
        ...
        Process::assertNotRan(
            fn ($process): bool => is_string($process->command) && str_contains($process->command, 'systemctl'),
        );
        Process::assertNotRan(
            fn ($process): bool => is_string($process->command) && str_contains($process->command, 'php-fpm'),
        );
        Process::assertNotRan(
            fn ($process): bool => is_string($process->command) && str_contains($process->command, 'php8.5-fpm'),
        );
        Process::assertNotRan(
            fn ($process): bool => is_string($process->command) && str_contains($process->command, 'tee /etc/php/'),
        );
        Process::assertNotRan(
            fn ($process): bool => (
                is_string($process->command) && str_contains($process->command, 'usermod -aG orbit caddy')
            ),
        );
    });
    ...
    it('copies issued gateway API TLS material to the caddy-readable /etc/orbit bind mount', function (): void {
        ...
        expect($writtenGatewayApiCaddyfile)
            ->toContain("tls {$caddyCertPath} {$caddyKeyPath}")
            ->and($writtenGatewayApiCaddyfile)
            ->not
            ->toContain($certsDir)
            ->and(gatewayApiInstallerPathIsCaddyVisible($caddyCertPath, $caddyContainer))
            ->toBeTrue('gateway API cert path must fall under an orbit-caddy bind mount')
            ->and(gatewayApiInstallerPathIsCaddyVisible($caddyKeyPath, $caddyContainer))
            ->toBeTrue('gateway API key path must fall under an orbit-caddy bind mount');
    ...
    it('preserves an existing global Caddyfile and only ensures managed imports and snippets', function (): void {
        ...
        Process::fake(function ($process) use (
            $readExistingCaddyfileCommand,
            &$writtenGlobalCaddyfile,
            &$writtenGatewayApiCaddyfile,
        ) {
        ...
        expect($writtenGlobalCaddyfile)
            ...
            ->and($writtenGlobalCaddyfile)
            ->toContain('import /etc/caddy/orbit/tld-proxies.caddy')
            ->and($writtenGlobalCaddyfile)
            ->toContain('import /etc/caddy/orbit/*.caddy')
            ->and(substr_count($writtenGlobalCaddyfile, 'import /etc/caddy/sites/*.caddy'))
            ->toBe(1)
            ->and(substr_count($writtenGlobalCaddyfile, 'import /etc/caddy/orbit/*.caddy'))
            ->toBe(1)
            ->and(substr_count($writtenGlobalCaddyfile, '{'))
            ->toBe(substr_count($writtenGlobalCaddyfile, '}'))
            ->and(substr_count($writtenGlobalCaddyfile, 'admin localhost:2019'))
            ->toBe(1)
            ->and($writtenGlobalCaddyfile)
            ->not->toContain('admin off')->and($writtenGatewayApiCaddyfile)->toContain(':80 {')->and(
                $writtenGatewayApiCaddyfile,
            )->toContain(':443 {')->and($writtenGatewayApiCaddyfile)
            ->not->toContain('https://10.6.0.2:443');
    });
});
