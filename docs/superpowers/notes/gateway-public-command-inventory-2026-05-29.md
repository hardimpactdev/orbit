...
- Invokable command rows covered: 12 total, including 1 gateway-owned runtime row (orbit-scheduler), 1 gateway-owned release maintenance row, 9 internal `App\Console\Commands\Internal\*` rows kept explicit for the allowed `orbit:internal:*` category, and `orbit:internal:node-register`. E2E runner commands (e2e:*) were extracted to apps/e2e in #9H and are no longer registered in gateway Artisan.
...
| `orbit:release-candidate:activate` | `App\Console\Commands\ReleaseCandidateActivateCommand` | `gateway-owned` | none in required app call-site sweep | `keep` | none; gateway-owned release maintenance command | apps/gateway/tests/Feature/Release/CandidateReleaseChannelCommandTest.php |
