...
| --- | --- | --- | --- | --- | --- |
| `domain` | `argument` | `Required in non-interactive mode.` | `Never.` | `None.` | Existing custom route domain, direct app/analytics/WebSocket route whose owning Instance is gone, or structurally complete tool-owned route whose matching installed tool is absent. |
...
- With destructive consent, also removes a direct `app`, `app-analytics`, or
  `app-websocket` route when its durable `instance_id` is null or references an
  Instance row that is absent. A conflicting tuple whose Instance still
  exists stays denied as repairable divergence.
- With destructive consent, also removes a structurally complete tool-owned
  ...
  keys.
- `--force` never becomes a general ownership bypass. Invalid app, analytics,
  WebSocket, workspace, gateway, S3, or other ownership does not prove
  removable ownership while the recorded owner still exists. Those non-custom
  routes remain stored and return `proxy.owned_route_denied`; invalid custom
  ownership remains stored and returns `proxy.owner_invalid`.
...
- When a missing owner is removed successfully, the success payload includes
  `removal_reason=orphan_owner` and the recorded `owner_type` (`app`,
  `app-analytics`, `app-websocket`, or `tool`) so operators can audit the
  repair.
...
- Orphan-owner removal uses the same destructive consent rules as custom route
  removal; there is no separate orphan flag.
...
repair exceptions are a direct app, analytics, or WebSocket route whose owning
Instance is gone, and a route with structurally complete tool ownership when
its matching installed `NodeTool` is absent on the serving node. Other
`proxy.owner_invalid` rows, including conflicting tuples whose Instance still
exists, are report-only until their owner domain or a later product decision
supplies a safe repair.
...
| Owned route denied | The route has non-custom ownership and is not an eligible orphan-owner repair. A living direct owner or other invalid ownership fails closed. | `error.code=proxy.owned_route_denied` |
...
remove a direct app, analytics, or WebSocket row whose owning Instance is gone,
or a structurally complete tool-owned row when the matching installed
`NodeTool` is absent on the serving node. Proxy Doctor can report those rows as
`proxy.owner_invalid`, but restore does not delete them. Other invalid
ownership tuples are report-only. Doctor is only for genuine cleanup failures
and remaining orphan extras; it is not a mandatory second step after a healthy
...
| Path | Coverage |
| --- | --- |
| `apps/gateway/tests/Feature/Http/Api/ProxyRouteMutationControllerTest.php` | Gateway proxy route removal authorization, custom route deletion, orphan-tool force removal, owned-route denial, destructive consent requirement, and mutation API shape. |
| `apps/cli/tests/Feature/Commands/Proxy/ProxyWriteCommandTest.php` | CLI `proxy:remove` force consent handling, interactive confirmation, DELETE forwarding, JSON success envelope, dynamic orphan-owner safety prose, and gateway error passthrough. |
| `apps/gateway/tests/Unit/Services/Proxy/ProxyRouteIntentTest.php` | Custom removal, missing direct-instance and complete missing-tool-owner force removal, living or malformed owner retention, cleanup failure keeps registry, ownership checks, and authorization. |
