...
On a node with an app role, the CLI remains a thin gateway client: it gathers command
input, sends the request to the gateway over the VPN, and renders the gateway
rejection. The CLI does not inspect local app-role role state or reject the
