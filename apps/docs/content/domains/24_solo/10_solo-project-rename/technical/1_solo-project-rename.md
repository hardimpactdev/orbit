...
- The caller has `solo:project:update` on the serving gateway node.
...
The command calls `PATCH /api/solo/project/rename` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
