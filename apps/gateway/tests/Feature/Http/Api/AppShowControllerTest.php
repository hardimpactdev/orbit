 ...
 */
function grantAppShowAccess(Node $caller, Node $appNode, array $permissions = ['project:read']): void
{
...
function create_app_show_instance(Project $app, Node $node, string $name = 'development'): AppInstance
{
...
describe('AppShowController', function (): void {
    it('returns app registry details by name', function (): void {
        ...
        $response = $this->call('GET', '/api/projects/docs', [], [], [], ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP]);
        ...
        $response
            ...
            ->assertJsonPath('success.data.project.dependency_audit_status', 'unknown')
            ->assertJsonPath('success.data.project.dependency_warning_count', 0)
            ->assertJsonPath('success.data.project.dependency_danger_count', 0)
            ->assertJsonPath('success.data.project.last_dependency_audit_at', null)
    ...
    it('returns dependency audit posture details by app name', function (): void {
        ...
        $response
            ->assertOk()
            ->assertJsonPath('success.data.project.dependency_audit_status', 'findings')
            ->assertJsonPath('success.data.project.dependency_warning_count', 14)
            ->assertJsonPath('success.data.project.dependency_danger_count', 2)
            ->assertJsonPath('success.data.project.last_dependency_audit_at', '2026-07-02T08:15:00+00:00')
    ...
    it('returns app process definitions with concrete instance identity', function (): void {
        ...
        $this
            ->call('GET', '/api/projects/docs', [], [], [], ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP])
            ->assertOk()
            ->assertJsonFragment([
                'name' => 'queue',
    ...
    it('returns only instances and workspaces placed on visible serving nodes', function (): void {
        ...
        $app = Project::factory()->for($hiddenNode, 'node')->create([
            'name' => 'docs',
            'domain' => null,
        ]);
    ...
    it('omits workspace details for app production callers and production placements', function (): void {
        ...
        $developmentApp = Project::factory()->for($developmentNode, 'node')->create(['name' => 'docs']);
        ...
        $productionApp = Project::factory()->for($productionNode, 'node')->create(['name' => 'shop']);
        ...
        $this
            ->call('GET', '/api/projects/shop', [], [], [], ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP])
            ->assertOk()
            ->assertJsonPath('success.data.details.instances.0.workspaces', [])
            ->assertJsonMissingPath('success.data.details.workspaces');
    });
    ...
    it('lets gateway callers resolve external instances by hostname', function (): void {
        ...
        $this
            ->call('GET', '/api/projects/docs.example.com', [], [], [], ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP])
            ->assertOk()
            ->assertJsonPath('success.data.project.name', 'docs');
    });
    ...
    it('resolves by hostname when no app name matches', function (): void {
        ...
        $response->assertOk()->assertJsonPath('success.data.project.name', 'docs');
    });
    ...
    it('prefers app name over hostname collisions', function (): void {
        ...
        $hostnameApp = Project::factory()->create([
            'name' => 'docs',
            'node_id' => $node->id,
            'domain' => 'docs.example.com',
        ]);
        $nameApp = Project::factory()->create([
        ...
        $response->assertOk()->assertJsonPath('success.data.project.name', 'docs.example.com');
    });

    it('rejects hidden apps when the caller lacks project:read on the owning node', function (): void {
        $caller = createAppShowCallerNode();
        $node = createTestAppHostNode();
        grantAppShowAccess($caller, $node, ['node:read']);
        $app = Project::factory()->create(['name' => 'hidden', 'node_id' => $node->id]);
        create_app_show_instance($app, $node);

        $response = $this->call('GET', '/api/projects/hidden', [], [], [], ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP]);
    ...
    it('authorizes hostname selectors against the owning node', function (): void {
        ...
        $response = $this->call(
            'GET',
            '/api/projects/hidden.example.com',
            [],
            [],
            [],
            ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP],
    ...
    it('lets an app role node show only its own app registry rows through its self grant', function (): void {
        ...
        $ownedApp = Project::factory()->create(['name' => 'owned', 'node_id' => $otherNode->id]);
        $hiddenApp = Project::factory()->create(['name' => 'hidden', 'node_id' => $caller->id]);
        create_app_show_instance($ownedApp, $caller);
        create_app_show_instance($hiddenApp, $otherNode);

        $response = $this->call('GET', '/api/projects/owned', [], [], [], ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP]);

        $response->assertOk()->assertJsonPath('success.data.project.name', 'owned');

        $hidden = $this->call('GET', '/api/projects/hidden', [], [], [], ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP]);
    ...
    it('returns not found for absent apps', function (): void {
        ...
        $response = $this->call('GET', '/api/projects/missing', [], [], [], ['REMOTE_ADDR' => APP_SHOW_CALLER_WG_IP]);

        $response
            ->assertNotFound()
            ->assertJsonPath('error.code', 'project.not_found')
            ->assertJsonPath('error.message', "Project 'missing' not found.");
    });
    ...
    it('rejects unauthenticated requests', function (): void {
        $response = $this->getJson('/api/projects/docs');
