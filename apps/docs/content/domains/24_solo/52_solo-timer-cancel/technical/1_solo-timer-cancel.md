...
- The caller has `solo:timer:*` on the serving gateway node.
...
The command calls `DELETE /api/solo/timer/cancel` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
