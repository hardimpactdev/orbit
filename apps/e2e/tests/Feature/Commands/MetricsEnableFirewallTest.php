    ...
    try {
        ...
        foreach (['agent-1', 'app-dev-1', 'app-prod-1'] as $nodeName) {
            ...
            $restore = $topology->ssh(
                'gateway',
                "cd {$checkout} && orbit doctor --node="
                .escapeshellarg($nodeName)
                .' --family=firewall_rule --key=firewall_rule.rule_missing --restore --json',
                timeoutSeconds: 180,
            );
            ...
            expect($restore->successful())
                ...
                ->and($doctor['doctor']['healthy'])
                ->toBeTrue(json_encode($doctor, JSON_PRETTY_PRINT));
