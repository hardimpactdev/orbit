...
class NodeRoleUpdateCommand extends Command
{
    use WithSpinner;
    use WithStepTree;

    protected $name = 'node role:update';

    protected $description = 'Update a hosted role on a node';
        ...
        if ((bool) config('orbit.is_gateway', false)) {
            ...
            try {
                ...
                $operation = function () use ($service, $node, $role, $settings, &$assignment): string {
                    $assignment = $service->update($node, $role, $settings);

                    return 'converged';
                };

                if (! $this->wantsJson()) {
                    $exitCode = $this->runRoleMutationTree($nodeName, $role, $operation, 'Role updated');
                ...
                return $this->respondMutation($nodeName, NodeRoleAssignmentPayload::fromModel($assignment));
        ...
        try {
            ...
            $operation = function () use ($nodeName, $role, $settings, &$dto): string {
                ...
                $dto = app(GatewayConnector::class)
                    ->send(new UpdateNodeRoleRequest($nodeName, $role, $settings))
                    ->dto();
            ...
            return $this->respondMutation($dto->node, $dto->assignment);
        } catch (GatewayApiException $exception) {
            return $this->failCommand(
                $exception->errorCode() ?? 'gateway_unavailable',
                $exception->getMessage() !== '' ? $exception->getMessage() : 'Gateway connection is required to update a node role.',
                $exception->errorMeta(),
            );
        } catch (Throwable) {
            return $this->failCommand('gateway_unavailable', 'Gateway connection is required to update a node role.', []);
        }
    }
     ...
     */
    private function respondMutation(string $nodeName, array $assignment): int
    {
        if ($this->wantsJson()) {
        ...
        $this->line("Role '{$assignment['role']}' updated on '{$nodeName}'");

        return self::SUCCESS;
    }

    private function runRoleMutationTree(string $nodeName, string $role, callable $operation, string $footer): int
    {
        return $this->runStepTree('Update Node Role', [
            [
        ...
        ], doneFooter: $footer, failFooter: "Failed to update role '{$role}' on '{$nodeName}'");
    }
