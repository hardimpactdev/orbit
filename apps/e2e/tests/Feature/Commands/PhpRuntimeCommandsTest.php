...
it('reads and changes PHP runtime intent without installing runtimes', function (): void {
    ...
    try {
        ...
        expect($list->successful())
            ->toBeTrue()
            ->and($listPayload['success']['data']['php'])
            ->toMatchArray([
                'node' => 'app-dev-1',
        ...
        $workspaceUsePayload = json_decode(
            trim($workspaceUse->output()),
            associative: true,
        ...
        expect($appUse->successful())
            ...
            ->and($workspaceUse->successful())
            ->toBeTrue()
            ->and($workspaceUsePayload['success']['data']['result'])
            ->toMatchArray([
                'target' => 'workspace',
            ...
            ->and($cliUse->successful())
            ->toBeTrue()
            ->and($cliUsePayload['success']['data']['result'])
            ->toMatchArray([
                'target' => 'node_cli',
                'node' => 'app-dev-1',
                'previous' => '8.4',
        ...
        expect($installLog->successful())->toBeTrue()->and(trim($installLog->output()))->toBe('');
    } finally {
        $topology->cleanup();
    }
