...
function createToolStopInteractiveLocalNode(string $role = 'gateway'): Node
{
...
describe('tool:stop interactive input mode', function (): void {
    it('prompts for tool name when omitted in interactive mode', function (): void {
        createToolStopInteractiveLocalNode('gateway');
        ...
        $this->artisan('tool:stop --node=app-1')
    ...
    it('does not prompt when tool argument is supplied', function (): void {
        ...
        $this->artisan('tool:stop', ['tool' => 'redis', '--node' => 'app-1', '--json' => true])
    ...
    it('fails with validation_failed when tool is missing in non-interactive mode', function (): void {
        ...
        $exitCode = Artisan::call('tool:stop', ['--json' => true]);
