...
- `name`: schedule slug, unique within the selected concrete app instance or node target.
- `--app`: concrete `app.instance` target for an app-scoped schedule. A bare
...
- The caller is authorized to manage schedules on the selected app instance's
