...
On Apple Silicon, Colima 0.10.0 or newer is required; older versions fail
closed as unsupported before any start or reset mutation. Colima creation and
existing starts set VZ,
`aarch64`, `--vz-rosetta=false`, and `--binfmt=false`. Desktop accepts readiness
only after
