# `orbit solo:process:stop`

Stop one Solo process after explicit force consent.

## Usage

```bash
orbit solo:process:stop <process> [--force] [--json]
```

## Contract

`solo:process:stop` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `POST /api/solo/process/stop`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
