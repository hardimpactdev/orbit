# Technical Contract: `node:revoke` On An App Node
...
- No command argument, option, gateway execution context, SSH target, or
  WireGuard identity prerequisite can make `node:revoke` valid from an app node.

## Allowed Paths

None. `node:revoke` may not run from an app node.
...
- Do not read or mutate gateway node records or node access grants.
...
| Path | Coverage |
| --- | --- |
| `tests/Feature/Commands/Nodes/NodeRevokeOnAppNodeContractTest.php` | Primary owner for app-caller rejection: explicit `general.local_node_role=app` callers fail before prompts, forwarding, local writes, SSH, WireGuard changes, gateway node-record reads, or other side effects. Renderer tests own the human and JSON formatting of that error. |