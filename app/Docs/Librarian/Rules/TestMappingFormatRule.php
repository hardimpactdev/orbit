...
final readonly class TestMappingFormatRule implements GroupedRule
{
    ...
    private function checkFile(string $file, string $contents): array
    {
        ...
        if ($rows === []) {
            return [
                $this->finding($file, 'Test Mapping must include at least one table row with a `tests/...Test.php` path and coverage description.'),
            ];
        }
        ...
        foreach ($rows as $row) {
            if (! str_starts_with($row['path'], 'tests/')) {
                $findings[] = $this->finding($file, "Mapped test path must live under tests/: {$row['path']}.");
            }
