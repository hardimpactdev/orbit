...
/**
 * Removal-only migration cleanup for Orbit-managed PolyScope residue after
 * first-party catalog support was withdrawn.
 *
 * @mago-expect lint:cyclomatic-complexity
 */
final readonly class LegacyPolyscopeRuntimeCleanup
{
    public const string TOOL = 'polyscope-server';

    public const string PROCESS_NAME = 'polyscope-server';
    ...
    public function cleanupScript(): string
    {
        ...
        return <<<BASH
            #!/usr/bin/env bash
            # orbit legacy-remove polyscope-server (removal-only migration; not product support)
            set -u
            # Fixed Orbit-managed targets — never env-overridable.
            POLYSCOPE_USER='agent'
            ...
            sudo pkill -u "\${POLYSCOPE_USER}" -f 'polyscope-server' >/dev/null 2>&1 || true
            sudo rm -f "\${POLYSCOPE_BIN}" 2>/dev/null || true
            if sudo pgrep -u "\${POLYSCOPE_USER}" -f 'polyscope-server' >/dev/null 2>&1; then
              echo "legacy polyscope cleanup incomplete: polyscope-server process still running for user \${POLYSCOPE_USER}" >&2
              exit 1
            fi
            if [ -e "\${POLYSCOPE_BIN}" ]; then
              echo "legacy polyscope cleanup incomplete: \${POLYSCOPE_BIN} still exists" >&2
              exit 1
            fi
            if [ -f "/etc/systemd/system/{$processUnit}" ] || [ -f "/lib/systemd/system/{$processUnit}" ] || [ -f "/usr/lib/systemd/system/{$processUnit}" ]; then
              echo "legacy polyscope cleanup incomplete: {$processUnit} unit file still present" >&2
              exit 1
            fi
            exit 0
            BASH;
    ...
    public function remove(?string $node = null, ?string $app = null): array|ToolRegistryFailure
    {
        ...
        if (! $target instanceof Node) {
            return ToolRegistryFailure::validation(
                'target',
                $node ?? $app ?? '',
                'Unable to resolve a tool-host node for legacy PolyScope removal.',
