...
```bash
orbit instance:env set billing.development --key=APP_DEBUG --value=false
orbit instance:env set billing.development --key=MAIL_MAILER --value=smtp --apply
orbit database:attach billing-db --instance=billing.development --env-prefix=DB
orbit instance:env render billing.development --json
```

## Arguments and options

- `action`: one of `list`, `set`, `render`. Required.
- `instance`: dotted `project.instance` selector. Required.
...
- `--apply`: for `set` only. Persist the value in gateway state and apply it to
  the selected instance's live `.env`, clear Laravel config and generated
  bootstrap cache files at that instance path as its runtime user, and reapply
  that instance's runtime container.
