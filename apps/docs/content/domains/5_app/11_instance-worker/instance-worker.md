...
- The FrankenPHP worker file exists at `<document_root>/frankenphp-worker.php`
  under the instance source. The path is resolved against the Instance
  `driver_config.document_root`, so an instance with the default
  `document_root=public` is checked at `public/frankenphp-worker.php`, an
  instance with `document_root=web` at `web/frankenphp-worker.php`, and an
  instance with `document_root=.` at `frankenphp-worker.php` directly under
  the instance root. This file is
