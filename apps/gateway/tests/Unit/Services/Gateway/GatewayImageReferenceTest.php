...
it('parses a registry image reference with tag and digest', function (): void {
    $reference = GatewayImageReference::fromString(
    ...
    expect($reference->registry())
        ...
        ->and($reference->isDigestPinned())
        ->toBeTrue()
        ->and($reference->canonical())
        ->toBe(
            'ghcr.io/hardimpactdev/orbit-gateway:1.2.3@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa',
        );
});
...
it('parses a local image reference with a tag', function (): void {
    ...
    expect($reference->registry())
        ...
        ->toBeFalse()
        ->and($reference->canonical())
        ->toBe('orbit-gateway:current');
});

it('parses a digest-only registry image reference', function (): void {
    $reference = GatewayImageReference::fromString(
        'ghcr.io/hardimpactdev/orbit-gateway@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb',
    );
...
it('rejects invalid gateway image references', function (string $image, string $message): void {
    ...
    'duplicate digest separator' => [
        'ghcr.io/hardimpactdev/orbit-gateway:1.2.3@sha256:aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa@sha256:bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb',
        'must contain at most one digest separator',
    ],
