...
2. **Node eligibility:** the node reference resolves to a visible active Ubuntu node with at least one active role assignment from `gateway`, `router`, `app-dev`, `app-prod`, `database`, `agent`, or `ingress`.
