...
describe('tool write commands', function (): void {
    it('posts tool:install payloads to the gateway', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['tool']['state'])->toBe('running');
    });
    ...
    it('posts tool lifecycle actions to their gateway endpoints', function (string $command, string $endpoint): void {
        fakeGateway(fakeSuccessEnvelope([
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['tool']['action'])->toBe($endpoint);
    })->with([
        ['tool:start', 'start'],
        ['tool:stop', 'stop'],
    ...
    it('posts tool:update payloads to the single-tool gateway endpoint', function (): void {
        fakeGateway(fakeSuccessEnvelope([
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['tool']['version'])->toBe('7.2');
    });

    it('posts tool:update bulk payloads when the tool argument is omitted', function (): void {
        fakeGateway(fakeSuccessEnvelope([
            'updated' => [],
            'skipped' => [
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['skipped'])->toHaveCount(1);
    });

    it('posts tool:reconfigure password payloads to the gateway', function (): void {
        fakeGateway(fakeSuccessEnvelope([
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['tool']['action'])->toBe('reconfigured');
    });

    it('preserves gateway error envelopes for tool write commands', function (): void {
        fakeGateway(fakeErrorEnvelope('tool.unsupported_action', "Tool 'docker' does not support install.", [
            'tool' => 'docker',
            'action' => 'install',
        ]), 422);
