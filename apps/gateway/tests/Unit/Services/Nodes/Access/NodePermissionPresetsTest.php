...
describe('node permission presets', function (): void {
    ...
    it('excludes removed command exec permissions from all presets', function (): void {
        ...
        foreach ($presets->names() as $name) {
            expect($presets->permissions($name))->not->toContain('app:exec', 'workspace:exec');
        }
    });
    ...
    describe('agent-self preset', function (): void {
        it('contains the expected permissions', function (): void {
            ...
            expect($permissions)
                ...
                ->toContain('tool:read')
                ->and($permissions)
                ->toContain('tool:update:agent-tools')
                ->and($permissions)
                ->toContain('doctor:verify');
        });
        ...
        it('excludes credentials and install', function (): void {
            ...
            expect($permissions)
                ...
                ->not->toContain('doctor:restore')->and($permissions)
                ->not->toContain('doctor:adopt');
        });
    });
    ...
    describe('role self presets', function (): void {
        it('maps role names to self preset names', function (): void {
            ...
            expect($presets->selfPresetNameForRole(NodeRoleName::Gateway))
                ->toBeNull()
                ->and($presets->selfPresetNameForRole(NodeRoleName::Vpn))
                ->toBe('vpn-self')
                ->and($presets->selfPresetNameForRole(NodeRoleName::AppDevelopment))
                ->toBe('app-dev-self')
                ->and($presets->selfPresetNameForRole(NodeRoleName::AppProduction))
                ->toBe('app-prod-self')
                ->and($presets->selfPresetNameForRole(NodeRoleName::Database))
                ->toBe('database-self')
                ->and($presets->selfPresetNameForRole(NodeRoleName::Agent))
                ->toBe('agent-self')
                ->and($presets->selfPresetNameForRole('unknown'))
                ->toBeNull();
        });
    ...
    describe('operator preset', function (): void {
        ...
        it('excludes doctor:restore and doctor:adopt', function (): void {
            ...
            expect($permissions)
                ->not->toContain('doctor:restore')->and($permissions)
                ->not->toContain('doctor:adopt')->and($permissions)
                ->not->toContain('database:query')->and($permissions)
                ->not->toContain('database:query:write')->and($permissions)
                ->not->toContain('database:write');
        });
    });
    ...
    describe('read-only preset', function (): void {
        it('contains only read permissions', function (): void {
            ...
            foreach ($permissions as $permission) {
                expect(
                    str_contains($permission, ':read')
                    || $permission === 'doctor:verify'
                    || str_contains($permission, ':list')
                    || str_contains($permission, ':resolve'),
                )
    ...
    describe('developer preset', function (): void {
        ...
        it('includes process and schedule surfaces', function (): void {
            ...
            expect($permissions)
                ->toContain('process:read')
                ->and($permissions)
                ->toContain('process:add')
                ->and($permissions)
                ->toContain('schedule:read')
                ->and($permissions)
                ->toContain('schedule:write');
        });
        ...
        it('includes proxy and deploy surfaces', function (): void {
            ...
            expect($permissions)
                ->toContain('proxy:read')
                ->and($permissions)
                ->toContain('proxy:add')
                ->and($permissions)
                ->toContain('deploy:read')
                ->and($permissions)
        ...
        it('includes database registry and query surfaces', function (): void {
            ...
            expect($permissions)
                ...
                ->and($permissions)
                ->toContain('database:query:write');
        });
    ...
    describe('admin preset', function (): void {
        ...
        it('includes full firewall authority', function (): void {
            ...
            expect($permissions)->toContain('firewall_rule:read')->and($permissions)->toContain('firewall_rule:write');
        });
        ...
        it('includes doctor restore and adopt', function (): void {
            ...
            expect($permissions)
                ->toContain('doctor:restore')
                ->and($permissions)
                ->toContain('doctor:adopt')
                ->and($permissions)
        ...
        it('excludes gateway-level node permissions', function (): void {
            ...
            expect($permissions)
                ->not->toContain('node:grant')->and($permissions)
                ->not->toContain('node:revoke')->and($permissions)
                ->not->toContain('node:new')->and($permissions)
                ->not->toContain('node:remove')->and($permissions)
                ->not->toContain('node:migrate')->and($permissions)
                ->not->toContain('role:add')->and($permissions)
                ->not->toContain('role:update')->and($permissions)
                ->not->toContain('role:remove');
        });
