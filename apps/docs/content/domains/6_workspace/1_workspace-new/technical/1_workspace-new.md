...
| `--instance` | `text` | No explicit selector or usable local context. | CWD-inferred concrete instance. | Dotted selectors choose one instance directly. A bare app or path context must resolve uniquely. |
...
| `--php-version` | `text` | Optional. | (owning instance PHP version) | Supported PHP version. When omitted, the workspace row stores a snapshot of the owning instance's version rather than inheriting the parent app's PHP version. |
...
3. **Validate Workspace Identity (gateway-side, static):**
   ...
   - Reserved name: `main` is reserved for the primary instance in
     runtime unit naming (`orbit_<app>_<instance>_main_<process>`); a workspace named
...
4. **Resolve PHP Version (`--php-version`):** If supplied, validate against
   ...
   stores a snapshot of the owning instance's PHP version, so a later change to
   that instance or to the app template never moves this workspace.
