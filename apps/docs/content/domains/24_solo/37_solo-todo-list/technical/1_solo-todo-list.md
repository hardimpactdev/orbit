# Technical Contract: `orbit solo:todo:list`

[Back to public `solo:todo:list` documentation.](../solo-todo-list.md)
...
```bash
orbit solo:todo:list [--node=<node>] [--node-transport=<transport>] [--project=<project>] [--json]
```
...
The command calls `GET /api/solo/todo/list` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above on the target node, or on the gateway node when `--node` is omitted.
