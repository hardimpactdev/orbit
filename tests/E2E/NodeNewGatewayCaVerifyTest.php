...
it('NodeNewGatewayCaVerify proves first-gateway CA trust from base VMs', function (): void {
    ...
    try {
        ...
        e2eProvisionStep('authorize SSH between control and gateway', fn () => nodeNewGatewayCaVerifyAuthorizeSsh($control, $gateway, $config->controlUser, $config->bootstrapUser, $key));
        ...
        expect($settings['gateway_url'])->toBe("https://{$gatewayIp}")
            ...
            ->and($settings['ca_pem_path'])->toBe("/home/{$config->controlUser}/orbit/storage/app/orbit/trust/gateway-1-ca.crt")
...
function nodeNewGatewayCaVerifyAuthorizeSsh(
    E2EInstance $control,
    E2EInstance $gateway,
    string $controlUser,
