...
class ToolStartCommand extends Command
{
        ...
        if (! $this->wantsJson()) {
            $progressResult = $this->isGatewayCaller()
                ...
                : $this->runGatewayToolActionProgress(
                    ...
                    unavailableMessage: 'Gateway connection is required to start tools.',
                    defaultFooter: 'Tool start failed',
                );
            ...
            $this->line("Started {$result['name']} on {$result['node']}.");

            return self::SUCCESS;
        }

        $result = $this->isGatewayCaller()
            ? $lifecycle->start($tool, node: $node, app: $app)
            : $this->startViaGateway($tool, node: $node, app: $app);
        ...
        if ($result instanceof GatewayApiException) {
            return $this->failCommand(
                ...
                message: $result->getMessage() !== ''
                    ? $result->getMessage()
                    : 'Gateway connection is required to start tools.',
     ...
     */
    private function startViaGateway(string $tool, ?string $node, ?string $app): array|GatewayApiException
    {
        try {
            $dto = app(GatewayConnector::class)
                ->send(new StartToolRequest(tool: $tool, app: $app, node: $node))
                ->dto();
        } catch (GatewayApiException $e) {
            return $e;
        } catch (Throwable) {
            return new GatewayApiException(
                message: 'Gateway connection is required to start tools.',
     ...
     */
    private function runStartLocalToolActionProgress(
        string $title,
        string $doneFooter,
        string $failFooter,
        callable $operation,
        ...
        $reporter->tree($title, $this->toolStartProgressSteps());

        try {
            $result = $this->runStartProgressOperation($reporter, $operation);
     ...
     */
    private function toolStartProgressSteps(): array
    {
        return [
            [
                'key' => 'resolve-target',
     ...
     */
    private function runStartProgressOperation(RemoteProgressReporter $reporter, callable $operation): array|ToolRegistryFailure|GatewayApiException
    {
