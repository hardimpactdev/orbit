...
- Instance path is derived from the app name and the target node's app root.
...
- **Apply Verification:** Verify that command-owned setup and artifact
  ...
  may still need app setup steps before it is healthy, and durable runtime
...
`schedule:add --instance=<app.instance>` after the concrete instance exists when
...
- **Apply Drift:** If configuration is written but registration (runtime
  ...
  --instance=<app.instance> --restore` or `instance:register <app.instance>
