...
- Prepends the scoped UFW allow before existing policy so an earlier deny does not make the requested allow ineffective. UFW computes the correct placement for IPv4 and IPv6 rules.
- Uses the stored reason as the UFW comment. If no reason is stored, uses `orbit:<name>` as the stable backend identity for safe convergence.
