...
| `app-development-setup-step:add` | `app:write` | an app-owned development default | App selector; app ownership is checked through a visible app instance | `authorization_failed` | Standard missing-permission meta |
| `app-development-setup-step:list` | `app:read` | an app-owned development default | App selector; app ownership is checked through a visible app instance | `authorization_failed` | Standard missing-permission meta |
| `app-development-setup-step:update` | `app:write` | an app-owned development default | App selector; app ownership is checked through a visible app instance | `authorization_failed` | Standard missing-permission meta |
| `app-development-setup-step:remove` | `app:write` | an app-owned development default | App selector; app ownership is checked through a visible app instance | `authorization_failed` | Requires explicit destructive consent |
...
| `php:use` | `php:write` | explicit CLI target node, selected workspace serving node, or the selected instance's serving node | The write authorizes exactly one resolved serving node and verifies that node's image availability before the instance row changes; sibling instances and workspaces are not preauthorized because they are not written | `authorization_failed` | Standard missing-permission meta plus denied `instance` and serving node when instance-scoped |
...
| `process:log` | `process:log` (`process:read` implies it) | node owner, or concrete instance serving node | Replaces removed public `process:logs` / permission `process:logs` with no aliases | `authorization_failed` | Standard missing-permission meta plus process and instance when applicable |
| `instance:log` | `instance:read` | resolved concrete instance serving node | Fixed Laravel application log only; no separate application-log permission | `authorization_failed` | Standard missing-permission meta plus instance and serving node |
| `app:log` | `instance:read` or `workspace:read` for the resolved target | resolved instance or workspace serving node | URL/hostname proxy resolution only; permission follows resolved target type | `authorization_failed` | Standard missing-permission meta plus resolved target type, identity, and serving node |
...
| `workspace:log` | `workspace:read` | resolved concrete workspace's instance serving node | Fixed Laravel application log only; optional CLI `--instance` for name disambiguation; API requires parent `instance` | `authorization_failed` | Standard missing-permission meta plus workspace and `instance` |
| `workspace:run:log` | `workspace:run:log` | resolved concrete workspace's instance serving node | Replaces lifecycle-history use of public `workspace:log` / permission `workspace:log` with no aliases; route remains `GET /api/workspaces/runs/{run}/log` | `authorization_failed` | Standard missing-permission meta plus workspace run and `instance` |
