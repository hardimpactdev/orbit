...
```bash
orbit solo:scratchpad:create <name> [--content=<content>] [--json]
```
...
The command calls `POST /api/solo/scratchpad/create` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
