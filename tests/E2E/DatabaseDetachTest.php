...
it('detaches a database connection from an app from the control node through the gateway api', function (): void {
...
PHP;
        ...
        $result = $topology->ssh(
            'control',
            sprintf(
                'cd %s && php artisan database:detach %s --app=%s --env-prefix=DB --json',
