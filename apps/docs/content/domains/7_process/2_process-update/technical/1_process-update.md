...
```bash
orbit process:update [name] [--instance=<app.instance>] [--workspace=<workspace>] [--node=<node>] [--name=<new-name>] [--label=<label>] [--command=<command>] [--restart-policy=<never|on_failure|always>] [--crash-notification=<none>] [--runtime=<docker|docker-swarm|systemd|launchd>] [--bind=<wireguard|loopback>] [--restart] [--json]
```
