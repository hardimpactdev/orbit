...
- The caller has `solo:todo:lock` on the serving gateway node.

## Signature

```bash
orbit solo:todo:lock <todo> [--project=<project>] [--title=<title>] [--body=<body>] [--force] [--json]
```
...
The command calls `POST /api/solo/todo/lock` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
