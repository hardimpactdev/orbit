...
it('routes the private websocket service to a websocket-role backend through the router', function (): void {
    ...
    try {
        ...
        expect($snapshot['schema']['nodes_role_column'])->toBeFalse()
            ...
            ->and($snapshot['websocket_role_assignment'])->toMatchArray([
                'role' => 'websocket',
                'status' => 'active',
            ])
            ->and($snapshot['redis_node'])->toBe('app-dev-1')
            ...
            ->and($snapshot['app_websocket_public_route_count'])->toBe(0)
            ->and($route['source_hash'])->toBe($route['expected_source_hash'])
            ->and($config['protocol'])->toBe('websocket')
            ->and($config['router_upstream']['node_id'])->toBeInt()
            ->and($config['router_upstream']['node'])->toBe('gateway')
            ->and($config['router_upstream']['url'])->toBe('http://10.6.0.2:80')
            ->and($config['tls'])->toBe([
                'managed_by' => 'internal',
            ...
            ->and($backendPool[0]['url'])->toBe('https://ws-1.websocket.orbit:8080')
            ->and($config['upstreams'][0]['node_id'])->toBeInt()
            ->and($config['upstreams'][0]['node'])->toBe('ws-1')
            ->and($config['upstreams'][0]['scheme'])->toBe('https')
            ->and($config['upstreams'][0]['host'])->toBe('ws-1.websocket.orbit')
            ->and($config['upstreams'][0]['port'])->toBe(8080)
            ->and($config['upstreams'][0]['url'])->toBe('https://ws-1.websocket.orbit:8080')
        ...
        expect($runtime['name'])->toBe('orbit-websocket-ws-1')
            ...
            ->and($runtime['network_aliases'])->toBe([
