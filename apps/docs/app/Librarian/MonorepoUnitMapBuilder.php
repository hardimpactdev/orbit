...
final readonly class MonorepoUnitMapBuilder
{
    ...
    private const array UNIT_DEFINITIONS = [
        ...
        'apps-macos' => [
            ...
            'purpose' => 'macOS-only Tauri Orbit Desktop app. It owns the local Orbit Agent lifetime, login-item registration, tray/menu status, and verified desktop/Agent/CLI self-update on Darwin.',
            'start_when' => [
                'Changing the macOS Orbit Agent menu-bar runtime, tray/menu status behavior, Tauri config, native assets, Agent supervisor, login items, or desktop updater.',
            ...
            'do_not_start_when' => [
                ...
                'Changing gateway-side Agent command dispatch or operation-token verification endpoint implementation instead of the local Agent listener boundary.',
                'Adding arbitrary privileged shell execution, approval UI, WebSocket presence, or SSH/RemoteShell replacement behavior.',
            ],
            'owning_paths' => [
                'apps/macos',
                'apps/docs/content/architecture.md',
        ...
        'apps-ui' => [
            'type' => 'laravel-browser-ui',
            'purpose' => 'Fresh Launch-based Laravel 13/Inertia 3/React 19 browser UI for Orbit. It owns the local dashboard development surface and consumes the configured gateway through the PHP Saloon SDK.',
            'start_when' => [
                'Changing Orbit browser UI routes, Inertia pages, React components, Agentation setup, or UI-local gateway SDK integration.',
                'Developing the local browser UI at https://orbit.nmbp.',
            ],
            'do_not_start_when' => [
                'Changing gateway HTTP/API behavior or SDK request contracts; route to apps/gateway or packages/sdk.',
                'Claiming production app.orbit hosting, a production ui role, gateway delegation, request identity, or native macOS behavior; those surfaces are deferred.',
            ],
            'owning_paths' => [
                'apps/ui',
                'apps/docs/content/architecture.md',
            ...
            'verification' => [
                'preferred_commands' => [
                    ...
                    'cd apps/ui && vendor/bin/phpstan analyse --memory-limit=512M',
                    'cd apps/ui && vendor/bin/pint --test',
                    'cd apps/ui && vp run prepare',
                    'cd apps/ui && vp run build',
                ],
                'path_argument_rules' => [
                    'After cd apps/ui, pass Pest, PHPStan, and Pint paths relative to apps/ui; do not route UI checks through gateway Mago.',
                    'Use Bun for apps/ui client and SSR checks. Browser tests and Agentation interaction are separate browser acceptance evidence, not part of the deterministic quality gate.',
                    'The UI server-side SDK reads its configured gateway endpoint; browser requests must not carry a gateway or node identity field.',
                ],
                'manual_only_commands' => [],
            ],
            'tooling_base_path' => 'apps/ui',
        ],
        'apps-reverb' => [
        ...
        'packages-sdk-typescript' => [
            ...
            'verification' => [
                'preferred_commands' => [
                    'cd packages/sdk-typescript && vp run test',
                    'cd packages/sdk-typescript && vp run build',
                ],
                'path_argument_rules' => [
                    ...
                    'src/generated/schema.ts is generated from the gateway OpenAPI contract; regenerate it with vp run generate instead of hand-editing.',
                ],
                'manual_only_commands' => [],
            ],
