...
final readonly class UpdateAllGatewayStreamClient implements UpdateAllGatewayStream
{
    ...
    public function run(callable $onEvent): int|GatewayApiException
    {
        ...
        try {
            $response = ($this->client ?? new Client)->request('POST', "{$gatewayUrl}/api/update/all/stream", [
        ...
        if ($body !== '') {
            ...
            if (is_array($error)) {
                return new GatewayApiException(
                    message: is_string($error['message'] ?? null) ? $error['message'] : 'Gateway connection is required to update the fleet.',
