# Technical Contract: `node:grant` On An App Node
...
- The caller role has been resolved before command inputs are read.
- No command argument, option, gateway execution context, SSH target, or
  WireGuard identity prerequisite can make `node:grant` valid from an app node.

## Allowed Paths

None. `node:grant` may not run from an app node.
...
- Do not create or modify node access grants.
...
| Path | Coverage |
| --- | --- |
| `tests/Feature/Commands/Nodes/NodeGrantOnAppNodeContractTest.php` | Primary owner for app-caller rejection: explicit `general.local_node_role=app` callers fail before prompts, forwarding, local writes, SSH, WireGuard changes, gateway node-record reads, or other side effects. Renderer tests own the human and JSON formatting of that error. |
