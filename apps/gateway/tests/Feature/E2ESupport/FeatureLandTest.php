...
it('blocks LAND before a complete indexed checkpoint chain', function (): void {
    ...
    try {
        ...
        $loop = str_replace(
            "complete | {$tip}",
            'ready | none',
            file_get_contents($fixture['worktree'].'/.orbit/loop.md'),
        );
        ...
        expect($process->getExitCode())
            ...
            ->toContain('terminal phase requires every indexed slice to be complete')
            ->and(trim(new Process(['git', 'rev-parse', 'main'], $fixture['repo'])->mustRun()->getOutput()))
            ->toBe($mainBefore)
            ->and(glob($fixture['repo'].'/.orbit/sessions/*-feature') ?: [])
            ->toBe([]);
    } finally {
        land_remove_fixture($fixture);
    }
...
it('rejects malformed checkpoints at the LAND boundary', function (): void {
    ...
    try {
        ...
        land_write_accepted_loop($fixture['repo'], $fixture['worktree']);
        $loop = str_replace(
            'complete | '.trim(new Process(['git', 'rev-parse', 'HEAD'], $fixture['worktree'])->mustRun()->getOutput()),
            'complete | malformed',
            file_get_contents($fixture['worktree'].'/.orbit/loop.md'),
        );
        file_put_contents($fixture['worktree'].'/.orbit/loop.md', $loop);
...
it('rejects non ancestor checkpoints at the LAND boundary', function (): void {
    ...
    try {
        ...
        $side = trim(
            new Process(['git', 'commit-tree', $tree, '-m', 'side'], $fixture['worktree'])->mustRun()->getOutput(),
        );
        $tip = trim(new Process(['git', 'rev-parse', 'HEAD'], $fixture['worktree'])->mustRun()->getOutput());
        file_put_contents($fixture['worktree'].'/.orbit/loop.md', str_replace(
            $tip,
            $side,
            file_get_contents($fixture['worktree'].'/.orbit/loop.md'),
        ));
...
it('rejects backwards checkpoints at the landed cleanup boundary', function (): void {
    ...
    try {
        ...
        $loop = preg_replace(
            ...
            "| `.orbit/slices/01-one.md` | complete | {$c2} |\n| `.orbit/slices/02-two.md` | complete | {$c1} |",
            $loop,
        );
        file_put_contents($fixture['worktree'].'/.orbit/loop.md', $loop);
        unlink($fixture['worktree'].'/.orbit/slices/01-example.md');
        file_put_contents(
            $fixture['worktree'].'/.orbit/slices/01-one.md',
            "# Orbit Feature Slice\n\n- Slice: 01-one\n- Depends on: none\n\n## Outcome\n\n## Scope\n- Included: land\n- Excluded: handoffs\n\n## Authority\n- Decisions: land\n- Product docs: land\n\n## Proof\n- Focused: land\n",
        );
        file_put_contents($fixture['worktree'].'/.orbit/slices/02-two.md', str_replace(
            '01-one',
            '02-two',
            file_get_contents($fixture['worktree'].'/.orbit/slices/01-one.md'),
        ));
        file_put_contents($fixture['worktree'].'/.orbit/slices/02-two.md', str_replace(
            'Depends on: none',
            'Depends on: 01-one',
            file_get_contents($fixture['worktree'].'/.orbit/slices/02-two.md'),
        ));
        $mainBefore = trim(new Process(['git', 'rev-parse', 'main'], $fixture['repo'])->mustRun()->getOutput());
        $process = land_run_land($fixture, land_args($fixture, ['--one-step']));
        expect($process->getExitCode())
            ...
            ->toBe(0)
            ->and($process->getErrorOutput().$process->getOutput())
            ->toMatch('/02-two|backward|order/i')
            ->and(trim(new Process(['git', 'rev-parse', 'main'], $fixture['repo'])->mustRun()->getOutput()))
            ->toBe($mainBefore)
...
it('lands a valid multi slice checkpoint chain', function (): void {
    ...
    try {
        ...
        $loop = preg_replace(
            '/\| `\.orbit\/slices\/01-example\.md` \| complete \| [0-9a-f]{40} \|/',
            "| `.orbit/slices/01-one.md` | complete | {$c1} |\n| `.orbit/slices/02-two.md` | complete | {$c2} |",
            file_get_contents($fixture['worktree'].'/.orbit/loop.md'),
        );
        file_put_contents($fixture['worktree'].'/.orbit/loop.md', $loop);
        unlink($fixture['worktree'].'/.orbit/slices/01-example.md');
        ...
        file_put_contents($fixture['worktree'].'/.orbit/slices/02-two.md', str_replace(
            ['01-one', 'Depends on: none'],
            ['02-two', 'Depends on: 01-one'],
            file_get_contents($fixture['worktree'].'/.orbit/slices/01-one.md'),
        ));
        ...
        expect($process->getExitCode())
            ...
            ->toContain('phase=merge')
            ->and($process->getOutput().$process->getErrorOutput())
            ->not->toMatch('/slice|checkpoint/i');
    } finally {
        land_remove_fixture($fixture);
    }
});
...
function land_write_accepted_loop(string $repo, string $worktree, string $gate = 'docs-lint'): void
{
    ...
    file_put_contents(
        "{$worktree}/.orbit/slices/01-example.md",
        "# Orbit Feature Slice\n\n- Slice: 01-example\n- Depends on: none\n\n## Outcome\n\n## Scope\n- Included: land\n- Excluded: handoffs\n\n## Authority\n- Decisions: land\n- Product docs: land\n\n## Proof\n- Focused: land\n",
    );
}
