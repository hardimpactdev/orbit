...
it('creates the orbit network, writes php.ini, and runs the app runtime container when none exists', function (): void {
    ...
    expect($outcome)
        ...
        ->not->toContain('/home/orbit/apps/docs/.orbit/frankenphp')->and($scripts[4])->toContain('docker run -d')->and(
            $scripts[4],
        )->toContain("--env 'SERVER_NAME=:8080'")->and($scripts[4])->toContain(
            "--env 'XDG_CONFIG_HOME=/tmp/orbit-frankenphp/config'",
        )->and($scripts[4])->toContain("--env 'XDG_DATA_HOME=/tmp/orbit-frankenphp/data'")->and($scripts[4])
        ->not->toContain(' --publish ')->and($scripts[4])
...
it('creates the app-dev packages bind mount source before running the app runtime container', function (): void {
    ...
    expect($script)
        ->toContain("sudo install -d -m 0775 -o 'nckrtl' -g 'nckrtl' '/home/nckrtl/packages'")
        ->and($script)
        ->toContain("--mount 'type=bind,source=/home/nckrtl/packages,target=/packages'")
...
it('treats app-dev runtime TLS certificate mounts as Orbit-managed built-ins', function (): void {
    ...
    expect($script)
        ->toContain(
            "--mount 'type=bind,source={$certSource},target="
            .AppDevelopmentInnerTlsPolicy::RuntimeTlsCertContainerPath
            .",readonly'",
        )
        ->and($script)
        ->toContain(
            "--mount 'type=bind,source={$keySource},target="
            .AppDevelopmentInnerTlsPolicy::RuntimeTlsKeyContainerPath
            .",readonly'",
        )
        ->and($script)
        ->not->toContain("sudo install -d -m 0775 -o 'nckrtl' -g 'nckrtl' '{$certSource}'")->and($script)
...
it('resolves production runtime users to numeric uid gid before creating the container', function (): void {
    ...
    expect($outcome)
        ...
        ->toContain("id -g 'docs'")
        ->and($scripts[4])
        ->not->toContain('/home/docs/app/.orbit/frankenphp')->and($scripts[4])->toContain("--user '1001:1002'")->and(
            $scripts[4],
        )
        ->not->toContain('target=/data')->and($scripts[4])
        ->not->toContain('target=/config')->and($scripts[4])
        ->not->toContain('/var/run/docker.sock')->and($scripts[4])
...
it('throws a runtime-user exception before creating the container when the production runtime user is missing', function (): void {
    ...
    expect(collect($scripts)->contains(
        fn (string $script): bool => str_contains($script, 'docker run -d'),
    ))->toBeFalse();
});
...
it('verifies image presence on the matching-running ("Unchanged") path before returning healthy', function (): void {
    ...
    foreach ($scripts as $script) {
        expect($script)
            ->not->toContain('docker run -d')->and($script)
            ->not->toContain('docker rm -f')->and($script)
...
it('verifies image presence on the matching-stopped ("Started") path before starting the container', function (): void {
    ...
    $shell = new AppRuntimeRecordingShell(
        ...
        new RemoteShellResult(
            exitCode: 0,
            stdout: inspectPayloadForApp($container, running: false),
            stderr: '',
            durationMs: 1,
        ),
        // image inspect succeeds (image still present on node)
    ...
    expect($outcome)
        ->toBe(AppRuntimeContainerApplyOutcome::Started)
        ->and($scripts[1])
        ->toContain('docker container inspect')
        ->and($scripts[2])
        ->toContain("docker image inspect 'ghcr.io/hardimpactdev/orbit-frankenphp:1-php8.5-bookworm'")
...
it('recreates the container when the rendered spec drifts', function (): void {
    ...
    $shell = new AppRuntimeRecordingShell(
        ...
        new RemoteShellResult(
            exitCode: 0,
            stdout: inspectPayloadForApp($container, specHash: 'stale'),
            stderr: '',
            durationMs: 1,
        ),
        // image inspect succeeds (image present on node)
    ...
    expect($outcome)
        ->toBe(AppRuntimeContainerApplyOutcome::Recreated)
        ->and($scripts[1])
        ->toContain('docker container inspect')
        ->and($scripts[2])
        ->toContain("docker image inspect 'ghcr.io/hardimpactdev/orbit-frankenphp:1-php8.5-bookworm'")
...
it('returns AlreadyAbsent only when docker inspect reports "No such object"', function (): void {
    ...
    $shell = new AppRuntimeRecordingShell(
        new RemoteShellResult(
            exitCode: 1,
            stdout: '',
            stderr: 'Error response from daemon: No such object: orbit-app-docs',
            durationMs: 1,
        ),
    );
...
it(
    'throws AppRuntimeImageUnavailableException when the selected FrankenPHP image is not on the node before creating a new container',
    function (): void {
        [$app, $node] = appAndNodeForManagerTest();
        $container = renderTestAppContainer($app);
...
it(
    'throws AppRuntimeImageUnavailableException when image was pruned out from under a matching running container before returning Unchanged',
    function (): void {
        [$app, $node] = appAndNodeForManagerTest();
        $container = renderTestAppContainer($app);
        ...
        expect($caughtImage)
            ...
            ->toBeFalse()
            ->and(collect($scripts)->contains(fn (string $s): bool => str_contains($s, 'docker run -d')))
            ->toBeFalse();
    },
);

it(
    'throws AppRuntimeImageUnavailableException when image was pruned out from under a matching stopped container before starting it',
    function (): void {
        [$app, $node] = appAndNodeForManagerTest();
        $container = renderTestAppContainer($app);
...
it('throws AppRuntimeImageUnavailableException when image is not on the node before recreating a drifted container', function (): void {
    ...
    expect(collect($scripts)->contains(fn (string $s): bool => str_contains($s, 'docker rm -f')))
        ->toBeFalse()
        ->and(collect($scripts)->contains(fn (string $s): bool => str_contains($s, 'docker run -d')))
        ->toBeFalse();
});
...
it(
    'throws AppRuntimeContainerApplyException — NOT AppRuntimeImageUnavailableException — when the image probe fails for an unknown Docker error and the container is absent',
    function (): void {
        [$app, $node] = appAndNodeForManagerTest();
        $container = renderTestAppContainer($app);
        ...
        expect($caught)
            ->toBeInstanceOf(AppRuntimeContainerApplyException::class)
            ->and($caught)
            ->not
            ->toBeInstanceOf(AppRuntimeImageUnavailableException::class)
...
it(
    'throws AppRuntimeContainerApplyException with hadExistingContainer=true when image probe fails unknown over a matching running container (must NOT return Unchanged)',
    function (): void {
        [$app, $node] = appAndNodeForManagerTest();
        $container = renderTestAppContainer($app);
        ...
        expect(collect($scripts)->contains(fn (string $s): bool => str_contains($s, 'docker start')))
            ->toBeFalse()
            ->and(collect($scripts)->contains(fn (string $s): bool => str_contains($s, 'docker rm -f')))
            ->toBeFalse()
            ->and(collect($scripts)->contains(fn (string $s): bool => str_contains($s, 'docker run -d')))
...
it(
    'throws AppRuntimeContainerApplyException with hadExistingContainer=true when image probe fails unknown over a matching stopped container (must NOT return Started)',
    function (): void {
        [$app, $node] = appAndNodeForManagerTest();
        $container = renderTestAppContainer($app);
