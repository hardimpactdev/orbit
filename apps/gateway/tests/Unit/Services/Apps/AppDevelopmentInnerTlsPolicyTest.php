...
it('applies inner TLS only to app-dev PHP apps that are not production', function (): void {
    ...
    $appDevStatic = App::factory()->for($appDevNode, 'node')->static()->create([
        'name' => 'marketing',
    ]);

    expect($policy->appliesToApp($appDevPhp))->toBeTrue()
        ->and($policy->appliesToApp($appProdPhp))->toBeFalse()
        ->and($policy->appliesToApp($appDevStatic))->toBeFalse();
});
...
it('describes runtime TLS mounts and upstream transport for an app-dev route domain', function (): void {
    ...
    expect($domain)->toBe('docs.test')
        ->and($policy->runtimeTlsEnvironment($domain))->toBe([
        ...
        ->and($policy->runtimeTlsMounts($node, $domain))->toBe([
            [
        ...
        ->and($policy->runtimeUpstreamTlsConfig($node, $domain))->toBe([
            'trusted_by_gateway_ca' => true,
            'ca_path' => '/etc/orbit/ca/root.crt',
            'server_name' => 'docs.test',
        ])
        ->and($policy->runtimeUpstreamTransportDirectives($policy->runtimeUpstreamTlsConfig($node, $domain)))->toBe(<<<'CADDY'
        transport http {
...
it('describes workspace route domains and inherits the app-dev inner TLS policy from the parent app', function (): void {
    ...
    expect($policy->workspaceRouteDomain($workspace))->toBe('feature-a.demo.test')
        ->and($policy->appliesToWorkspace($workspace))->toBeTrue();
});
