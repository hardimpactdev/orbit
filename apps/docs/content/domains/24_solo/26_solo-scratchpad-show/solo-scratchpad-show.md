# `orbit solo:scratchpad:show`

Show one Solo scratchpad.

## Usage

```bash
orbit solo:scratchpad:show <scratchpad> [--json]
```

## Contract

`solo:scratchpad:show` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `GET /api/solo/scratchpad/show`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
