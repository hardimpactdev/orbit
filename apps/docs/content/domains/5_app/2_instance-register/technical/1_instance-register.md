...
```bash
orbit instance:register [app] [--node=] [--path=] [--root=] [--php-version=] [--runtime-proxy-transport=] [--domain=] [--json]
```

`--repo` is intentionally absent. In the current converted app and instance command surface,
...
creates the app with `repository=null` and its first instance.
...
5. **Resolve runtime proxy transport**:
   ...
   - Existing app runtime config when registered.
...
- **Production Activation**: If `--domain` is supplied:
  ...
  - If DNS or TLS prerequisites are not yet satisfied (propagation pending,
    ...
    app and selected-instance configuration persist, and the inactive domain
    is reported as a non-fatal warning under `success.meta.warnings[]` with
    `code=proxy.domain_inactive`, `family=proxy`, and a
    self-pointing `next_command=instance:register <app.instance> --domain=<host>`. The
...
- **Idempotence (Re-apply Refresh)**: `instance:register` always re-applies
  ...
  or adopted instance may still need app setup steps before it is healthy, and
...
- **Remote Execution Failures**: Agent-push timeout before configuration can be
  ...
  (`error.code=app.creation_failed`).
