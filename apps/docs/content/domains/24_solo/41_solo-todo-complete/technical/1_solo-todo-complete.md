...
- The caller has `solo:todo:write` on the serving gateway node.

## Signature

```bash
orbit solo:todo:complete <todo> [--project=<project>] [--title=<title>] [--body=<body>] [--force] [--json]
```
...
The command calls `POST /api/solo/todo/complete` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
