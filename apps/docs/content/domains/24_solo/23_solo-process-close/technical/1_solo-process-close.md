...
- The caller has `solo:process:delete` on the serving gateway node.
...
The command calls `DELETE /api/solo/process/close` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
