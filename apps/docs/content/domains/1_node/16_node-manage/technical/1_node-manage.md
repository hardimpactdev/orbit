...
| `GET` | `/api/nodes/self/manage-key` | authenticated roleless active node | Return the gateway management SSH public key. |
...
- Add a managed flag or transport object.
...
`node:manage` performs the first opt-in and reachability check for a roleless
operator node. Later drift in SSH host keys, user metadata, platform metadata,
or reachability belongs to [`doctor --family=node`](../../node-doctor.md).
