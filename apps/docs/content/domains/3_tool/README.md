...
| [`php-cli`](catalog/php-cli.md) | PHP CLI | Orbit-owned static host binaries (`coverage` on app-dev, `standard` on app-prod) | Installable/updatable host toolchain on `app-dev` and `app-prod` | `runtime` | install, update, probe, adopt |
