...
final readonly class DockerTopologyProvider implements E2ETopologyProvider
{
    ...
    private const int GatewaySiblingContainers = 1;
    ...
    public function availability(E2ETopologyKind $kind): ProviderAvailability
    {
        ...
        $selection = $this->selectHost($kind, checkCapacity: $this->config->dockerHostSlots === []);

        if ($selection['host'] !== null) {
            return ProviderAvailability::available(
                "docker prepared topology {$kind->value} is available on {$selection['host']->host}",
            );
        }
    ...
    public function acquire(
        ...
        try {
            $sourcePath = $this->sourceMountedCheckoutHostPath(
                $host,
                $options->sourceMountedCheckout,
                $timer,
                'docker.source-sync',
            );
            $networkPlan = $timer->measure('docker.network', fn (): DockerTopologyNetworkPlan => $this->createNetwork(
                $host,
                $network,
                $runId,
            ));

            $instances = $this->startContainers(
                $host,
                $kind,
                $runId,
                $network,
            ...
            $timer->measure('docker.seedClientCliGateways', fn () => $this->seedClientCliGateways(
                $instances,
                $networkPlan,
                $topologyMode,
            ));
            $timer->measure('docker.seedSshAccess', fn () => $this->seedRemoteShellSshAccess($instances));
            $timer->measure('docker.prepareGatewayState', fn () => $this->prepareGatewayState(
                $instances,
                $networkPlan,
                $topologyMode,
            ));
            $timer->measure('docker.startGatewayScheduler', fn () => $this->startGatewayScheduler($host, $instances));
            $timer->measure('docker.seedOperatorIdentity', fn () => $this->seedOperatorIdentity(
                $instances,
                $networkPlan,
                $topologyMode,
            ));
            $timer->measure('docker.seedGatewayRegistry', fn () => $this->seedGatewayRegistry(
                $kind,
                $instances,
                $networkPlan,
                $topologyMode,
            ...
            $timer->measure('docker.primeGatewayApi', fn () => $this->primeGatewayApi(
                $runId,
                $instances,
                $networkPlan,
            ));
        ...
        $rebuild = function (E2EPhaseTimer $cycleTimer) use (
            ...
            $newInstances = $this->startContainers(
                $host,
                $kind,
                $runId,
                $network,
            ...
            $cycleTimer->measure('reset.seedGatewayNameShim', fn () => $this->seedGatewayContainerNameShim(
                $newInstances,
            ));
            $cycleTimer->measure('reset.seedClientCliGateways', fn () => $this->seedClientCliGateways(
                $newInstances,
                $networkPlan,
                $topologyMode,
            ));
            $cycleTimer->measure('reset.seedSshAccess', fn () => $this->seedRemoteShellSshAccess($newInstances));
            $cycleTimer->measure('reset.prepareGatewayState', fn () => $this->prepareGatewayState(
                $newInstances,
                $networkPlan,
                $topologyMode,
            ));
            $cycleTimer->measure('reset.startGatewayScheduler', fn () => $this->startGatewayScheduler(
                $host,
                $newInstances,
            ));
            $cycleTimer->measure('reset.seedOperatorIdentity', fn () => $this->seedOperatorIdentity(
                $newInstances,
                $networkPlan,
                $topologyMode,
            ));
            $cycleTimer->measure('reset.seedGatewayRegistry', fn () => $this->seedGatewayRegistry(
                $kind,
                $newInstances,
                $networkPlan,
                $topologyMode,
            ...
            $cycleTimer->measure('reset.primeGatewayApi', fn () => $this->primeGatewayApi(
                $runId,
                $newInstances,
                $networkPlan,
            ));
        ...
        $bulkCleanup = fn (E2EPhaseTimer $cleanupTimer) => $this->cleanupContainersForRoles(
            $host,
            $roles,
            $runId,
            $cleanupTimer,
    ...
    {
        return E2ETopologyFacts::for($kind)->logicalRoles;
    }
    ...
    public static function gatewaySiblingImage(): string
    {
        return self::gatewayImage();
    }

    public static function gatewayImage(): string
    {
        return E2ETopologyArtifactNamespace::dockerRuntimeImage('orbit-gateway');
    }
    ...
    private function selectHost(E2ETopologyKind $kind, ?array $hostNames = null, bool $checkCapacity = true): array
    {
        ...
        foreach ($hostNames as $hostName) {
            ...
            if (self::rolesNeedGatewaySibling($roles) && ! $this->hasGatewaySiblingImage($host)) {
                $failures[] = "{$hostName}: docker gateway image ".self::gatewaySiblingImage().' is not available';
    ...
    private function hasGatewaySiblingImage(DockerHost $host): bool
    {
        return $host->run(
            sprintf('docker image inspect %s >/dev/null', escapeshellarg(self::gatewaySiblingImage())),
            timeoutSeconds: $this->dockerMetadataProbeTimeoutSeconds(),
        )->successful();
    }
    ...
    private function startContainers(
        ...
        $tasks = [];
        $sourceMountedCheckoutPath = $sourceMountedCheckout ? self::OrbitPath : null;
        ...
        foreach ($roles as $role) {
            ...
            $tasks[$role] = [
                ...
                'gateway_command' => $this->startsGatewaySibling($role)
                    ? $this->startGatewayContainerCommand($name, $network, $role, $sourcePath)
                    : null,
            ];
            $tasks[$role]['command'] = implode(
                ' && ',
                array_filter([
                    $tasks[$role]['node_command'],
                    $tasks[$role]['gateway_command'],
                ]),
            );
        }
        ...
        try {
            if (! $this->parallelStartsEnabled()) {
                ...
                foreach ($tasks as $role => $task) {
                    $timer->measure("{$timerPrefix}.{$role}", function () use ($host, $task): void {
                        $host->mustRun($task['node_command'], "Could not start container {$task['name']}");

                        if (is_string($task['gateway_command'])) {
                            $host->mustRun(
                                $task['gateway_command'],
                                "Could not start gateway container {$this->gatewayContainerName($task['name'])}",
                            );
                        }
                    });

                    $instances[$role] = new DockerInstance($host, $task['name'], $network, $sourceMountedCheckoutPath);
                }
            ...
            foreach ($tasks as $role => $task) {
                ...
                if (! $result->successful()) {
                    throw new \RuntimeException(
                        "Could not start container {$task['name']}: "
    ...
    private function startContainerCommand(
        DockerHost $host,
        string $name,
        string $network,
        string $role,
        ...
        $networkAlias = $topologyMode === 'dns-alias' ? ' --network-alias '.escapeshellarg($role) : '';
        $gatewayContainerEnv = $this->startsGatewaySibling($role)
            ? ' --env '.escapeshellarg("ORBIT_GATEWAY_CONTAINER={$this->gatewayContainerName($name)}")
            : '';
        $sourceMount = $sourcePath !== null ? ' --mount '.escapeshellarg($this->sourceBindMount($sourcePath)) : '';
    ...
    private function startGatewayContainerCommand(
        ...
        return sprintf(
            'docker run -d --restart unless-stopped --name %s --network %s --volume %s --mount %s%s --mount %s --env %s --env %s --env %s --env %s --env %s --env %s --env %s --workdir %s %s tail -f /dev/null',
            ...
            escapeshellarg(self::gatewaySiblingImage()),
        );
    }
     ...
     */
    private function seedGatewayContainerNameShim(array $instances): void
    {
        foreach ($instances as $instance) {
            $gatewayContainer = $this->gatewayContainerName($instance->name());
            ...
            E2ECommand::exec(
                ...
                "Could not install Docker gateway container name shim in Docker container {$instance->name()}",
                timeoutSeconds: 60,
            );
        }
    }
     ...
     * @param  array<string, DockerInstance>  $instances
     */
    private function seedGatewayRegistry(
        E2ETopologyKind $kind,
        array $instances,
        DockerTopologyNetworkPlan $networkPlan,
        string $mode,
        ...
        $seedTasks = $this->gatewayRegistrySeedTasks($kind, $instances, $networkPlan, $mode);

        if ($seedTasks['tasks'] === [] && $seedTasks['after_successful_tasks'] === []) {
            return;
        }

        $script = DockerDownstreamProvisioningScript::make($seedTasks['tasks'], $seedTasks['after_successful_tasks']);
        $scriptPath = '/tmp/orbit-e2e-docker-runtime-downstream.sh';
        ...
        E2ECommand::ssh(
            ...
            'orbit',
            new SshKeyPair('/dev/null', '/dev/null'),
            "cat > {$scriptPathArgument} <<'BASH'\n{$script}\nBASH\nchmod 755 {$scriptPathArgument} && bash {$scriptPathArgument}",
            timeoutSeconds: 300,
        );
    }
     ...
     *     after_successful_tasks: array<string, string>,
     * }
     */
    private function gatewayRegistrySeedTasks(
        E2ETopologyKind $kind,
        array $instances,
        DockerTopologyNetworkPlan $networkPlan,
        string $mode,
        ...
        return [
            'tasks' => $tasks,
            'after_successful_tasks' => $afterSuccessfulTasks,
        ];
    }
    ...
    {
        return $mode === 'dns-alias' ? ' --host-key-host='.$networkPlan->ipForRole($role) : '';
    }
     ...
     * @param  list<string>  $roles
     */
    private function cleanupContainersForRoles(
        DockerHost $host,
        array $roles,
        string $runId,
        ?E2EPhaseTimer $timer = null,
    ...
    private function prunePreparedGatewayRegistry(array $instances, ?SshKeyPair $key = null): void
    {
        ...
        $php = E2EPreparedTopology::gatewayRegistryPrunePhp(E2EPreparedTopology::gatewayNodeNamesForRoles(array_keys(
            $instances,
        )));
    ...
    private function containerNamesForRoles(array $roles, string $runId): array
    {
        ...
        foreach ($roles as $role) {
            array_push($names, ...$this->managedContainerNames(
                "{$this->config->instancePrefix}-{$runId}-{$role}",
                $role,
            ));
        }
    ...
    private function volumeNamesForRoles(array $roles, string $runId): array
    {
        ...
        foreach ($roles as $role) {
            array_push($names, ...$this->managedVolumeNames("{$this->config->instancePrefix}-{$runId}-{$role}"));
        }
    ...
    private function managedContainerNames(string $nodeContainer, string $role): array
    {
        ...
        if ($this->startsGatewaySibling($role)) {
    ...
    private function sourceMountedCheckoutHostPath(
        DockerHost $host,
        bool $sourceMountedCheckout,
        E2EPhaseTimer $timer,
        string $phase,
        ...
        return $timer->measure($phase, fn (): string => $this->sourceSyncer()->sync($host->host, 'docker'));
    }
    ...
    private function startsGatewaySibling(string $role): bool
    {
        return $role === 'gateway';
    }
    ...
    {
        return (
            (count($roles) * self::ContainersPerRole)
            + (self::rolesNeedGatewaySibling($roles) ? self::GatewaySiblingContainers : 0)
        );
    }
     ...
     * @param  list<string>  $roles
     */
    private static function rolesNeedGatewaySibling(array $roles): bool
    {
