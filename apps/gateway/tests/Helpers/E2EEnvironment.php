...
function dockerSocketGroupIdProcessFake(
    string $command,
    int $localGroupId = 999,
    ?int $remoteGroupId = null,
): ?ProcessResult {
    $remoteGroupId ??= $localGroupId + 1;

    if (
        str_starts_with($command, 'ssh -o BatchMode=yes')
    ...
    if (
        $command === DockerHost::localDockerSocketGroupIdCommand()
