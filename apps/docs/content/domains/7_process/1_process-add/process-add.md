...
- **Runtime Boundary**: App/workspace host-command processes use `systemd` on Linux. `docker-swarm` is only valid for node-owned managed service processes. `docker` is valid for service definitions and Orbit-managed runtime processes.
...
Rendering does not start the runtime unit. Systemd units are enabled but not
started. The `--start` flag is required to actually run the unit through the
