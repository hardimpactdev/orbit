...
`tool-reconfigure` changes gateway configuration intent only where the tool definition owns generated config, then performs command-owned enactment. [`tool-doctor.md`](../../tool-doctor.md) owns the authoritative tool-family probe, issue codes, fix map, and adopt map.
