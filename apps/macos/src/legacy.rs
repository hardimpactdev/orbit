...
mod tests {
    ...
    #[test]
    fn unrelated_hardimpact_launch_agents_are_not_migrated() {
        let decision = inspect_legacy_agent(
            &home(),
            Some(LegacyLaunchdState {
                label: "dev.hardimpact.orbit.macos".to_string(),
                plist_path: home().join("Library/LaunchAgents/dev.hardimpact.orbit.macos.plist"),
                program: Some(PathBuf::from(
                    "/Applications/Orbit Desktop.app/Contents/MacOS/orbit-macos",
                )),
                loaded: true,
                pid: Some(7),
            }),
        ...
        assert!(matches!(decision, LegacyDecision::Conflict { .. }));
    }
