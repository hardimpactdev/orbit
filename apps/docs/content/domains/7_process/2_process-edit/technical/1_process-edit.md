...
```bash
orbit process:edit [name] [--app=<app>] [--workspace=<workspace>] [--node=<node>] [--command=<command>] [--restart-policy=<never|on_failure|always>] [--crash-notification=<none|agent_ide>] [--runtime=<docker|docker-swarm|systemd>] [--restart] [--json]
```
