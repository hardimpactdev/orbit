...
describe('Manifest CLI commands', function (): void {
    it('puts the custom manifest URL to the gateway', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['manifest']['source'])->toBe('custom')
            ->and($decoded['success']['data']['manifest']['url'])->toBe('https://artifacts.example.com/channels/live-test/orbit-release-manifest.json');
    });
    ...
    it('deletes the custom manifest URL from the gateway', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ->and($decoded['success']['data']['manifest']['source'])->toBe('default')
            ->and($decoded['success']['data']['manifest']['custom_url'])->toBeNull();
    });
    ...
    it('renders manifest update as concise human output', function (): void {
        ...
        expect($exitCode)->toBe(0)
            ->and($output)->toContain('Release manifest URL updated.')
            ->and($output)->toContain('Source: custom')
            ->and($output)->toContain('URL: https://artifacts.example.com/channels/live-test/orbit-release-manifest.json')
    ...
    it('preserves gateway validation errors in json mode', function (): void {
        ...
        expect($exitCode)->toBe(1)
            ...
            ->and($decoded['error']['meta']['field'])->toBe('url');
    });
});
