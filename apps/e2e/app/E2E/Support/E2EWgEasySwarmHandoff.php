...
final readonly class E2EWgEasySwarmHandoff
{
    ...
    {
        E2ECommand::exec($gateway, sprintf(<<<'SH'
            set -euo pipefail
            handoff_complete=0
        ...
        )), "Could not stage prepared wg-easy state for the Swarm runtime on {$gateway->name()}", timeoutSeconds: 120);
    }

    public function complete(E2EInstance $gateway): void
    {
        E2ECommand::exec($gateway, <<<'SH'
            set -euo pipefail
            peer_snapshot=/run/orbit/wg-easy-handoff-peers.tsv
            ...
            sudo rm -f "$peer_snapshot"
            SH, "Could not remove the stopped prepared wg-easy container on {$gateway->name()}", timeoutSeconds: 60);
    }
    ...
    public function restoreStandalone(E2EInstance $gateway): void
    {
        E2ECommand::exec($gateway, <<<'SH'
            ...
            sudo rm -f "$peer_snapshot"
            SH, "Could not restore the prepared standalone wg-easy runtime on {$gateway->name()}", timeoutSeconds: 60);
    }
