 ...
 * ProxyRouteProbe.
 */
function legacyPhpFastCgiAppCaddyfile(
    string $domain,
    string $documentRoot,
    string $phpSocket,
    string $certPath,
...
function legacyPhpFastCgiPrivateBackendCaddyfile(
    int $port,
    string $domain,
    string $documentRoot,
    string $phpSocket,
...
it(
    ...
    function (): void {
        ...
        expect($config['runtime_upstream'] ?? null)
            ...
            ->toBe('/home/orbit/apps/legacy-docs/public')
            ->and($config['tls']['cert_path'])
            ->toBe('/etc/orbit/certs/legacy-docs.test.crt');
    },
);
...
it('does not backfill static app routes (they have no runtime_upstream)', function (): void {
    ...
    expect(array_key_exists('runtime_upstream', $config))
        ->toBeFalse()
        ->and($config['document_root'])
        ->toBe('/home/orbit/apps/legacy-marketing/public');
});
...
it(
    'updates each ingress backend_artifact source_hash from the legacy private-backend php_fastcgi hash to the Docker-first reverse_proxy hash',
    function (): void {
