...
describe('node permission registry', function (): void {
    ...
    it('recognizes known specific permissions', function (): void {
        ...
        expect($registry->isKnown('tool:read'))
            ...
            ->toBeTrue()
            ->and($registry->isKnown('doctor:verify'))
            ->toBeTrue()
            ->and($registry->isKnown('firewall_rule:read'))
            ->toBeTrue();
    });

    it('rejects unknown permissions', function (): void {
        expect(new NodePermissionRegistry()->isKnown('unknown:permission'))
            ->toBeFalse()
            ->and(new NodePermissionRegistry()->isKnown('tool:hack'))
            ->toBeFalse()
            ->and(new NodePermissionRegistry()->isKnown('app:exec'))
            ->toBeFalse()
            ->and(new NodePermissionRegistry()->isKnown('workspace:exec'))
            ->toBeFalse()
            ->and(new NodePermissionRegistry()->isKnown('role:update'))
            ->toBeFalse()
            ->and(new NodePermissionRegistry()->isKnown('invalid'))
            ->toBeFalse();
    });
    ...
    it('returns implied permissions for database umbrellas', function (): void {
        ...
        expect($registry->impliedBy('database:read'))
            ->toContain('database:list')
            ->and($registry->impliedBy('database:read'))
            ->toContain('database:schema')
            ->and($registry->impliedBy('database:read'))
            ->not->toContain('database:query')->and($registry->impliedBy('database:write'))->toContain(
                'database:add',
            )->and($registry->impliedBy('database:write'))->toContain('database:add-user')->and($registry->impliedBy(
                'database:write',
            ))->toContain('database:detach')->and($registry->impliedBy('database:write'))
            ->not->toContain('database:query:write')->and($registry->impliedBy('database:query:write'))->toContain(
                'database:query',
            );
    });
    ...
    it('checks whether permission sets allow a required permission', function (): void {
        ...
        expect($registry->allows(['tool:read'], 'tool:show'))
            ...
            ->toBeFalse()
            ->and($registry->allows(['instance:read'], 'instance:credentials'))
            ->toBeFalse()
            ->and($registry->allows(['instance:read'], 'instance-setup-step:list'))
            ->toBeTrue()
            ->and($registry->allows(['instance:write'], 'instance:credentials'))
            ->toBeFalse()
            ->and($registry->allows(['instance:write'], 'instance:setup'))
            ->toBeTrue()
            ->and($registry->allows(['instance:write'], 'instance-setup-step:add'))
            ->toBeTrue()
            ->and($registry->allows(['instance:write'], 'instance-setup-step:list'))
            ->toBeFalse()
            ->and($registry->allows(['instance:credentials'], 'instance:credentials'))
            ->toBeTrue()
            ...
            ->toBeTrue()
            ->and($registry->allows(['tool:update:agent-tools'], 'tool:update'))
            ->toBeFalse()
            ->and($registry->allows(['database:read'], 'database:tables'))
            ->toBeTrue()
            ->and($registry->allows(['database:read'], 'database:query'))
            ->toBeFalse()
            ->and($registry->allows(['database:query:write'], 'database:query'))
            ->toBeTrue()
            ->and($registry->allows(['database:write'], 'database:add-user'))
            ->toBeTrue()
            ->and($registry->allows(['database:write'], 'database:query:write'))
            ->toBeFalse()
            ->and($registry->allows(['node:*'], 'node:update'))
            ->toBeTrue()
            ->and($registry->allows(['*'], 'firewall_rule:write'))
            ->toBeTrue();
    });

    it('returns empty implied list for permissions without implications', function (): void {
        expect(new NodePermissionRegistry()->impliedBy('tool:credentials'))
            ->toBe([])
            ->and(new NodePermissionRegistry()->impliedBy('instance:credentials'))
            ->toBe([])
            ->and(new NodePermissionRegistry()->impliedBy('doctor:verify'))
            ->toBe([]);
    });
    ...
    it('returns namespace permissions for namespace wildcard', function (): void {
        ...
        expect($implied)
            ...
            ->toContain('node:show')
            ->and($implied)
            ->not->toContain('tool:read')->and($implied)
            ->not->toContain('node:*');
    });
    ...
    it('reports coverage correctly', function (): void {
        ...
        expect($registry->isCoveredBy('tool:list', 'tool:read'))
            ...
            ->toBeFalse()
            ->and($registry->isCoveredBy('database:show', 'database:read'))
            ->toBeTrue()
            ->and($registry->isCoveredBy('database:query', 'database:read'))
            ->toBeFalse()
            ->and($registry->isCoveredBy('node:read', '*'))
            ->toBeTrue()
            ->and($registry->isCoveredBy('tool:read', 'node:*'))
            ->toBeFalse()
            ->and($registry->isCoveredBy('node:show', 'node:*'))
            ->toBeTrue();
    });
    ...
    it('does not include global wildcard in implied results', function (): void {
        ...
        expect($registry->impliedBy('*'))
            ->not->toContain('*')->and($registry->impliedBy('node:*'))
            ->not->toContain('node:*');
    });
