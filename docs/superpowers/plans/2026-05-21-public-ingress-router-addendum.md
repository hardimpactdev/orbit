# Public Ingress Router Addendum

> Addendum to `docs/superpowers/plans/2026-05-21-public-ingress-role.md`.

## Status

This addendum changes the public-ingress implementation direction before the
initial public-ingress role lands.

The current plan routes public production traffic directly from
`public-ingress` to app-production backend pools. That should change.
`public-ingress` should be the public edge only. A new `router` role should own
...
- `public-ingress`: public 80/443 edge, public TLS, public hardening, and
...
```text
Browser
  -> public-ingress Caddy: public HTTPS and edge policy
...
```text
Browser
  -> public-ingress Caddy: public WSS and edge policy
...
## Superseded Public-Ingress Plan Items

The original plan says public production app routes have:

- public route artifacts on `public-ingress`;
- private backend artifacts on `app-production`;
- backend pools rendered by `public-ingress`.

Replace that with:

- public route artifacts on `public-ingress` terminate public TLS and forward
...
- public-ingress Caddy does not need to know the app-production backend pool.

The backend pool concept remains, but its owner moves from `public-ingress` to
`router`.
...
| --- | --- | --- |
| `gateway` | `vpn`, `router` | `app-development`, `app-production`, `database`, `agent`, `public-ingress` |
| `vpn` | `gateway`, `router` | `app-development`, `app-production`, `database`, `agent`, `public-ingress` |
| `router` | `gateway`, `vpn` | `app-development`, `app-production`, `database`, `agent`, `public-ingress` |
| `app-development` | `database` | `gateway`, `vpn`, `router`, `app-production`, `agent`, `public-ingress` |
| `app-production` | `public-ingress` | `gateway`, `vpn`, `router`, `app-development`, `database`, `agent` |
| `database` | `app-development` | `gateway`, `vpn`, `router`, `app-production`, `agent`, `public-ingress` |
| `agent` | none | `gateway`, `vpn`, `router`, `app-development`, `app-production`, `database`, `public-ingress` |
| `public-ingress` | `app-production` | `gateway`, `vpn`, `router`, `app-development`, `database`, `agent` |
...
`reverse_proxy`. The current public-ingress slice only needs HTTP/WebSocket
routing through `router`; full TCP service routing can be a follow-up router
feature.

## Public Ingress Baseline
...
The `public-ingress` baseline does not own backend-pool selection for
app-production or future websocket nodes. It forwards eligible public traffic to
`router`.

## Command And State Impact

Add to the public-ingress implementation plan:
...
- Change public-ingress route rendering so public route artifacts upstream to
  the active router, not directly to app-production nodes.
- Add router route rendering for private app backend pools.

Do not add the `websocket` role in the current public-ingress implementation
unless the public-ingress worker has already finished the router contract and
...
- public-ingress forwards branded public WebSocket hosts to `router`;
...
```text
visitor -> public-ingress -> router -> app node
```
...
topology work, not public-ingress validation.
...
- `node role:add <node> router` fails because `router` is gateway-coupled in
  v1.
- Public-ingress route artifacts forward to the active router over WireGuard.
