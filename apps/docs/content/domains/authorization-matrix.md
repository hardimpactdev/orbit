...
| `process:log` | `process:log` (`process:read` implies it) | node owner, or concrete instance serving node | Replaces removed public `process:log` / permission `process:log` with no aliases | `authorization_failed` | Standard missing-permission meta plus process and instance when applicable |
