...
it('builds operator_gateway prepared images through transient docker resources', function (): void {
    Process::fake([
        ...
        "docker run -d --cap-add NET_ADMIN --cap-add NET_BIND_SERVICE --name 'orbit-e2e-build-operator_gateway-control' --network 'orbit-e2e-build-operator_gateway' --ip '10.6.0.3' 'orbit-e2e-topology-runtime:current'" => Process::result(output: "control-id\n"),
        "docker run -d --cap-add NET_ADMIN --cap-add NET_BIND_SERVICE --name 'orbit-e2e-build-operator_gateway-gateway' --network 'orbit-e2e-build-operator_gateway' --ip '10.6.0.2' 'orbit-e2e-topology-runtime:current'" => Process::result(output: "gateway-id\n"),
        ...
        "docker exec 'orbit-e2e-build-operator_gateway-gateway' sh -lc 'systemctl stop caddy >/dev/null 2>&1 || true'" => Process::result(),
        ...
        "docker exec 'orbit-e2e-build-operator_gateway-gateway' sh -lc *nohup*" => Process::result(),
        ...
        "docker rm -f 'orbit-e2e-build-operator_gateway-control' >/dev/null 2>&1 || true" => Process::result(),
        "docker rm -f 'orbit-e2e-build-operator_gateway-gateway' >/dev/null 2>&1 || true" => Process::result(),
...
it('uses the configured instance prefix for transient resources but stable image tags', function (): void {
    Process::fake([
        ...
        "docker run -d --cap-add NET_ADMIN --cap-add NET_BIND_SERVICE --name 'ci-foo-build-operator_gateway-control' --network 'ci-foo-build-operator_gateway' --ip '10.6.0.3' 'orbit-e2e-topology-runtime:current'" => Process::result(output: "control-id\n"),
        "docker run -d --cap-add NET_ADMIN --cap-add NET_BIND_SERVICE --name 'ci-foo-build-operator_gateway-gateway' --network 'ci-foo-build-operator_gateway' --ip '10.6.0.2' 'orbit-e2e-topology-runtime:current'" => Process::result(output: "gateway-id\n"),
        ...
        "docker rm -f 'ci-foo-build-operator_gateway-control' >/dev/null 2>&1 || true" => Process::result(),
        "docker rm -f 'ci-foo-build-operator_gateway-gateway' >/dev/null 2>&1 || true" => Process::result(),
