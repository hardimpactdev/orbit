# `orbit solo:project:show`

Show one Solo project.

## Usage

```bash
orbit solo:project:show <project> [--json]
```

## Contract

`solo:project:show` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `GET /api/solo/project/show`; the gateway must also have the Solo extension enabled before execution reaches the node-local Solo API.
