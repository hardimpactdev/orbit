# Technical Contract: `orbit tool:stop <tool> [--app=<app>] [--node=<node>] [--node-transport=<transport>] [--json|--stream-json]`
...
```bash
orbit tool:stop <tool> [--app=<app>] [--node=<node>] [--node-transport=<transport>] [--json|--stream-json]
```
...
- Dispatches the selected tool definition's explicit stop script.
...
- `tool:stop` does not imply support for tool log streaming or reload commands.

### Scope Boundaries

`tool-stop` must not create apps, workspaces, processes, schedules, proxy
...
| Unsupported tool action | The selected tool definition does not support stop. | `error.code=tool.unsupported_action` |
...
`tool-stop` performs a lifecycle action that an operator requested explicitly.
...
| Path | Coverage |
| --- | --- |
| `apps/cli/tests/Feature/Commands/Tool/ToolWriteCommandTest.php` | CLI `tool:stop` target resolution, JSON envelope, stream request shape, and gateway error envelope pass-through. |
