...
```bash
orbit solo:timer:pause <timer> [--node=<node>] [--json]
```

## Contract

`solo:timer:pause` is available only when the local Solo extension is enabled with `orbit extension:enable solo`. The command calls the gateway Solo proxy route `POST /api/solo/timer/pause`; the gateway must also have the Solo extension enabled. Use `--node=<node>` to target that node's configured node-local Solo API; when omitted, Orbit uses local `node:default`, then the caller node.
