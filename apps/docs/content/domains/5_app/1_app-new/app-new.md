...
plan. After creating the app source, it atomically writes the app and
...
This command is the primary way to start a fresh app or spin up a new
...
- **Production Activation:** When `--domain` is provided, production placement
  ...
  through [`instance:register <app.instance> --domain=<host>`](../2_instance-register/instance-register.md),
...
- **Retry Safety:** If source creation fails, no app configuration is written; fix the
  ...
  `instance:register <app.instance>` or `doctor --family=instance --instance=<app.instance>
...
- [`orbit instance:register <app.instance>`](../2_instance-register/instance-register.md)
