...
| `instance` | `[instance]` | Always. | Never. | None. | Dotted instance selector. A bare project name or hostname is shorthand only when exactly one instance exists; otherwise fail with `validation_failed` and `meta.reason=instance_required`. |
...
- `doctor --family=instance --instance=<project.instance>` verifies the selected instance's
