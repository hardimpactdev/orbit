...
- **Runtime Unit Rendering**: Node-owned and workspace-owned definitions normally render one runtime unit. Instance-owned definitions render one main-instance unit and one unit for each registered workspace belonging to that same instance, all on the instance's serving node.
