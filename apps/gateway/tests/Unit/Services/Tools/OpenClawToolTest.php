...
it('configures secure gateway intent without printing tokens or installing a native service', function (): void {
    ...
    expect($install)
        ...
        ->toContain('OPENCLAW_SUPERVISOR_MODE=external')
        // Missing/zero-byte/whitespace-only token files regenerate (Hermes parity).
        ->toContain('tr -d "[:space:]"')
        ->toContain('if [ -z "$(tr -d "[:space:]" < "${TOKEN_FILE}" 2>/dev/null || true)" ]')
        ->not->toContain('if [ ! -f "${TOKEN_FILE}" ]')
