...
- The caller has `solo:lock:*` on the serving gateway node.
...
The command calls `DELETE /api/solo/lock/release` through the configured gateway client. Gateway execution requires gateway Solo extension state and the caller permission listed above.
