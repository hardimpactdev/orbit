...
it('installs OpenCode Server and creates an accessible OpenCode-backed workspace on Incus', function (): void {
    ...
    try {
        ...
        expect($install->successful())->toBeTrue()
            ->and($installData['tool'])->toMatchArray([
                'name' => 'opencode-server',
                'node' => 'app-dev-1',
                'state' => 'installed',
            ])
            ->and($installData['tool']['process'])->toMatchArray([
                'name' => 'opencode-server',
                'runtime' => 'systemd',
                'tool' => 'opencode',
            ]);
        ...
        expect($app->successful())->toBeTrue()
            ->and($appData['app'])->toMatchArray([
                'name' => $appName,
                'node' => 'app-dev-1',
                'path' => $appPath,
            ]);
        ...
        expect($agentIde->successful())->toBeTrue()
            ->and($agentIdePayload['success']['data']['agent_ide'])->toMatchArray([
                'adapter' => 'opencode',
                'source' => 'app',
        ...
        expect($workspace->successful())->toBeTrue()
            ->and($workspaceData['result'])->toBe(['action' => 'created'])
            ->and($workspaceData['workspace'])->toMatchArray([
                'name' => $workspaceName,
                'app' => $appName,
                'node' => 'app-dev-1',
                'path' => $workspacePath,
        ...
        $adapterSession = $topology->ssh(
            'gateway',
            sprintf('curl -fsS %s', escapeshellarg("http://{$state['app_node_host']}:4096/session/session-{$workspaceName}?directory={$workspacePath}")),
            timeoutSeconds: 60,
        );
