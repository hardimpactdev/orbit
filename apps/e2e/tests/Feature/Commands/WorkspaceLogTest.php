...
it('reads workspace run logs from a non-gateway caller through the gateway api', function (): void {
    ...
    try {
        ...
        $completedPayload = json_decode(
            trim($completedResult->output()),
            associative: true,
        ...
        expect($completedRun)
            ->toBeArray()
            ->and($completedRun['id'])
            ->toBe($runIds['completed'])
            ->and($completedRun['status'])
            ->toBe('completed')
            ->and($completedRun['steps'][0]['stdout'])
            ->toContain('Dependencies installed')
            ->and($completedRun['steps'][0]['duration_ms'])
            ->toBe(4000)
            ->and($failedRun)
            ->toBeArray()
            ->and($failedRun['id'])
            ...
            ->toBe('failed')
            ->and($failedRun['steps'][0]['stdout'])
            ->toContain('Dependency resolution failed')
            ->and($failedRun['steps'][0]['stdout_truncated'])
            ->toBeTrue()
            ->and($failedRun['steps'][0]['duration_ms'])
            ->toBe(8000);
    } finally {
        $topology->cleanup();
    }
